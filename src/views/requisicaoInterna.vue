  ...
  <div>
    ...
    <div class="row">
      ...
      <q-select
        ...
        class="margin1 col-2 opcoes"
        v-model="aplicacao"
        :options="lookup_forma_aplicacao"
      ...
      <q-select
        ...
        class="margin1 col-2 opcoes"
        v-model="motivo"
        :options="lookup_forma_motivo"
        option-value="cd_motivo_req_interna"
        option-label="nm_motivo_req_interna"
        label="Motivo"
      >
        <template v-slot:prepend>
          ...
          <h2 class="content-block" style="display: block">
            ...
            >
              ...
              <div class="margin1 borda-bloco col-2">
                {{
                  `Aplicação: ${
                    this.aplicacao.cd_aplicacao_produto
                      ? this.aplicacao.nm_aplicacao_produto
                      : ""
                  }`
                }}
              </div>
              <div class="margin1 borda-bloco col-2">
                {{
                  `Motivo: ${
                    this.motivo.cd_motivo_req_interna
                      ? this.motivo.cd_motivo_req_interna
                      : ""
                  }`
                }}
...
export default {
  ...
  async created() {
    ...
    ////////////////////////
    let lookup_forma_aplicacao = await Lookup.montarSelect(
      this.cd_empresa,
      315
    );
    this.lookup_forma_aplicacao = JSON.parse(
      JSON.parse(JSON.stringify(lookup_forma_aplicacao.dataset))
    );
    ////////////////////////
    let lookup_forma_motivo = await Lookup.montarSelect(this.cd_empresa, 2575);
    this.lookup_forma_motivo = JSON.parse(
      JSON.parse(JSON.stringify(lookup_forma_motivo.dataset))
    );
  ...
  methods: {
    ...
    async onEnviarSolicitacao() {
      try {
        ...
        notify("Salvando requisição");
      }
      var json_envia_pedido = {
        cd_departamento: this.departamento.cd_departamento
          ? this.departamento.cd_departamento
          : null,
        cd_aplicacao: this.aplicacao.cd_aplicacao_produto
          ? this.aplicacao.cd_aplicacao_produto
          : null,
        cd_motivo: this.motivo.cd_motivo_req_interna
          ? this.motivo.cd_motivo_req_interna
          : null,
      ...
      let [result_pedido] = await Incluir.incluirRegistro(
        "873/1350", //this.api,
        json_envia_pedido
      ); //pr_gera_requisicao_interna
      if (result_pedido != undefined && result_pedido.cd_requisicao_compra) {
        notify(
          `Requisição ${result_pedido.cd_requisicao_compra} enviada com sucesso`
        );
      } else if (result_pedido.Msg) {
        notify(`${result_pedido.Msg}`);
      } else {
        notify("Não foi possível enviar a requisição");
      }
