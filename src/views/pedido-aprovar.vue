    ...
    >
      ...
      <template #master-detail="{ data }">
        <MasterDetail
          ...
          :cd_parametroID="cd_parametro_detalhe"
          :cd_procedimentoID="cd_detalhe"
          :master-detail-data="data"
          :botao="botaoMasterDetail"
          :linha="linha"
...
export default {
  data() {
    return {
      ...
      botaoMasterDetail: {
        ...
        icone: "check",
        label: "Aprovar",
        tooltip: "Aprovar pedido",
        funcao: this.onClickAprovar(),
      },
      autoNavigateToFocusedRow: true,
      isReady: false,
      dataSourceConfig: [],
  ...
  methods: {
    ...
    async carregaDados() {
      ...
      this.cd_parametro_detalhe = dados.cd_parametro_detalhe;
    ...
    async onClickAprovar(e) {
      ...
      localStorage.cd_documento = this.linha.cd_documento
        ? this.linha.cd_documento
        : 0;
      if (e != undefined) {
        const buttonText = e.component.option("text");
        notify(
          `Aguarde... vamos ${this.capitalize(buttonText)} o pedido para você !`
        );
      } else {
        notify(`Aguarde... vamos aprovar o pedido para você !`);
      }

      localStorage.cd_cliente = 0;

      api = "";
      dados = await Menu.montarMenu(cd_empresa, 0, cd_api); //'titulo';
      ...
      // localStorage.cd_identificacao = 16288;

      //alert(localStorage.cd_identificacao);
