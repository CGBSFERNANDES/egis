<template>
  <div>
    <h2 class="content-block">Pesquisa</h2>

    <div class="content-block dx-card responsive-paddings fantasia-nome">
      <dx-form
        id="form"
        label-location="top"
        :form-data="formData"
...
export default {
  props: {
    ...
    //nm_fantasia_cliente: { type: String, default: 0 }
    nm_fantasia_cliente: {
      type: [String, Number],
      default: "",
    },
  ...
  methods: {
    ...
    onClick(e) {
      ...
      notify(
        `Aguarde... vamos ${this.capitalize(buttonText)} os dados para você !`
      );
      ...
      if (!this.formData.Nome == "") {
        localStorage.nm_razao_social = this.formData.Nome;
      }

      if (!this.formData.Fantasia == "") {
        localStorage.nm_fantasia = this.formData.Fantasia;
      } else if (this.nm_fantasia_cliente != "") {
        localStorage.nm_fantasia = this.nm_fantasia_cliente;
      }

      //alert(localStorage.nm_fantasia_produto);
