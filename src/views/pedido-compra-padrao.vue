<template>
  <div>
    ...
    <div class="margin1">
      <fornecedor
        :cd_usuario="cd_usuario"
        @SelectFornecedor="SelecionaFornecedor($event)"
        :ic_pesquisa="true"
      />
    </div>
    <!-- Cliente -->
...
export default {
  ...
  components: {
    ...
    fornecedor: () => import("./fornecedor.vue"),
  ...
  methods: {
    ...
    async SelecionaFornecedor(e) {
      localStorage.cd_tabela_preco = e.cd_tabela_preco;
      this.cd_fornecedor = e.cd_fornecedor;
      this.forma_pagamento = this.lookup_forma_pagamento.filter(
        (p) => p.cd_forma_pagamento == e.cd_forma_pagamento
      );
      await this.carregaDados();
