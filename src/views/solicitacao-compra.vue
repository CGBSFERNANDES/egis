<template>
  <div>
    ...
    <!-- Fornecedor -->
    <div class="margin1">
      <fornecedor :ic_pesquisa="true" @SelectFornecedor="SelecionaFornecedor($event)" />
    </div>
    <!-- Produto -->
    <div>
      <produtoComprado
    ...
    <div class="row">
      <q-select
        ...
        :disable="$store._mutations.SET_Usuario.ic_tipo_usuario === 'S' ? false : true"
      ...
      <div class="margin1">
        <q-btn-dropdown rounded color="primary" :label="`Total ${this.vl_total_produto_formatado}`">
          ...
          <h2 class="content-block" style="display: block">
            ...
            >
              ...
              <div class="margin1 borda-bloco col-2">
                {{
                  `Departamento: ${
                    this.departamento.cd_departamento ? this.departamento.nm_departamento : ''
                  }`
                }}
              </div>
              <div class="margin1 borda-bloco col-2">
                {{ `Necessidade: ${this.dt_necessidade ? this.dt_necessidade : ''}` }}
              </div>
        ...
        <div class="card-cesta row">
          ...
          >
            <q-card-section>
              ...
              {{ `Total: ${i.vl_total_item_formatado ? i.vl_total_item_formatado : '0'}` }}
              <br />
...
export default {
  ...
  components: {
    ...
    fornecedor: () => import('../views/fornecedor.vue'),
  },

  async created() {
    let lookup_forma_departamento = await Lookup.montarSelect(this.cd_empresa, 726)
    ...
    ;[this.departamento] = this.lookup_forma_departamento.filter(
      (d) => d.cd_departamento == this.$store._mutations.SET_Usuario.cd_departamento
    )
  },
  ...
  methods: {
    async carregaDados(showNotify) {
      ...
      if (!sApis == '') {
        ...
        this.dataSourceConfig = this.dataSourceConfig ? this.dataSourceConfig : []
    ...
    async onEnviarSolicitacao() {
      ...
      if (this.fornecedorSelecionado && !this.fornecedorSelecionado.cd_fornecedor) {
      ...
      if (this.dataSourceConfig.length == 0) {
        notify('Selecione ao menos um produto')
        return
      }
    ...
    async onPDF() {
      ...
      if (this.dataSourceConfig.length > 0) {
        try {
          ...
          this.dt_necessidade = formataData.DiaMesAno(String(this.dt_necessidade + 'T12:00'))
          this.ic_grid_cesta = true
          await funcao.sleep(1000)
    ...
    attDataCesta() {
      ...
      this.dataSourceConfig.map((c) => {
        ...
        this.vl_total_produto += c.vl_produto * c.qt_digitacao
    ...
    async attQtd(e) {
      ...
      e.rows.map((i) => {
        ...
        if (
          i.data.qt_digitacao == 0 &&
          this.dataSourceConfig.find((e) => e.cd_controle === i.data.cd_controle)
        ) {
          this.$refs.gridPadrao.instance.cellValue(i.rowIndex, 'qt_digitacao', 0.0)
          this.$refs.gridPadrao.instance.cellValue(i.rowIndex, 'vl_total_item', 0.0)
        }
        if (i.data.qt_digitacao > 0) {
          ...
          if (
            ...
            let alterouQtd = this.dataSourceConfig.find((e) => e.cd_controle === i.data.cd_controle)
            ...
            if (alterouQtd.qt_digitacao != i.data.qt_digitacao) {
              this.dataSourceConfig[alterouQtd.index].qt_digitacao = i.data.qt_digitacao
            }
          }
          let vl_total_item = i.data.qt_digitacao.toFixed(2) * i.data.vl_produto.toFixed(2)
