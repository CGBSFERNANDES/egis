<template>
  <div>
    ...
    >
      ...
      <DxHeaderFilter :visible="true" :allow-search="false" />
      <DxSearchPanel
        :visible="temPanel"
        :width="300"
    ...
    <div class="buttons-column" v-if="temD === true">
      <div>
        <DxButton
          :width="120"
          visible="false"
...
export default {
  ...
  methods: {
    ...
    alteracao: async function (e) {
      e.data.cd_usuario = localStorage.cd_usuario;
      // alert(e.data);
      // alert(e.data.cd_pedido_compra);
      // alert(e.data.cd_item_pedido_compra);
    ...
    async carregaDados() {
      ...
      notify(
        `Aguarde... estamos montando a consulta para você, aguarde !`,
        "",
        10
      );
    ...
    async onClickAprovar(e) {
      ...
      notify(
        `Aguarde... vamos ${this.capitalize(buttonText)} para você !`,
        "",
        1500
      );
      ...
      api = "";
      dados = await Menu.montarMenu(cd_empresa, 0, 136); //'titulo';
    ...
    onClickDeclinar(e) {
      const buttonText = e.component.option("text");
      notify(`Aguarde... vamos ${this.capitalize(buttonText)} para você !`);
      //Limpar os Dados para Você
      this.carregaDados();
    },
