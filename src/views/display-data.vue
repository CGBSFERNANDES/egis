    ...
    <div>
      ...
      <div class="row items-center">
        ...
        <div class="col">
          ...
          <transition name="slide-fade">
            <q-btn
              v-if="linha_selecionada && linha_selecionada.nm_nfe_link"
              style="float: right"
              round
              flat
              icon="task"
              color="positive"
              @click="onClickAbrirNfe(linha_selecionada)"
            >
              <q-tooltip>Visualizar DANFE</q-tooltip>
            </q-btn>
          </transition>
          <q-btn
            v-if="menu_relatorio"
...
export default {
  ...
  methods: {
    ...
    onClickAbrirNfe(linha_selecionada) {
      window.open(linha_selecionada.nm_nfe_link, "_blank");
    },
    ...
    async carregaDados() {
      ...
      try {
        const primeiroItem = this.dataSourceConfig?.[0];
        const TemDocumento = primeiroItem?.nm_documento_pdf;
        //var TemDocumento = this.dataSourceConfig[0].nm_documento_pdf;
        this.arquivo_abrir = TemDocumento !== undefined && TemDocumento !== null;
       ...
       // }
        ...
        // eslint-disable-next-line no-console
        console.error('Erro ao verificar nm_documento_pdf:',err);
      }
    },
