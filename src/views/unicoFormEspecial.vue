      ...
      >
        <template v-slot:append>
          <q-btn
            ...
            :title="`Pesquisar ${attr.nm_edit_label || attr.nm_atributo_consulta}`"
          />
        </template>
      </q-input>
    </div>
...
<!-- FIM DAS TABSHEETS DO FORMULARIO -->
    ...
    <div
       v-if="menuTabs.length" class="q-mt-md grid-align">
...
dashboardCards() {
  ...
  dialogModalComposicao (aberto) {
    if (!aberto) this.limparSelecaoGrid()
  },
    ...
    async consultar(payloadManual = null) {
      ...
      try {
       ...
       // console.log('relatório do menu:', this.cd_relatorio);
        ...
        await this.runHook("beforeFetchRows", { cd_menu: this.cd_menu, filtros: this.filtrosValores })
...
if (this.ic_modal_pesquisa === 'S') {
  ...
  if (chave && valorChave != null) {
    ...
    this.legendaAcoesGrid = [];
        ...
        //
        await this.runHook("afterFetchRows", { cd_menu: this.cd_menu, rows: this.rows })
        //
