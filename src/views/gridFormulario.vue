<template>
  <div>
        ...
        >
          <q-item-section>
            <q-item-label @click="onClickProcesso(acao)">
              <div>
                <q-icon
                  :name="acao.cd_form > 0 ? 'description' : 'manufacturing'"
                />
                {{ `${acao.nm_processo}` }}
              </div>
            </q-item-label>
    ...
    <div v-if="dataRow && ic_painel_form === 'S'" class="row">
      <q-field
        :label="selectedCaption(key)"
        class="col-2 margin1"
        rounded
        standout
        stack-label
        v-for="(value, key) in dataRow"
        :key="key"
      >
...
export default {
  ...
  data() {
    return {
      ...
      cd_form_capa: localStorage.cd_form,
      cd_documento: 0,
      cd_relatorio: 0,
      cd_cliente: 0,
      cd_filtro_form: 0,
  ...
  async mounted() {
    ...
    localStorage.cd_tipo_filtro = 0;
    localStorage.cd_documento = 0;
    this.api = this.cd_menuID ? "934/1447" : localStorage.nm_identificacao_api;
    this.cd_menu = this.cd_menuID || localStorage.cd_menu;
    this.carregaDados();
  },
  ...
  methods: {
    ...
    selectedCaption(selectedField) {
      const field = this.columns.find((f) => f.dataField === selectedField);
    ...
    async showMenu() {
      ...
      localStorage.cd_form_capa = dados.cd_form;
      this.cd_filtro_form = dados.cd_filtro_form;
      this.ic_painel_form = dados.ic_painel_form;
      this.dataAcoes = dados.Acoes
        ? JSON.parse(JSON.parse(JSON.stringify(dados.Acoes)))
        : null;
      if (
      ...
      //dados da coluna
      this.columns = dados.coluna
        ? JSON.parse(JSON.parse(JSON.stringify(dados.coluna)))
        : null;
      this.columns.map((c) => {
        if (c.ic_botao === "S") {
      ...
      this.tabs = dados.TabSheet
        ? JSON.parse(JSON.parse(JSON.stringify(dados.TabSheet)))
        : null;
    ...
    onNew() {
      localStorage.cd_form =
        parseInt(localStorage.cd_form_capa) > 0
          ? localStorage.cd_form_capa
          : localStorage.cd_form;
    ...
    async onRelatorio() {
      try {
        let json_relatorio = {
          ...
          cd_documento_form: this.dataRow.cd_documento,
          cd_processo: "",
          cd_usuario: localStorage.cd_usuario,
        };
        let [relatorio] = await Incluir.incluirRegistro(
    ...
    async fechaPopup() {
      ...
      await this.carregaDados();
      localStorage.cd_form = localStorage.cd_form_capa;
    },
    ...
    async onClickProcesso(acao) {
      localStorage.cd_form_capa = localStorage.cd_form;
      if (acao.cd_form > 0) {
        this.cd_form = acao.cd_form;
        this.cd_menu = acao.cd_menu;
        this.cd_api = acao.cd_api;
        this.pop_formulario = true;
        return;
      }
      this.loadAcoes = true;
