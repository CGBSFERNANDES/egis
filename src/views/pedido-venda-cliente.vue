  ...
  <div>
    ...
    <div class="row">
      ...
      <div>
        ...
        <q-btn
          v-if="forma_pagamento.cd_forma_pagamento === 1"
          round
          class="margin1"
          color="white"
          text-color="white"
          ...
          @click="onPIX()"
        >
          <img src="../../public/pix.png" alt="PIX" width="36" height="36" />
          <q-tooltip
            anchor="bottom middle"
            self="top middle"
            :offset="[10, 10]"
    ...
    <q-dialog v-model="popup_pix" persistent>
      <q-card>
        <q-card-section class="row items-center q-pb-none">
          <div class="text-h6">PIX QRCode</div>
          <q-space />
        ...
        <q-card-section class="row items-center q-pb-none">
          <PIXQRCode
            :chave="this.$store._mutations.SET_Usuario.chave_pix"
            :nome="this.$store._mutations.SET_Usuario.nm_empresa"
            :valor="vl_liquido"
            @close="popup_pix = false"
          ></PIXQRCode>
        </q-card-section>
      </q-card>
    </q-dialog>
...
export default {
  ...
  watch: {
    dataCesta() {
      this.attDataCesta();
      //this.qtd = novo.length;
      //this.produtos = this.dataCesta.length//novo.length;
    },
  },
  ...
  methods: {
    async carregaDados(showNotify) {
      ...
      if (!sApis == "") {
        ...
        if (this.dataSourceConfig[0].cd_forma_pagamento) {
          [this.forma_pagamento] = this.lookup_forma_pagamento.filter(
            (p) =>
              p.cd_forma_pagamento ==
              this.dataSourceConfig[0].cd_forma_pagamento
          );
        }
      }
    },
    ...
    async onEnviarPedido() {
      ...
      var json_envia_pedido = {
        cd_forma_pagamento: this.forma_pagamento.cd_forma_pagamento
          ...
          : null,
        //cd_tipo_frete: this.frete.cd_tipo_frete ? this.frete.cd_tipo_frete : null,
        //cd_condicao_pagamento: this.condicao_pagamento.cd_condicao_pagamento ? this.condicao_pagamento.cd_condicao_pagamento : null,
        //cd_transportadora: this.Transportadora.cd_transportadora ? this.Transportadora.cd_transportadora : null,
      ...
      if (result_pedido != undefined && result_pedido.cd_consulta) {
        notify(`Pedido ${result_pedido.cd_consulta} enviada com sucesso`);
