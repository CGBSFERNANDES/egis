IF EXISTS (SELECT name 
	   FROM   sysobjects 
	   WHERE  name = N'pr_egis_relatorio_retirada_caixa' 
	   AND 	  type = 'P')
    DROP PROCEDURE pr_egis_relatorio_retirada_caixa

GO

-------------------------------------------------------------------------------
--sp_helptext pr_egis_relatorio_retirada_caixa
-------------------------------------------------------------------------------
--pr_egis_relatorio_retirada_caixa
-------------------------------------------------------------------------------
...
--
------------------------------------------------------------------------------
create procedure pr_egis_relatorio_retirada_caixa
@cd_relatorio int   = 0,
@cd_parametro int   = 0, 
@json nvarchar(max) = '' 
...
--------------------------------------------------------------------------------------------------------------------------
 select
	  ...
	  sum(isnull(m.vl_movimento_caixa,0))                                as vl_movimento_caixa,
...
select
  @vl_total = sum(isnull(vl_movimento_caixa,0))
from
  #aux_retirada

select
   *,
   pc_retirada = cast(round(vl_movimento_caixa/@vl_total * 100,2) as decimal(25,2))
   into
   #Retirada_rel
from 
  #aux_retirada
...
set @html_geral = ' <div class="section-title">  
        ...
        <p style="display: inline; text-align: center; padding: 20%;">Retirada de Caixa</p>  
    </div>
	<table>
...
while exists ( select top 1 cd_controle from #Retirada_rel)
begin
	select top 1
     ...
     from #Retirada_rel
	 delete from #Retirada_rel where cd_controle = @id
 end
...
------------------------------------------------------------------------------
--exec pr_egis_relatorio_retirada_caixa 374,0,''
------------------------------------------------------------------------------
