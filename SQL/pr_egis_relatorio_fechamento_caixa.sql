IF EXISTS (SELECT name 
	   FROM   sysobjects 
	   WHERE  name = N'pr_egis_relatorio_fechamento_caixa' 
	   AND 	  type = 'P')
    DROP PROCEDURE pr_egis_relatorio_fechamento_caixa

GO

-------------------------------------------------------------------------------
--sp_helptext pr_egis_relatorio_fechamento_caixa
-------------------------------------------------------------------------------
--pr_egis_relatorio_fechamento_caixa
-------------------------------------------------------------------------------
...
--
------------------------------------------------------------------------------
create procedure pr_egis_relatorio_fechamento_caixa
@cd_relatorio int   = 0,
@cd_parametro int   = 0, 
@json nvarchar(max) = '' 
...
--Abertura do caixa---------------------------------------
	...
	select   
	   IDENTITY(int,1,1) as cd_controle,
        fc.dt_fechamento_caixa      as dt_fechamento_caixa,
	    fc.vl_fechamento_caixa      as vl_fechamento_caixa,
...
set @html_geral = ' <div class="section-title">  
        ...
        <p style="display: inline; text-align: center; padding: 20%;">Fechamento de Caixa</p>  
    </div>
	<table>
...
while exists ( select top 1 cd_controle from #fechamento_caixa_rel)
begin
	select top 1
     ...
     from #fechamento_caixa_rel
	 delete from #fechamento_caixa_rel where cd_controle = @id
 end
...
------------------------------------------------------------------------------
--exec pr_egis_relatorio_fechamento_caixa 374,0,''
------------------------------------------------------------------------------
