IF EXISTS (SELECT name 
	   FROM   sysobjects 
	   WHERE  name = N'pr_egis_relatorio_pedido_cancelado' 
	   AND 	  type = 'P')
    DROP PROCEDURE pr_egis_relatorio_pedido_cancelado

GO
  
-------------------------------------------------------------------------------  
--sp_helptext pr_egis_relatorio_pedido_cancelado  
-------------------------------------------------------------------------------  
--pr_egis_relatorio_pedido_cancelado
-------------------------------------------------------------------------------  
...
--  
------------------------------------------------------------------------------  
create procedure pr_egis_relatorio_pedido_cancelado 
@cd_relatorio int   = 0,  
@cd_parametro int   = 0,  
@json nvarchar(max) = ''   
...
---------------------------------------------------------------------------------------------------------------
    select 
	  identity(int,1,1)                             as cd_controle,
		...
		CONVERT(DECIMAL(18,2),(select sum(i.qt_item_pedido_venda * i.vl_unitario_item_pedido) from pedido_venda_item i where i.cd_pedido_venda = p.cd_pedido_venda) )as 'vl_total_pedido_venda',  
		dbo.fn_data_string(p.dt_cancelamento_pedido) as dt_cancelamento_pedido,
		...
		st.nm_status_pedido as nm_status_pedido,
    ...
    from
      ...
      left outer join Transportadora tp       on tp.cd_transportadora     = p.cd_transportadora
      left outer join Status_Pedido st        on st.cd_status_pedido      = p.cd_status_pedido
	  left outer join egisadmin.dbo.usuario u on u.cd_usuario             = p.cd_usuario
    where
...
declare 
	@qt_pedido_venda       int = 0,
	@vl_total_pedido_venda float = 0
select 
	@qt_pedido_venda        = count(cd_pedido_venda),
...
while exists( select Top 1 cd_controle from #CapaCancelado )
  begin
  select top 1   
      @id                      = cd_controle,
	  @html_cab_det = @html_cab_det +
		...
		'<tr class="tamanho">	
			...
			<td>'+cast(isnull(dbo.fn_formata_valor(vl_total_pedido_venda),'')as nvarchar(20))+'</td>
			<td>'+isnull(dt_cancelamento_pedido,'')+'</td>
			<td>'+isnull(ds_cancelamento_pedido,'')+'</td>
			<td>'+isnull(nm_vendedor_interno,'')+'</td>
			<td>'+isnull(nm_vendedor_externo,'')+'</td>
			<td>'+isnull(BaixaTotal,'')+'</td>	
			<td>'+isnull(CondicaoPagto,'')+'</td>	
			<td>'+isnull(Transportadora,'')+'</td>	
			<td>'+isnull(nm_status_pedido,'')+'</td>	
			<td>'+isnull(nm_usuario,'')+'</td>
	    </tr>'
     from #CapaCancelado
	 DELETE FROM #CapaCancelado WHERE cd_controle = @id
...
set @html_rodape ='
		<tr class="tamanho" style="font-weight: bold; font-size: 14px;">	
		    <td>Total</td>
		    <td>'+cast(isnull(@qt_pedido_venda,'')as nvarchar(20))+'</td> 
			<td></td>
			<td></td>
			<td>'+cast(isnull(dbo.fn_formata_valor(@vl_total_pedido_venda),'')as nvarchar(20))+'</td>
			<td></td>
...
--exec pr_egis_relatorio_pedido_cancelado 291,0,''
