IF EXISTS (SELECT name 
	   FROM   sysobjects 
	   WHERE  name = N'pr_relatorio_pedido_aguardando_logistica' 
	   AND 	  type = 'P')
    DROP PROCEDURE pr_relatorio_pedido_aguardando_logistica

GO
  
-------------------------------------------------------------------------------    
--sp_helptext pr_relatorio_pedido_aguardando_logistica    
-------------------------------------------------------------------------------    
--pr_relatorio_pedido_aguardando_logistica    
-------------------------------------------------------------------------------    
...
--    
------------------------------------------------------------------------------    
create procedure pr_relatorio_pedido_aguardando_logistica  
@cd_relatorio int   = 0,     
@cd_parametro int   = 0,    
@json nvarchar(max) = ''     
...
where    
  ...
  and  
  p.cd_pedido_venda not in (select cd_pedido_venda from Nota_Saida_Item where dt_cancel_item_nota_saida is null)  
  and    
...
group by    
 ...
 begin    
    select * from #Diario order by dt_pedido_venda, hr_inicial_pedido    
    return    
 end    
...
--------------------------------------------------------------------------------------------------------------    
set @html_geral = '<div>    
  <p class="section-title" style="text-align: center; padding: 8px;">Pedido Aguardando Logistica de '+cast(isnull(dbo.fn_data_string(@dt_inicial),'')as nvarchar(20))+' á '+cast(isnull(dbo.fn_data_string(@dt_final),'')as nvarchar(20))+'</p>    
 </div>    
 <div>    
    <table>    
      <tr>    
...
--exec pr_relatorio_pedido_aguardando_logistica