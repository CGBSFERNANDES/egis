...
IF EXISTS (SELECT name 
	   FROM   sysobjects 
	   WHERE  name = N'vw_nfe_chave_acesso_110' 
	   AND 	  type = 'V')
    DROP VIEW vw_nfe_chave_acesso_110
GO

CREATE VIEW vw_nfe_chave_acesso_110
------------------------------------------------------------------------------------
...
-- 26.10.2010 - Chave de acesso da versão anterior - Carlos Fernandes
------------------------------------------------------------------------------------
as
...
--  dbo.fn_strzero(isnull(snf.qt_serie_nota_fiscal,0),3)  as 'serie',
  ...
  dbo.fn_strzero(ns.cd_nota_saida,9)                    as 'nNF',
...
--     case when isnull(snf.cd_modelo_serie_nota,'')<>'' then snf.cd_modelo_serie_nota else '55' end
     ...
     dbo.fn_strzero(ns.cd_nota_saida,9) +
     dbo.fn_strzero(ns.cd_nota_saida,9),43)            as 'cDV',
...
--     case when isnull(snf.cd_modelo_serie_nota,'')<>'' then snf.cd_modelo_serie_nota else '55' end
     ...
     dbo.fn_strzero(ns.cd_nota_saida,9) +
     dbo.fn_strzero(ns.cd_nota_saida,9),43)             as 'ChaveAcesso'
...
-- Testando a View
------------------------------------------------------------------------------------
--select top 1000 * from vw_nfe_chave_acesso_110
------------------------------------------------------------------------------------
