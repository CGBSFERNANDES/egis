IF EXISTS (SELECT name 
	   FROM   sysobjects 
	   WHERE  name = N'pr_consulta_nfe_nota_saida' 
	   AND 	  type = 'P')
    DROP PROCEDURE pr_consulta_nfe_nota_saida
GO
...
-- 26.05.2014 - Novos Atributos & Ajustes para o novo Lay-out 3.10
-- 20.06.2015 - ajustes diversos - Carlos Fernandes
-- 07.02.2016 - acerto da série da nota fiscal - Carlos Fernandes
-- 02.09.2019 - Acerto Update Chave de acesso, apenas nota não validada - Fagner Cardoso
-----------------------------------------------------------------------------------------------------------------------------------
...
if @cd_serie_nota is null 
   set @cd_serie_nota = 0


--Atualiza a Chave de Acesso--------------------------------------------------------------------------------
--Carlos - 20.11.2012

update
  nota_saida
set
  cd_chave_acesso = vw.ChaveAcesso
from
  nota_saida ns
  inner join vw_nfe_chave_acesso  vw       with (nolock) on vw.cd_nota_saida             = ns.cd_nota_saida  
where
  isnull(ns.cd_chave_acesso,'')='' or vw.ChaveAcesso <> ns.cd_chave_acesso
  and
...
set @i110               = 0
set @sg_versao_nfe      = '2.00'

if @dt_inicial >= '01/01/2014'
   set @sg_versao_nfe = '3.10'

--select * from versao_nfe

select
  @cd_nota_saida_v110 = isnull(cd_nota_saida,0),
  @sg_versao_nfe      = isnull(sg_versao_nfe,@sg_versao_nfe)
from
...
set @i110 = 1 --Carlos em 09.11.2012

if @cd_nota_saida_v110>0 and @i110 = 0
...
from  
  ...
  left outer join vw_destinatario vwd           with (nolock) on vwd.cd_destinatario          = ns.cd_cliente and
                                                                 vwd.cd_tipo_destinatario     = ns.cd_tipo_destinatario
...
--  and isnull(ns.ic_nfe_nota_saida,'N') = case when @cd_nota_saida = 0 then 'N'              else isnull(ns.ic_nfe_nota_saida,'N') end  
  ...
  and isnull(ns.cd_serie_nota,0)=case when @cd_serie_nota = 0 then isnull(ns.cd_serie_nota,0) else @cd_serie_nota end
...
--------------------------------------------------------------------------------------------------------------------------------------------------
--nova rotina sem considerar a versão 110
--------------------------------------------------------------------------------------------------------------------------------------------------
--07.11.2012
...
--    substring(substring(vw110.chaveAcesso,4,len(vw110.chaveacesso)),1,1)       
--  else
    substring(substring(ns.cd_chave_acesso,4,len(ns.cd_chave_acesso)),1,1)       
...
--    substring(substring(vw110.chaveAcesso,4,len(vw110.chaveacesso)),len(substring(vw110.chaveAcesso,4,len(vw110.chaveacesso))) - 1,2)
--  else
    substring(substring(ns.cd_chave_acesso,4,len(ns.cd_chave_acesso)),len(substring(ns.cd_chave_acesso,4,len(ns.cd_chave_acesso))) - 1,2)
--  end
                                                                                                       as 'PosicaoFinal',  
...
--   end                                           as ChaveAcesso,  
  ...
  substring(ns.cd_chave_acesso,4,len(ns.cd_chave_acesso)) as ChaveAcesso, 
  --ns.cd_chave_acesso
...
from
  ...
  inner join pedido_venda pv    on pv.cd_pedido_venda = ni.cd_pedido_venda
  inner join tipo_pedido tp     on tp.cd_tipo_pedido  = pv.cd_tipo_pedido
  inner join nota_saida ns      on ns.cd_nota_saida   = ni.cd_nota_saida

where
  isnull(tp.ic_imposto_tipo_pedido,'N')='N' and
  isnull(ni.cd_pedido_venda,0)>0 and
  ns.dt_nota_saida between @dt_inicial and @dt_final  

delete from
  #NFE_
where
  cd_nota_saida in ( select cd_nota_saida from #NotaTipoPedido_ )
...
------------------------------------------------------------------------------
--exec pr_consulta_nfe_nota_saida 88679,'09/02/2019','09/02/2019'
------------------------------------------------------------------------------
