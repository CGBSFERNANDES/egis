IF EXISTS (SELECT name 
	   FROM   sysobjects 
	   WHERE  name = N'vw_nfe_cofins_st_nota_fiscal' 
	   AND 	  type = 'V')
    DROP VIEW vw_nfe_cofins_st_nota_fiscal
GO

CREATE VIEW vw_nfe_cofins_st_nota_fiscal
------------------------------------------------------------------------------------
--sp_helptext vw_nfe_cofins_st_nota_fiscal
------------------------------------------------------------------------------------
...
select
  ...
  isnull(CONVERT(varchar, convert(numeric(14,2),round(case when nsi.pc_cofins > 0 then nsi.pc_cofins else null end,6,2)),103),'')           as 'pCOFINS',
...
-- Testando a View
------------------------------------------------------------------------------------
--select top 1000 * from vw_nfe_cofins_st_nota_fiscal
------------------------------------------------------------------------------------
