...
select                            
  ...
  case when isnull(nsi.cd_produto,0)=0 then                            
    ...
    case when isnull(snf.ic_medida_codigo,'N')='S' then isnull(( select top 1 vwp.codigo_produto from vw_medida_produto_item_nota vwp where vwp.cd_pedido_venda = nsi.cd_pedido_venda and vwp.cd_item_pedido_venda = nsi.cd_item_pedido_venda),nsi.cd_mascara_produto)                           else                            
 ...
 case when isnull(pc.ic_nf_produto_cliente,'N')='S'                            
    then case when isnull(pc.nm_produto_cliente,'') <> ''                            
           then isnull(pc.nm_produto_cliente,'')                            
     else ltrim(rtrim(nsi.nm_produto_item_nota))                            
        end                            
 else 
	 cast(
	 case when (isnull(snf.ic_medida_codigo,'N')='S') then
		 case when pv.cd_tipo_pedido = 7 and ltrim(rtrim(pvim.nm_fantasia_produto)) = '260'
				then 'FILM DE PVC ALIMENT AMA'
				else case when pv.cd_tipo_pedido <> 7 and ltrim(rtrim(pvim.nm_fantasia_produto)) = '185'
							 then 'FILM DE PVC REC AMA'
							 else case when pv.cd_tipo_pedido = 7 and ltrim(rtrim(pvim.nm_fantasia_produto)) = '261'
											then 'FILM DE PVC ALIMENT AZ'
											else case when pv.cd_tipo_pedido <> 7 and ltrim(rtrim(pvim.nm_fantasia_produto)) = '186'
														 then 'FILM DE PVC REC AZ'
														 else rtrim(ltrim(isnull(nsi.nm_produto_item_nota,'')))
													 end
										 end
						 end
			 end
 ...
 case when isnull(snf.ic_medida_codigo,'N')='S' then   
      ' '+( select top 1 vwp.cd_rec_medida   
                 from vw_medida_produto_item_nota vwp   
                where vwp.cd_pedido_venda = nsi.cd_pedido_venda and vwp.cd_item_pedido_venda = nsi.cd_item_pedido_venda )    
 else  
   ''  
 end  
 +                       
 ...
 case when fam.nm_familia_produto <> 'LINHA DOMÉSTICA' then --Fagner                        
  ...
  case when isnull(nsi.cd_servico,0)<>0 then                            
    ...
    case when isnull(opf.ic_anulacao_frete,'N')='N' then                            
      ...
      replicate('0',2 - len(rtrim(ltrim(replace(cf.cd_mascara_classificacao,'.','')))))                            
    else                  
      ''                            
    end                            
...
--      cast('40' as varchar(2))                                      
  ...
  case when ti.cd_digito_tributacao_icms = '70' then                             dbo.fn_mascara_valor_duas_casas(nsi.vl_base_icms_item)                                             
  else                                      
    '0.00'                                              
  end       as 'vBC70',                                                
  case when ti.cd_digito_tributacao_icms = '70' then                                       
...
from                            
  ...
  left outer join Pedido_Venda_Item_Medida pvim     with (nolock) on pvim.cd_pedido_venda       = nsi.cd_pedido_venda 
                                                                 and pvim.cd_item_pedido_venda  = nsi.cd_item_pedido_venda                 
...
-- Testando a View                
------------------------------------------------------------------------------------                
--select top 1 * from vw_nfe_produto_servico_nota_fiscal where cd_nota_saida = 16027
--select * from vw_nfe_produto_servico_nota_fiscal where cd_nota_saida = 9501477                
--select utrib, qtrib, vuntrib, * from vw_nfe_produto_servico_nota_fiscal where cd_identificacao_nota_saida = 73457
---------------------------------------------------------------------------------- 