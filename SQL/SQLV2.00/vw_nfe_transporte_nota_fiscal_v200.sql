IF EXISTS (SELECT name 
	   FROM   sysobjects 
	   WHERE  name = N'vw_nfe_transporte_nota_fiscal' 
	   AND 	  type = 'V')
    DROP VIEW vw_nfe_transporte_nota_fiscal
GO
...
-- 18.07.2012 - Tipo de Mercado - Carlos Fernandes
-- 11.03.2014 - Placa - Carlos Fernandes 
-- 20.05.2014 - Acerto do cnpj da Transportadora - Carlos Fernandes
-- 01.02.2018 - Acerto referente ao modFrete, Estava sempre Saindo 0 ou 1
-- 01.02.2018 - Acerto das casas decimais PesoL e PesoB, correto 15,3
-- 02.02.2018 - acerto do Flag da transportadora - emitente - Carlos Fernandes/Fagner
-- 26.04.2019 - Verificação para não passar a transportadora - Carlos Fernandes
-- 12.09.2022 - Nova regra do SEFAZ, quando o frete for CIF tem que utilizar a modalidade correta 
--              (0 - transportadora terceirizada e 3 - transportadora própria) - Pedro Jardim
---------------------------------------------------------------------------------------------------------
as
...
--                                                               as 'modFrete',
	case when (e.cd_cgc_empresa = t.cd_cnpj_transportadora) and (tp.cd_identifica_nota_fiscal = 0)
			 then '3' else isnull(tp.cd_identifica_nota_fiscal,1) end                        as 'modFrete',
  ...
  case when t.cd_tipo_mercado=1 then 'N' else 'S' end as ic_mercado_externo,

  ns.cd_transportadora,

  case when isnull(t.ic_destinatario,'N')='N' OR isnull( vw.cd_cnpj,'') = '' or vw.cd_tipo_mercado = 2 then
     ...
     case when t.cd_tipo_pessoa = 1 then
       case when isnull(vw.cd_cnpj,'')<>'' and vw.cd_tipo_mercado = 1 then
         cast(replace(replace(replace(vw.cd_cnpj,'-',''),'.',''),'/','') as varchar(15))
       else
         cast(replace(replace(replace(t.cd_cnpj_transportadora,'-',''),'.',''),'/','') as varchar(15))       end
     else
       cast('' as varchar(14))
     end
  end                                                                                               as 'CNPJ',
  ...
  case when isnull(t.ic_destinatario,'N')='N' then
     ...
     case when t.cd_tipo_pessoa = 2 and vw.cd_tipo_mercado = 1 then
       cast(replace(replace(replace(vw.cd_cnpj,'-',''),'.',''),'/','') as varchar(15))
     else
       cast('' as varchar(11) )
     end                                                                                         
  ...
  then
    ...
    case when isnull(t.ic_ie_isento_transportadora,'N')='S' then
      ...
      case when isnull(t.ic_destinatario,'N')='N' or isnull(vw.cd_inscestadual,'') = '' then
        cast(replace(replace(replace(t.cd_insc_estadual,'-',''),'.',''),'/','') as varchar(15))      
      else
        case when isnull(vw.cd_inscestadual,'')<>'' then
  ...
  case when isnull(t.ic_destinatario,'N')='N' then
     ...
     case when isnull(vw.nm_cidade,'')<>'' then
  ...
  'X18'                                                                                                     as 'VeicTrans',
  rtrim(ltrim(cast(replace(isnull(ns.cd_placa_nota_saida,'XXX9999'),'-','') as varchar(80))))               as 'Placa',
  ...
  isnull(CONVERT(varchar, convert(numeric(15,3),round(case when ns.qt_peso_liq_nota_saida> 0 then ns.qt_peso_liq_nota_saida else null end,15,3)),103),'0.000') as 'pesoL',
...
--  isnull(CONVERT(varchar, convert(int,case when ns.qt_peso_bruto_nota_saida > 0 then ns.qt_peso_bruto_nota_saida else null end),103),'') as 'pesoB',
  isnull(CONVERT(varchar, convert(numeric(15,3),round(case when ns.qt_peso_bruto_nota_saida> 0 then ns.qt_peso_bruto_nota_saida else null end,15,3)),103),'0.000') as 'pesoB',
  --Egis não tem----------------------------
  ...
  --Dados da Cobrança------------------------
   ...
   isnull(opf.ic_opinterestadual_op_fis,'N')                        as ic_opinterestadual_op_fis,
   isnull(tp.cd_sped_fiscal,0)                                      as cd_sped_fiscal
...
--select * from transportadora where cd_transportadora = 988
--select * from vw_destinatario

------------------------------------------------------------------------------------
-- Testando a View
------------------------------------------------------------------------------------
--select top 1000 * from vw_nfe_transporte_nota_fiscal where cd_nota_saida = 3080
--select top 1000 * from vw_nfe_transporte_nota_fiscal where cd_identificacao_nota_saida = 23974
------------------------------------------------------------------------------------
--select * from nota_saida_parcela where cd_parcela_nota_saida > 1
------------------------------------------------------------------------------------
--select * from tipo_pessoa

-- select 
--   len(cast(ds_obs_compl_nota_saida as varchar(8000))) as tam,
