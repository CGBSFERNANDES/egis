IF EXISTS (SELECT name 
	   FROM   sysobjects 
	   WHERE  name = N'pr_egis_relatorio_vendedor_empresa' 
	   AND 	  type = 'P')
    DROP PROCEDURE pr_egis_relatorio_vendedor_empresa

GO

-------------------------------------------------------------------------------
--sp_helptext pr_egis_relatorio_vendedor_empresa
-------------------------------------------------------------------------------
--pr_egis_relatorio_vendedor_empresa
-------------------------------------------------------------------------------
...
--
------------------------------------------------------------------------------
create procedure pr_egis_relatorio_vendedor_empresa
@cd_relatorio int   = 0,
@json nvarchar(max) = '' 
...
select
  ...
  n.cd_vendedor,
  n.cd_serie_nota,  
  sum( isnull(i.vl_total_item,0) +  isnull(i.vl_ipi,0) + isnull(i.vl_icms_subst_icms_item,0))                     as vl_total_item,
...
into
  #FaturamentoEmpresaVendedor

from
  nota_saida n
  left outer join nota_saida_empresa ne  on ne.cd_nota_saida         = n.cd_nota_saida
  ...
  left outer join empresa_faturamento ef on ef.cd_empresa            = case when isnull(ne.cd_empresa,0)=0 then snf.cd_empresa_selecao else ne.cd_empresa end
...
select
  ...
  *,
  pc_faturamento = cast(round(vl_total_item/@vl_total,2)*100 as decimal(25,2))
into
  #FinalFaturamentoEmpresaVendedor
from 
  #FaturamentoEmpresaVendedor
order by
  vl_total_item desc
...
while exists ( select top 1 cd_controle from #FinalFaturamentoEmpresaVendedor )
begin

  select 
    top 1
    @id              = cd_controle,
	@dados_relatorio = @dados_relatorio + '["'+nm_fantasia_empresa+'", '+cast(vl_total_item as varchar(30))+'],',
    ...
    @html_detalhe = @html_detalhe + '
			...
			<td style="font-size:12px; text-align:center;width: 20px">'+isnull(nm_fantasia_empresa,'')+'</td>
			<td style="font-size:12px; text-align:center;width: 20px">'+dbo.fn_formata_valor(isnull(vl_total_item,0))+'</td>			
			...
			<td style="font-size:12px; text-align:center;width: 20px">'+cast(isnull(qt_familia_produto,0) as varchar(20))+'</td>			
            </tr>'
  ...
  delete from #FinalFaturamentoEmpresaVendedor
  where
    cd_controle = @id
...
set @html_titulo = '     <div class="section-title">  
...
set @html_totais = '<div class="section-title"><strong>'+@titulo_total+'</strong></div>
                    <div> 
                    <tr>					
					<p style="font-size:18px; text-align:left;">Clietes: '+cast(@qt_total as varchar(10))+'</p>
			        <p style="font-size:18px; text-align:left;">Valor: '+'R$ '+dbo.fn_formata_valor(@vl_total)+'</p>
					</tr>
					</div>'
...
--select @html, @html_empresa as html_empresa, @html_titulo, @html_cab_det, @html_detalhe, @html_rod_det, @html_totais, @html_grafico, @html_rodape
...
------------------------------------------------------------------------------
--exec pr_egis_relatorio_vendedor_empresa 22,''
------------------------------------------------------------------------------
