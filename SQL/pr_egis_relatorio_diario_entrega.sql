IF EXISTS (SELECT name 
	   FROM   sysobjects 
	   WHERE  name = N'pr_egis_relatorio_diario_entrega' 
	   AND 	  type = 'P')
    DROP PROCEDURE pr_egis_relatorio_diario_entrega

GO

-------------------------------------------------------------------------------
--sp_helptext pr_egis_relatorio_diario_entrega
-------------------------------------------------------------------------------
--pr_egis_relatorio_diario_entrega
-------------------------------------------------------------------------------
...
--
------------------------------------------------------------------------------
create procedure pr_egis_relatorio_diario_entrega
@cd_relatorio int   = 0,
@cd_parametro int   = 0, 
@json nvarchar(max) = '' 
...
where
 r.dt_romaneio = @dt_hoje
 
------------------------------------------------------------------------------------------------------------		 
   
 if isnull(@cd_parametro,0) = 1  
 begin  
    select * from #DiarioEntregas  
  return  
 end  
...
WHILE EXISTS (SELECT TOP 1 cd_controle FROM #DiarioEntregas)
BEGIN
    SELECT TOP 1
        @id                          = cd_controle,
	...
	FROM #DiarioEntregas
    DELETE FROM #DiarioEntregas WHERE cd_controle = @id
END
--------------------------------------------------------------------------------------------------------------------
...
------------------------------------------------------------------------------
--exec pr_egis_relatorio_diario_entrega 345,0,''
------------------------------------------------------------------------------
