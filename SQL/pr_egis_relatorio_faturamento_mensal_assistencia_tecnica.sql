IF EXISTS (SELECT name 
	   FROM   sysobjects 
	   WHERE  name = N'pr_egis_relatorio_faturamento_mensal_assistencia_tecnica' 
	   AND 	  type = 'P')
    DROP PROCEDURE pr_egis_relatorio_faturamento_mensal_assistencia_tecnica
...
--Alteração        :
------------------------------------------------------------------------------  
create procedure pr_egis_relatorio_faturamento_mensal_assistencia_tecnica  
@cd_relatorio int   = 0,  
@cd_usuario   int   = 0,  
@cd_parametro int   = 0,  
@json nvarchar(max) = ''   
...
declare @cd_mes_inicial int = 0  
declare @cd_mes_final   int = 0  
declare @cd_ano_anterior int = 0  
  
set @cd_mes_inicial = month(@dt_inicial) set @cd_mes_final   = month(@dt_final)  
        
--Anterior--  
set @cd_ano_anterior      = @cd_ano - 1  
set @dt_inicial_anterior = dbo.fn_data_inicial(@cd_mes,@cd_ano)        
set @dt_final_anterior   = dbo.fn_data_final(@cd_mes,@cd_ano)     
  
select  
  @vl_meta = sum(vl_periodo)  
from  
  Objetivo_Tema_Valores v  
where  
  cd_objetivo = 1  
  and  
  cd_mes between @cd_mes_inicial and @cd_mes_final  
...
select  
    ...
    year(os.dt_entrega)                                     as cd_ano,  
 month(os.dt_entrega)                                    as cd_mes,  
 SUM(isnull(os.vl_final_ordem_servico,0) )    as vl_total,  
 COUNT(distinct o.cd_cliente)             as qt_cliente,  
 MAX(os.dt_entrega)                                      as dt_agenda,  
 count(distinct o.cd_ordem_servico)         as cd_ordem_servico  
  
  into #FaturamentoMensal  
  
  from  
    ordem_servico o  
 left outer join ordem_Servico_orcamento os on os.cd_ordem_servico = o.cd_ordem_servico  
  
  where  
    os.dt_entrega between @dt_inicial and @dt_final  
...
select  
  ...
  @vl_total_anterior  =   
  sum(case when isnull(o.vl_final_ordem_servico,0)>0 then  
    o.vl_final_ordem_servico  
  else  
      case when isnull(o.vl_pagamento,0)>0   
      then o.vl_pagamento  
      else  
...
where  
  o.dt_entrega between @dt_inicial_anterior and @dt_final_anterior  
...
where  
  o.dt_entrega between @dt_inicial and @dt_final  
  
  --((Faturamento - Meta) / Meta) * 100  
--select  
  --@pc_crescimento = case when @vl_total_anterior>0 then (@vl_total - @vl_total_anterior)/@vl_total_anterior * 100 else 0.00 end  
  
  select  
    @pc_crescimento = case when @vl_meta>0 then (@vl_total - @vl_meta)/@vl_meta * 100 else 0.00 end  
...
begin  
  ...
  begin  
   ...
   @nm_valor     = replace(CAST(dbo.fn_formata_valor(vl_total) as varchar(100)),'.',''),  
      @html_detalhe = @html_detalhe + '  
            <tr>                      
   <td style="font-size:12px; text-align:center;width: 20px">'+cast(cd_mes as varchar(20))+'</td>  
   ...
   <td '+case when cd_mes = @cd_mes then 'class="table-info" 'else '' end  
        +'style="font-size:12px; text-align:center;width: 20px">'+dbo.fn_formata_valor(vl_total)+'</td>  
  ...
  if exists(select top 1 cd_controle from #FinalFaturamentoMensal)  
  begin  
    set @labels  = @labels + ', '  
    set @valores = @valores + ', '  
  end  
  ...
  --Dados da Tabela---  
   set @tabela = '<div class="container mt-5">  
    <h2 class="mb-4">'+CAST(year(@dt_final) as varchar(4))+' - Meses'+'</h2>  
...
<section class="secao4" id="sobre">  
  <div class="secao4-div">  
      ...
      <div class="secao4-div-card">                    
          <h3>Meta</h3>  
          <p>R$ '+cast(isnull(dbo.fn_formata_valor(@vl_meta),0) as varchar(20))+'</p>  
      </div>  
   <div class="secao4-div-card">            
          <h3>Faturamento</h3>  
          <p>'+cast(isnull(dbo.fn_formata_valor(@vl_total),0) as varchar(20))+'</p>   
      </div>  
      <!-- Card 3 -->  
      <div class="secao4-div-card">            
          <h3>Crescimento</h3>  
          <p>'+cast(isnull(dbo.fn_formata_valor(@pc_crescimento),0) as varchar(20))+'(%) </p>  
      </div>  
...
------------------------------------------------------------------------------
--Gráfico
--exec pr_egis_relatorio_faturamento_mensal_assistencia_tecnica 337,4253,3,''
------------------------------------------------------------------------------
