IF EXISTS (SELECT name 
	   FROM   sysobjects 
	   WHERE  name = N'pr_egis_relatorio_pedido_compra_modelo_01' 
	   AND 	  type = 'P')
    DROP PROCEDURE pr_egis_relatorio_pedido_compra_modelo_01

GO

-------------------------------------------------------------------------------    
--sp_helptext pr_egis_relatorio_pedido_compra_modelo_01    
-------------------------------------------------------------------------------    
--pr_egis_relatorio_pedido_compra_modelo_01    
-------------------------------------------------------------------------------    
...
--    
------------------------------------------------------------------------------    
create procedure pr_egis_relatorio_pedido_compra_modelo_01   
@cd_relatorio int   = 0,      
@json varchar(8000) = ''     
...
select        
 ...
 isnull(i.vl_total_item_pedido_comp,0.00) - isnull(i.vl_frete_item_pedido_compra,0.00) - isnull(i.vl_ipi_item_pedido_compra,0.00) - isnull(i.vl_icms_item_pedido_compra,0.00) - isnull(i.vl_desconto_item_pedido,0.00)                                                                       
                                                                                                                   as VL_TOT,  
 isnull(i.vl_item_icms_st,0)                                                                                       as vl_item_icms_st,
...
into             
 #PedidoCompraItens            
from             
  pedido_compra_item i with(nolock)  --select * from   pedido_compra_item
...
--select * from #PedidoCompraItens return
select             
  it.cd_pedido_compra,            
...
from    
 ...
 order by 
	f.CD_ITEM 
 --   select * from #FinalFaturamentoMensal return
...
-- Dados do Fornecedor -----------------------------------------------------            
 ...
 IsNull(RTrim(LTrim(ef.sg_estado)),'')                     as sg_estado_for,          
  ...
  pc.dt_nec_pedido_compra                 as dt_nec_pedido_compra,            
   '(' +  
   ltrim(rtrim(ap.cd_aplicacao_produto))+') '+  
   --ltrim(rtrim(ap.sg_aplicacao_produto))+' - '+  
   ltrim(rtrim(ap.nm_aplicacao_produto))                                  as nm_aplicacao_produto,     
  de.nm_destinacao_produto                                as nm_destinacao_produto,    
     '(' +  
   ltrim(rtrim(pfin.cd_plano_financeiro))+') '+  
   ltrim(rtrim(pfin.cd_mascara_plano_financeiro))+' - '+  
   ltrim(rtrim(pfin.nm_conta_plano_financeiro))           as nm_plano_financeiro,  
    '(' +  
   ltrim(rtrim(cc.cd_centro_custo))+') '+  
   ltrim(rtrim(cc.sg_centro_custo))+' - '+  
   ltrim(rtrim(cc.nm_centro_custo))                       as nm_sg_centro_custo,             
...
--------------------------------------------------------------------------------------------------          
 ...
 isnull(ltrim(rtrim(efemp.cd_cep)),'')                   as nm_end_emp_fat_completo,     
    ...
    isnull(Rtrim(Ltrim(eo.cd_cep)),'')                      as 'nm_empreendimento_entrega', --OPORTUNA    
 ...
 ee.cd_cep_entrega,               
  ...
  isnull(ti.vl_TOTAL,0.00) +  isnull(pc.vl_icms_st,0) + ISNULL(pc.vl_total_ipi_pedido,0)     as VL_TOT_PROD,            
  ...
  - (vl_desconto_pedido_compra)                           as VL_TOT,            
...
((pc.vl_total_pedido_compra + pc.vl_frete_pedido_compra             
  ...
  isnull(pc.vl_desconto_pedido_compra,0.00)               as vl_desconto_pedido_compra,    
    (isnull(ti.vl_TOTAL,0.00) + isnull(pc.vl_total_ipi_pedido,0.00) + isnull(pc.vl_icms_st,0) + isnull(pc.vl_frete_pedido_compra,0.00))  
  - (isnull(vl_desconto_pedido_compra,0))                           as VL_TOT_IPI_ST,  
  -- Dados do Pedido -----------------------------------------------------------            
...
--   fo.cd_fornecedor                                      
  ...
  u.nm_usuario                                            as nm_usuario,    
   ...
   cm.cd_email_comprador                                  as cd_email_comprador,  
   plc.nm_plano_compra                                    as nm_plano_compra,  
   cp.qt_parcela_condicao_pgto                             as qt_parcela_condicao_pgto,
   plc.cd_mascara_plano_compra                            as cd_mascara_plano_compra,  
   IsNull(RTrim(LTrim(cf.nm_cidade)),'')+' - '+  
   IsNull(RTrim(LTrim(ef.sg_estado)),'')                     as nm_cidade_uf_fornecedor,  
    '('+replace(replace(IsNull(RTrim(LTrim(  
    case when isnull(co.cd_ddd_contato_fornecedor,'')<>'' then          
      co.cd_ddd_contato_fornecedor          
    else          
      fo.cd_ddd                
 ...
 IsNull(RTrim(LTrim(  
    case when isnull(co.cd_telefone_contato_forne,'')<>'' then          
      ...
      fo.cd_telefone end)),'')                                as cd_telefone_for,  
   fpp.nm_forma_pagamento                                  as nm_forma_pagamento,  
   fpp.cd_forma_pagamento as cd_forma_pagamento,
   pc.vl_total_pedido_compra                                      as vl_pedido_compra,  
  isnull(vw.nm_fantasia_usuario,'')                           as nm_fantasia_gestor,
      pc.dt_vencimento_adiantamento                           as dt_vencimento_adiantamento,  
	     case when isnull(pcc.cd_conta,0) > 0 then 
     '(' +
     ltrim(rtrim(pcc.cd_conta))+') '+
     ltrim(rtrim(pcc.cd_mascara_conta))+' - '+
     ltrim(rtrim(pcc.nm_conta))
   else
     ''
   end                                                    as nm_sg_conta_contabil
 ...
 from              
  ...
  left outer join fornecedor_contato co        with(nolock) on co.cd_fornecedor              = pc.cd_fornecedor           and co.cd_contato_fornecedor      = pc.cd_contato_fornecedor              left outer join cidade cf                    with(nolock) 
on cf.cd_cidade                  = fo.cd_cidade            
  ...
  left outer join cidade ct                    with(nolock) on ct.cd_cidade                  = tr.cd_cidade            
  ...
  left outer join vw_usuario_oficial vw        with(nolock) on vw.cd_usuario                 = pc.cd_usuario
  ...
  left outer join Cidade cee                   with(nolock) on cee.cd_cidade                 = ee.cd_cidade            
  ...
  left outer join Cidade ceo                                on ceo.cd_cidade                 = eo.cd_cidade            
  ...
  left outer join Cidade cef                   with(nolock) on cef.cd_cidade                 = efemp.cd_cidade and    
                                                               cef.cd_estado                 = eef.cd_estado           
  left outer join importador i                 with(nolock) on i.cd_importador               = 1    
  left outer join Cidade ceee                  with(nolock) on ceee.cd_cidade                = i.cd_cidade            
  ...
  left outer join Plano_Financeiro pfin        with(nolock) on pfin.cd_plano_financeiro      = pc.cd_plano_financeiro  
  left outer join forma_pagamento fpp          with(nolock) on fpp.cd_usuario                = pc.cd_usuario  
  left outer join Plano_Conta pcc              with(nolock) on pcc.cd_conta                  = pfin.cd_conta
  
 where    
  pc.cd_pedido_compra = @cd_documento

  --select * from #PedidoCompraDadosRel return
 -- select * from #PedidoCompraDadosRel return
 --   if @cd_empresa in (317,318)
 -- begin
 --   select        
	-- identity(int,1,1)      as cd_controle, 
 --    cd_parcela_pedido      as NParcela,        
 --    dt_vcto_parcela_pedido as VencParcela,           
 --    vl_parcela_pedido      as VParcela,        
 --    nm_obs_parcela_pedido  as EspacoModelo
	-- into
	-- #parcela
 --   from        
 --    pedido_compra_parcela
 --   where        
 --    cd_pedido_compra = @cd_documento
 -- end
 -- else
 -- begin
 --   select        
...
declare @NParcela                             int = 0
declare @nm_razao_social_fornecedor           varchar(80) = ''
declare @nm_sg_conta_contabil				  varchar(80) = ''
declare @nm_fantasia_fornecedor               varchar(8000) = ''     
declare @nm_endereco_fornecedor               varchar(8000) = ''    
declare @cd_ddd_contato_fornecedor            varchar(8000) = ''    
declare @cd_telefone_contato_forne            varchar(8000) = ''    
declare @nm_email_fornecedor                  varchar(8000) = ''    
declare @cd_cnpj_fornecedor                   varchar(8000) = ''    
declare @cd_inscestadual_fornecedor           varchar(8000) = ''    
declare @nm_fantasia_contato_forne            varchar(8000) = ''    
declare @dt_pedido_compra                     varchar(8000) = ''    
declare @nm_condicao_pagamento_pc             varchar(8000) = ''    
declare @dt_nec_pedido_compra                 varchar(8000) = ''    
declare @nm_transportadora                    varchar(8000) = ''    
declare @nm_endereco_transp                   varchar(8000) = ''    
declare @cd_ddd_transp                        varchar(8000) = ''    
declare @cd_telefone_transp                   varchar(8000) = ''    
declare @nm_email_transportadora              varchar(8000) = ''    
declare @cd_cnpj_transportadora               varchar(8000) = ''    
declare @cd_inscestadual_transportadora       varchar(8000) = ''    
...
declare @nm_tipo_entrega_produto              varchar(8000) = ''    
declare @dt_conf_pedido_compra                varchar(8000) = ''    
declare @nm_conf_pedido_compra                varchar(8000) = ''    
declare @dt_cancel_ped_compra                 varchar(8000) = ''    
declare @ds_ativacao_pedido_compra            varchar(8000) = ''  
declare @nm_comprador                         varchar(60) = ''   
declare @nm_pedfornec_pedido_compra           varchar(60) = ''  
declare @nm_plano_compra                      varchar(60) = ''  
declare @cd_mascara_plano_compra              varchar(60) = ''  
declare @nm_centro_custo                      varchar(60) = ''  
declare @nm_aplicacao_produto                 varchar(60) = ''  
declare @cd_email_comprador                   varchar(30) = ''  
declare @nm_cidade_uf_fornecedor              varchar(100) = ''  
declare @nm_bairro_for                        varchar(50) = ''  
declare @cd_cep_for                           varchar(20) = ''  
declare @nm_plano_financeiro                  varchar(100) =''  
declare @vl_total_item                        float = 0  
declare @ds_observacao_pedido                 varchar(500) = ''  
declare @nm_forma_pagamento                   varchar(50) = ''  
declare @vl_pedido_compra                     float = 0  
declare @qt_parcela                           float = 0  
declare @dt_vencimento_adiantamento           datetime   
DECLARE @nm_fantasia_gestor                   VARCHAR(200)
declare @VL_TCOM_DESC                         float = 0 
declare @cd_condicao_pagamento                int = 0 
declare @cd_fornecedor                        int = 0
  
  select     
	@nm_sg_conta_contabil             = nm_sg_conta_contabil,
    ...
    @nm_endereco_fornecedor           = nm_endereco_for,    
    @cd_ddd_contato_fornecedor        = cd_ddd_fornecedor,    
    @cd_telefone_contato_forne        = cd_telefone_for,    
    @nm_email_fornecedor              = nm_email_fornecedor,    
    @cd_cnpj_fornecedor               = cd_cnpj_forn,    
	...
	@qt_parcela                       = qt_parcela_condicao_pgto,
    ...
    @VL_TOTAL_GERAL                   = VL_TOT_PROD, --VL_TOT_IPI_ST,    
    @VL_TOT_IPI                       = VL_TOT_IPI,    
    @VL_TOT_ICMS                      = vl_icms_st,    
    ...
    @nm_comprador                     = nm_fantasia_comprador,  
    ...
    @nm_centro_custo                  = nm_sg_centro_custo,  
    @cd_email_comprador               = cd_email_comprador,  
    @nm_cidade_uf_fornecedor          = nm_cidade_uf_fornecedor,  
	@cd_fornecedor                    = cd_fornecedor,
    @nm_bairro_for                    = nm_bairro_for,  
    @cd_cep_for                       = cd_cep_for,  
    @nm_plano_financeiro              = nm_plano_financeiro,  
    @vl_total_item                    = vl_pedido_compra,--VL_TOT_PROD,  
    @ds_observacao_pedido             = ds_pedido_compra,  
    @nm_forma_pagamento               = nm_forma_pagamento,  
    @vl_pedido_compra                 = vl_pedido_compra,  
    @dt_vencimento_adiantamento       = dt_vencimento_adiantamento,  
    @VL_TCOM_DESC                     = VL_TCOM_DESC              
  from #PedidoCompraDadosRel    

   select 
	   @NParcela = COUNT(NParcela)
	from #parcela
  
--Gráfico----------------------------------------------------------------------------------------------------------    
set @html_geral = '  
   <td style="width: 20%; text-align: center;">  
                <p><strong>PEDIDO: '+cast(isnull(@cd_documento,'')as varchar(20))+'</strong></p>  
                <p><strong>DATA:'+isnull(dbo.fn_data_string(@dt_pedido_compra),'')+'</strong> </p>  
                <p><strong>REIMPR: '+isnull(dbo.fn_data_string(@dt_hoje),'')+' '+isnull(@hr_imp,'')+'</strong> </p>  
            </td>  
  </tr>  
    </table>  
    <h3 class="section-title" style=" text-align: center;">'+@titulo+'</h3>  
	<table style="width: 100%;">
    <tr>
      <td style="display: flex; flex-direction: column; gap: 20px;">
        <div style="display: flex; justify-content: space-between; align-items: flex-start; width: 100%;">
          <p><strong>Comprador:</strong> '+isnull(@nm_comprador,'')+'</p>
          <p><strong>Email: </strong>'+cast(isnull(@cd_email_comprador,'') as varchar(20))+'</p>
          <p><strong>Aplicação: </strong>'+isnull(@nm_aplicacao_produto,'')+'</p>  
          </div>
      </td>
    </tr>
  </table>
     <div style="border: 0.5px solid black; padding: 10px; margin-top: 10px;">
      <div style="display: flex; justify-content: space-between; align-items: flex-start; width: 100%;">
        <div style="text-align: left; width: 45%;">    
                        <p><strong>Fornecedor:</strong> ('+cast(isnull(@cd_fornecedor,'') as varchar(20))+') '+isnull(@nm_razao_social_fornecedor,'')+'</p>    
                        <p><strong>Contato:</strong> '+isnull(@nm_fantasia_contato_forne,'')+'</p>    
                        <p><strong>CNPJ:</strong> '+isnull(@cd_cnpj_fornecedor,'')+'</p>    
                        <p><strong>Endereço:</strong> '+isnull(@nm_endereco_fornecedor,'')+'<p>    
                        <p><strong>Cidade:</strong> '+isnull(@nm_cidade_uf_fornecedor,'')+'</p>    
                    </div>        
                    <div style="text-align: left;">    
						<p><strong>Fantasia:</strong> '+isnull(@nm_fantasia_fornecedor,'')+'</p>    
                        <p><strong>Fone:</strong> '+cast(isnull(@cd_telefone_contato_forne,'') as varchar(100))+' </p>    
                        <p><strong>Email:</strong> '+isnull(@nm_email_fornecedor,'')+'</p>    
                        <p><strong>Bairro:</strong> '+isnull(@nm_bairro_for,'')+'</p>    
                        <p><strong>Cep:</strong> '+isnull(@cd_cep_for,'')+'</p>   
    ...
    <div style="border: 0.5px solid black; padding: 10px; margin-top: 10px;">
        <div style="display: flex; justify-content: space-between; align-items: flex-start; width: 100%;">
          <div style="text-align: left; width: 45%;"> 
						<p><strong>C.R.:</strong> '+isnull(@nm_centro_custo,'')+' </p> 
                        <p><strong>C.C.:</strong> '+isnull(@nm_sg_conta_contabil,'')+' </p>    
                    </div>        
                    <div style="text-align: left;">    
                        <p><strong>Natureza:</strong> '+ISNULL(@nm_plano_financeiro,'')+'</p>    
						<p><strong>P.F.:</strong> '+ISNULL(@nm_plano_financeiro,'')+' </p> 
                    </div>       
                </div>    
            </div>   
...
declare @id int = 0   
while exists ( select cd_controle from #FinalFaturamentoMensal )    
begin    
  ...
  select top 1    
    ...
    @html_geral = @html_geral + '    
           <tr>        
               <td>'+cast(ISNULL(CD_ITEM, 0) as varchar(4))+'</td>  
			   <td>'+cast(ISNULL(FANTASIA, '') as varchar(max))+'</td>  
			   <td style="text-align: left;">'+cast(ISNULL(NM_PROD, '') as varchar(300))+' </td>   
			   <td>'+cast(ISNULL(SG_UNID, '') as varchar(10))+'</td>   
			   <td>'+cast(ISNULL(QT_ITEM, 0) as varchar(30))+'</td>  
			   <td>'+ISNULL(dbo.fn_formata_valor(VL_ITEM), 0)+'</td>   
			   <td>'+ISNULL(dbo.fn_formata_valor(PC_IPI), 0)+'</td>  
			   <td>'+ISNULL(dbo.fn_formata_valor(PC_ICMS), 0)+'</td> 
			   <td>'+cast(ISNULL(dbo.fn_formata_valor(vl_item_icms_st), 0) as varchar(20))+'</td> 
			   <td>'+cast(ISNULL(dbo.fn_formata_valor(VL_DESC), 0)as varchar(20))+'</td> </td>  
			   <td>'+ISNULL(dbo.fn_formata_valor(VL_TOT), 0)+'</td>    
           </tr>'    
  ...
  delete from #FinalFaturamentoMensal  where  cd_controle = @id    
...
set @html_rodape = @html_rodape + '  
   ...
   <div style="border: 0.5px solid black; padding: 10px; margin-top: 10px;">
          <div style="text-align: right;"> 
                <p><strong>Total Item:</strong> '+cast(ISNULL(dbo.fn_formata_valor(@vl_total_item), 0)as varchar(20))+' </p>    
                <p><strong>Frete:</strong> '+cast(ISNULL(dbo.fn_formata_valor(@vl_frete_pedido_compra), 0)as varchar(20))+' </p>    
                <p><strong>IPI:</strong> '+cast(ISNULL(dbo.fn_formata_valor(@VL_TOT_IPI), 0)as varchar(20))+' </p>    
                <p><strong>ICMS-ST:</strong> '+cast(ISNULL(dbo.fn_formata_valor(@VL_TOT_ICMS), 0)as varchar(20))+' </p>   
                <p><strong>Desconto:</strong> '+cast(ISNULL(dbo.fn_formata_valor(@vl_desconto_pedido_compra), 0)as varchar(20))+' </p>   
                <p><strong>Total:</strong> '+cast(ISNULL(dbo.fn_formata_valor(@VL_TOTAL_GERAL), 0)as varchar(20))+' </p>   
                   </div>       
                </div>    
            </div>
			'+ case when ISNULL(@ds_observacao_pedido,'') = '' then ''
			else '
            <div style="border: 0.5px solid black; padding: 10px; margin-top: 10px;">
                <p style="font-size: 16px;"><b>Observações:</b></p>  
                <p>' + ISNULL(@ds_observacao_pedido,'') + '</p>  
            </div>'
			end+'

			'+ case when ISNULL(@cd_condicao_pagamento,'') = '' then ''
			else '
			<div style="border: 0.5px solid black; padding: 10px; margin-top: 10px;">
                <p><b>Condição de Pagamento:</b></p>  
                <p>(' + CAST(isnull(@cd_condicao_pagamento,0) as varchar(20)) + ') ' + ISNULL(@nm_condicao_pagamento_pc,'') + '</p>  
            </div> ' 
			end+'

     '+ case when ISNULL(@NParcela,0) > 0 then'
...
while exists ( select cd_controle from #parcela )    
begin    
  ...
  select top 1    
    ...
    @html_rodape = @html_rodape +'  
         <tr> 
        <td>'+CAST(isnull(NParcela,0) as varchar(20))+'</td>  
        <td>'+isnull(dbo.fn_data_string(VencParcela),'')+'</td>  
        <td>'+cast(ISNULL(dbo.fn_formata_valor(VParcela), 0)as varchar(20))+'</td>  
        <td>'+ISNULL(EspacoModelo,'')+'</td>  
		</tr> 
    '  
	  from
    #parcela 
...
--else 
-- begin
--	while exists ( select cd_controle from #parcela_emp )    
--begin    
...
exec pr_egis_relatorio_pedido_compra_modelo_01 339,''
