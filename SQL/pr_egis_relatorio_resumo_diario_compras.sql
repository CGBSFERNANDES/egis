IF EXISTS (SELECT name 
	   FROM   sysobjects 
	   WHERE  name = N'pr_egis_relatorio_resumo_diario_compras' 
	   AND 	  type = 'P')
    DROP PROCEDURE pr_egis_relatorio_resumo_diario_compras

GO
  
-------------------------------------------------------------------------------  
--sp_helptext pr_egis_relatorio_resumo_diario_compras  
-------------------------------------------------------------------------------  
--pr_egis_relatorio_resumo_diario_compras  
-------------------------------------------------------------------------------  
...
--  
------------------------------------------------------------------------------  
create procedure pr_egis_relatorio_resumo_diario_compras  
@cd_relatorio int   = 0,  
@cd_parametro int   = 0,  
@json nvarchar(max) = ''   
...
------------------------------------------------------------------------------------------------------------
  ...
  set @vl_geral = (select sum( isnull(pc.vl_total_pedido_compra,0) ) from  Pedido_Compra pc
     where
  ...
  select @vl_total_compra = @vl_total_compra + isnull(pc.vl_total_pedido_compra,0)
  from
     Pedido_Compra pc
  where
     pc.dt_pedido_compra  between @dt_inicial and @dt_final and
  ...
  select 
     ...
     isnull(sum( isnull(pc.vl_total_pedido_compra,0) ),0)                        as TotalCompra,
     isnull((sum( isnull(pc.vl_total_pedido_compra,0) ) * 100) / @vl_geral,0)    as Perc
...
------------------------------------------------------------------------------------------------------------
  ...
  select 
    @vl_quantidade_geral = sum(QtdPedido),
    @vl_total            = sum(TotalCompra), 
	@qt_total_produto    = COUNT(QtdPedido)
  from #relDiarioCompras
 ------------------------------------------------------------------------------------------------------------  
 if isnull(@cd_parametro,0) = 1    
 begin    
    select * from #relDiarioCompras    
  return    
 end    
...
WHILE EXISTS (SELECT TOP 1 cd_controle FROM #relDiarioCompras)  
BEGIN  
    SELECT TOP 1  
        @id                          = cd_controle,  
      ...
      @html_detalhe = @html_detalhe + 
        '<tr>  
		   <td class="tamanho">'+cast(ISNULL(dbo.fn_data_string(DataPedido),0)as nvarchar(20))+'</td> 
		   <td class="tamanho">'+cast(ISNULL(dbo.fn_formata_valor(QtdPedido),0)as nvarchar(20))+'</td>  
		   <td class="tamanho">'+cast(ISNULL(dbo.fn_formata_valor(TotalCompra),0)as nvarchar(20))+'</td>  
		   <td class="tamanho">'+cast(ISNULL(dbo.fn_formata_valor(Perc),0)as nvarchar(20))+'</td> 
        </tr>'  
  from #relDiarioCompras  
    DELETE FROM #relDiarioCompras WHERE cd_controle = @id  
END  
--------------------------------------------------------------------------------------------------------------------  
...
-------------------------------------------------------------------------------------------------------------------------------------------------------  
 ...
 --exec pr_egis_relatorio_resumo_diario_compras 267,0,''
