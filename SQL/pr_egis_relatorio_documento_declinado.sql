IF EXISTS (SELECT name 
	   FROM   sysobjects 
	   WHERE  name = N'pr_egis_relatorio_documento_declinado' 
	   AND 	  type = 'P')
    DROP PROCEDURE pr_egis_relatorio_documento_declinado
...
--
------------------------------------------------------------------------------
create procedure pr_egis_relatorio_documento_declinado
@cd_relatorio int   = 0,
@cd_parametro int   = 0, 
@json nvarchar(max) = '' 
...
set @html_geral = '
 <div class="section-title">  
        ...
        <p style="display: inline; text-align: center; padding: 20%;">Documentos Declinados</p>  
    </div> 
	<table>
	...
	select top 1
	    ...
	    @html_geral  = @html_geral + '
		                          <tr class="tamanho">
									...
									<td>'+isnull(dbo.fn_data_string(dt_rejeicao_movimento),'')+' '+case when isnull(hr_rejeicao_movimento,'') = '' then '' else isnull(hr_rejeicao_movimento,'') end+'</td>
...
set @html_rodape =
	...
	<div class="section-title">  
	<p>Total Declinados: <td>'+cast(isnull(@qt_documento,0)as varchar)+'</td></p>
	</div>
...
------------------------------------------------------------------------------
--exec pr_egis_relatorio_documento_declinado 384,0,''
------------------------------------------------------------------------------
