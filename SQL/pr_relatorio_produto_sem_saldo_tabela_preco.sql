IF EXISTS (SELECT name 
	   FROM   sysobjects 
	   WHERE  name = N'pr_relatorio_produto_sem_saldo_tabela_preco' 
	   AND 	  type = 'P')
    DROP PROCEDURE pr_relatorio_produto_sem_saldo_tabela_preco

GO

-------------------------------------------------------------------------------
--sp_helptext pr_relatorio_produto_sem_saldo_tabela_preco
-------------------------------------------------------------------------------
--pr_relatorio_produto_sem_saldo_tabela_preco
-------------------------------------------------------------------------------
...
--
------------------------------------------------------------------------------
create procedure pr_relatorio_produto_sem_saldo_tabela_preco
@cd_relatorio int   = 0,
@json nvarchar(max) = '' 
...
order by
  tp.nm_tabela_preco,
  p.nm_produto
 -- select * from #produtos_sem_saldo
...
declare @id int = 0
while exists ( select top 1 cd_controle from #produtos_sem_saldo where cd_controle < 600)
begin
	select top 1
		@id                         = cd_controle,
  ...
  set @html_geral = @html_geral +'<tr>
	 ...
	 delete from #produtos_sem_saldo where cd_controle = @id
 end
...
set @html_rodape =
	...
	'+
	case when isnull(@id,0) >= 600 then 
	'<div>
...
------------------------------------------------------------------------------
--exec pr_relatorio_produto_sem_saldo_tabela_preco 222,'' 
------------------------------------------------------------------------------
