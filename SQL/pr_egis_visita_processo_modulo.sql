...
IF EXISTS (SELECT name 
	   FROM   sysobjects 
	   WHERE  name = N'pr_egis_visita_processo_modulo' 
	   AND 	  type = 'P')
    DROP PROCEDURE  pr_egis_visita_processo_modulo
...
IF OBJECT_ID('pr_egis_visita_processo_modulo','P') IS NOT NULL
    DROP PROCEDURE pr_egis_visita_processo_modulo;
GO


-------------------------------------------------------------------------------
--sp_helptext  pr_egis_visita_processo_modulo
-------------------------------------------------------------------------------
-- pr_egis_visita_processo_modulo
-------------------------------------------------------------------------------
...
--
------------------------------------------------------------------------------
create procedure  pr_egis_visita_processo_modulo
------------------------
@json nvarchar(max) = ''
------------------------------------------------------------------------------
--with encryption
...
select @cd_promotor            = valor from #json where campo = 'cd_promotor'   
...
set @cd_promotor           = isnull(@cd_promotor,0)
...
--Consulta de Controle de Visitas---

if @cd_parametro = 1
begin
  exec pr_consulta_visita_cliente_vendedor @cd_vendedor, @dt_inicial, @dt_final, 0, @cd_promotor
  return
end
...
------------------------------------------------------------------------------
--exec  pr_egis_visita_processo_modulo
------------------------------------------------------------------------------
...
/*
exec  pr_egis_visita_processo_modulo '[{"cd_parametro": 0}]'
exec  pr_egis_visita_processo_modulo '[{"cd_parametro": 1, "cd_modelo": 1}]' 
