IF EXISTS (SELECT name 
	   FROM   sysobjects 
	   WHERE  name = N'pr_egis_relatorio_analise_contas_receber' 
	   AND 	  type = 'P')
    DROP PROCEDURE pr_egis_relatorio_analise_contas_receber

GO

-------------------------------------------------------------------------------
--sp_helptext pr_egis_relatorio_analise_contas_receber
-------------------------------------------------------------------------------
--pr_egis_relatorio_analise_contas_receber
-------------------------------------------------------------------------------
...
--
------------------------------------------------------------------------------
create procedure pr_egis_relatorio_analise_contas_receber
@cd_relatorio int   = 0,
@cd_usuario   int   = 0,
@cd_parametro int   = 0,
@json nvarchar(max) = '' 
...
select
  ...
  SUM( case when d.dt_vencimento_documento<@dt_hoje 
       then isnull(d.vl_saldo_documento,0)
	   else 0.00
	   end
	   )                                             as vl_atraso_documento
...
from
  ...
  left outer join Cliente c        on c.cd_cliente        = d.cd_cliente
  left outer join Cliente_Grupo cg on cg.cd_cliente_grupo = c.cd_cliente_grupo
  left outer join Mes m            on m.cd_mes            =  MONTH(d.dt_vencimento_documento) 
where
  d.dt_cancelamento_documento is null
  and
  d.dt_devolucao_documento is null
  ...
  ISNULL(d.vl_saldo_documento,0)>0
  and
  YEAR(d.dt_vencimento_documento) = @cd_ano
...
select
  ...
  @vl_atraso            = SUM(vl_atraso_documento),
  @dt_inicio_vencimento = MIN(dt_inicio_vencimento),
  @dt_fim_vencimento    = MAX(dt_fim_vencimento)
from
  #Ano

select
 *,
 pc_ano = case when @vl_total>0 then round(vl_saldo_documento/@vl_total * 100,2) else 0.00 end
into
  #FinalAno
from
 #Ano
...
begin
  ...
  while exists ( select top 1 cd_controle from #Mensal )
  begin

    select top 1 
	  @cd_controle     = cd_controle,
	  @nm_label_mensal = isnull(nm_mes,''),
	  @nm_valor_mensal = replace(CAST(dbo.fn_formata_valor(isnull(vl_saldo_documento,0)) as varchar(100)),'.','')
	...
	order by
	 ...
	 if exists(select top 1 cd_controle from #Mensal)
	 begin
  ...
  --select @labels_mensal, @valores_mensal

  --Ano

  while exists ( select top 1 cd_controle from #FinalAno )
  begin

    select top 1 
	  @cd_controle  = cd_controle,
	  @nm_label     = CAST(cd_ano as varchar(4)),
	  @nm_valor     = replace(CAST(dbo.fn_formata_valor(vl_saldo_documento) as varchar(100)),'.',''),
      @html_detalhe = @html_detalhe + '
            <tr> 					           			
			<td style="font-size:12px; text-align:center;width: 20px">'+cast(cd_ano as varchar(20))+'</td>			
			<td style="font-size:12px; text-align:center;width: 20px">'+dbo.fn_formata_valor(vl_saldo_documento)+'</td>
			<td style="font-size:12px; text-align:center;width: 20px">'+cast(pc_ano as varchar(20))+'</td>
            <td style="font-size:12px; text-align:center;width: 20px">'+cast(qt_documento as varchar(20))+'</td>
			<td style="font-size:12px; text-align:center;width: 20px">'+cast(qt_cliente_grupo as varchar(20))+'</td>
    ...
    from
	 ...
	 delete from #FinalAno
	 where
	   cd_controle = @cd_controle

	 if exists(select top 1 cd_controle from #FinalAno)
	 begin
	   set @labels  = @labels + ', '
	   set @valores = @valores + ', '
	 end
  ...
  --Dados da Tabela---
   ...
   set @tabela = '<div class="container mt-5">
    <h2 class="mb-4">'+@titulo_tabela+'</h2>
...
@media (max-width: 768px) {
  ...
  <body>
    ...
    const monthlyReceivablesCtx = document.getElementById("monthlyReceivablesChart").getContext("2d");
    const monthlyReceivablesChart = new Chart(monthlyReceivablesCtx, {
        type: "bar",
        data: {
            labels: '+@labels_mensal+',
            datasets: [{
                label: "Contas a Receber (R$) - 2024",
                data: '+@valores_mensal+',
...
------------------------------------------------------------------------------
--Gráfico
--exec pr_egis_relatorio_analise_contas_receber 187,4253,3,''
------------------------------------------------------------------------------
