IF EXISTS (SELECT name 
	   FROM   sysobjects 
	   WHERE  name = N'pr_relatorio_ordem_servico_periodo' 
	   AND 	  type = 'P')
    DROP PROCEDURE pr_relatorio_ordem_servico_periodo
...
--
------------------------------------------------------------------------------
create procedure pr_relatorio_ordem_servico_periodo
@cd_relatorio int   = 0,
@json nvarchar(max) = '' 
...
select
  ...
  isnull(oso.vl_pagamento,0)             as vl_total_ordem_servico,
  dbo.fn_data_string(DATEADD(DAY, 180, os.dt_ordem_servico)) as dt_pagamento_mais_180, 
  ...
  tp.nm_tipo_prioridade,
   a.nm_atendente                             as nm_atendente,
  --Usuário
...
from
  ...
  LEFT outer join Atendente a                  on a.cd_atendente              = os.cd_atendente
  left outer join ordem_servico_orcamento oso  on oso.cd_ordem_servico        = os.cd_ordem_servico
  left outer join status_ordem_servico sos     on os.cd_status_ordem_servico  = sos.cd_status_ordem_servico
  ...
  left outer join forma_pagamento fp           on fp.cd_forma_pagamento    = case when isnull(oso.cd_forma_pagamento,0) = 0 then os.cd_forma_pagamento
	...
	where
	 oso.dt_pagamento between @dt_inicial and @dt_final
	 and
	 --isnull(oso.vl_pagamento,0) > 0   
	 --and
	 ...
	 order by 
	 os.dt_ordem_servico
	--select vl_total_ordem_servico,* from #ordemServicoPeriodo
--------------------------------------------------------------------------------------------------------------
...
select 
	@vl_total_os             = sum(vl_total_ordem_servico),
...
set @html_geral = '<div class="section-title">  
				   ...
				   <table>
					   <tr>  
							...
							<td class="tamanho" style="font-size: 110%;"><strong>Emissão</strong></td>
							...
							<td class="tamanho" style="font-size: 110%;"><strong>Total R$</strong></td>
							<td class="tamanho" style="font-size: 110%;"><strong>Forma Pagamento</strong></td>
							<td class="tamanho" style="font-size: 110%;"><strong>Atendente</strong></td>
					   </tr>'
...
begin
	select top 1
		...
		@html_geral = @html_geral +'<tr>
						...
						<td class="tamanho">'+isnull(dt_ordem_servico,'')+'</td>
						...
						<td class="tamanho">'+isnull(dt_pagamento_mais_180,'')+'</td>						
						<td class="tamanho">'+cast(isnull(dbo.fn_formata_valor(vl_total_ordem_servico),0) as nvarchar(10))+'</td>
						<td class="tamanho">'+isnull(nm_forma_pagamento,'')+'</td>
						<td class="tamanho">'+isnull(nm_atendente,'')+'</td>
					  </tr>'
     from #ordemServicoPeriodo
...
------------------------------------------------------------------------------
--exec pr_relatorio_ordem_servico_periodo 246,'' 
------------------------------------------------------------------------------
