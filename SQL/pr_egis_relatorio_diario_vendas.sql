...
-------------------------------------------------------------------------------  
--sp_helptext pr_egis_relatorio_diario_vendas  
-------------------------------------------------------------------------------  
--pr_egis_relatorio_diario_vendas  
-------------------------------------------------------------------------------  
...
--  
------------------------------------------------------------------------------  
create or alter procedure pr_egis_relatorio_diario_vendas  
@cd_relatorio int   = 0,  
@cd_parametro int   = 0,   
@json nvarchar(max) = ''   
...
as  
print 'iniciou a pr_egis_relatorio_diario_vendas'  
...
--  dbo.fn_hora_string(p.hr_inicial_pedido) as hr_inicial_pedido,    
  ...
  convert(varchar,p.dt_pedido_venda,103)            as dt_pedido_venda,    
...
from    
 pedido_venda p with(nolock)
  ...
  left outer join vendedor v               with(nolock)  on v.cd_vendedor                = p.cd_vendedor    
  ...
  left outer join condicao_pagamento cp    with(nolock)  on cp.cd_condicao_pagamento     = p.cd_condicao_pagamento  
  left outer join forma_pagamento fp       with(nolock)  on fp.cd_forma_pagamento        = p.cd_forma_pagamento  
...
set @html_geral = '     
     <div class="section-title">    
        ...
        <p style="display: inline; text-align: center; padding: 15%;">Diário de Vendas</p>    
    </div>    
 <div>  
    <table>  
      <tr class="tamanho">  
...
WHILE EXISTS (SELECT TOP 1 cd_controle FROM #FinalDiario)  
BEGIN  
    SELECT TOP 1  
        @id                          = cd_controle,  
    ...
    SET @html_geral = @html_geral +  
        '<tr class="tamanho">  
            ...
            <td>' + CAST(ISNULL(@cd_pedido_venda, '') AS NVARCHAR(10)) + '</td>  
            <td>' + ISNULL(@nm_tipo_pedido, '') + '</td>  
            <td>' + ISNULL(@nm_fantasia_cliente, '') + ' (' + CAST(ISNULL(@cd_cliente_tb, '') AS NVARCHAR(10)) + ')</td>  
            <td>' + cast(ISNULL(dbo.fn_formata_valor(@vl_total_pedido), '') as nvarchar(20)) +'</td>  
    ...
    DELETE FROM #FinalDiario WHERE cd_controle = @id  
END  
--------------------------------------------------------------------------------------------------------------------  
...
set @html_rodape =  
 ...
 <div class="section-title" style="margin-top:10px;">  
      <p style="margin-bottom:10px ;">Total Consulta: '+cast(isnull(@qt_total,0) as nvarchar(10))+'</p>  
...
-------------------------------------------------------------------------------------------------------------------------------------------------------  
select isnull(@html,'') as RelatorioHTML, 'Diario_Vendas' as pdfName 
-------------------------------------------------------------------------------------------------------------------------------------------------------  

go


--exec pr_egis_relatorio_padrao '[{"cd_documento": 0, "cd_empresa": 317, "cd_item_processo": 0, "cd_menu": 7866, "cd_modulo": 239, "cd_parametro": 281, "cd_processo": 0, "cd_relatorio": 281, "cd_usuario": 4254, "dt_final": "03-07-2025", "dt_inicial": "03-07-2025"}]'