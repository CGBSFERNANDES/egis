...
IF EXISTS (SELECT name 
	   FROM   sysobjects 
	   WHERE  name = N'pr_egis_manifesto_processo_modulo' 
	   AND 	  type = 'P')
    DROP PROCEDURE  pr_egis_manifesto_processo_modulo
...
IF OBJECT_ID('pr_egis_manifesto_processo_modulo','P') IS NOT NULL
    DROP PROCEDURE pr_egis_manifesto_processo_modulo;
GO

-------------------------------------------------------------------------------
--sp_helptext  pr_egis_manifesto_processo_modulo
-------------------------------------------------------------------------------
-- pr_egis_manifesto_processo_modulo
-------------------------------------------------------------------------------
...
--
------------------------------------------------------------------------------
create procedure  pr_egis_manifesto_processo_modulo
------------------------
...
begin
        ...
        SELECT 
               ...
               i.manifesto_quando,
               i.recebido_em,
               TRY_CAST(JSON_VALUE(i.resumo_json, '$.CNPJ') AS VARCHAR(14)) AS cnpj_emit,
               JSON_VALUE(i.resumo_json, '$.xNome') AS emitente,
               JSON_VALUE(i.resumo_json, '$.dhEmi') AS dhEmi,
               JSON_VALUE(i.resumo_json, '$.vNF')   AS vNF
        FROM dbo.DFe_Inbox AS i
        WHERE 
          i.chave_nfe IS NOT NULL
          AND 
          i.recebido_em BETWEEN @dt_inicial AND @dt_final
        ORDER BY i.recebido_em DESC, i.nsu DESC
...
CREATE TABLE DFe_Sync_Log(
  ...
  next_allowed_utc datetime2(0) null,
...
------------------------------------------------------------------------------
--exec  pr_egis_manifesto_processo_modulo
------------------------------------------------------------------------------
...
go
/*
exec  pr_egis_manifesto_processo_modulo '[{"cd_parametro": 0, "cd_modelo": 1}]'                                   
exec  pr_egis_manifesto_processo_modulo '[{"cd_parametro": 1}, {"nm_fantasia_produto": 2} ]'   
