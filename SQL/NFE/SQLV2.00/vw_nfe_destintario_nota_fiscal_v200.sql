IF EXISTS (SELECT name 
	   FROM   sysobjects 
	   WHERE  name = N'vw_nfe_destintario_nota_fiscal' 
	   AND 	  type = 'V')
    DROP VIEW vw_nfe_destintario_nota_fiscal
...
-- 02.04.2014 - Versão 3.0 - Id Estrangeiro - Carlos Fernandes
-- 22.04.2019 - Ajuste no indIEDest para Pessoa Fisica -Pedro Jardim
-- 29.04.2019 - Ajuste no E16a_indIEDest para Isento Nordeste, passar 9 - Fagner Cardoso 
-- 27.01.2020 - Novo filtro na E16a_indIEDest para verificar se o cliente é isento - Pedro Jardim
-- 28.04.2022 - Acerto campo e-mail --- Fagner Cardoso
-- 23.10.2023 - IdEstrangeiro Operação PResencial -- Fagner Cardoso
-- 17.11.2023 - Inclusão da sigla 'PR' no campo E16a_indIEDest - Luis Santana/Pedro Jardim
-- 10.08.2024 - Ajuste do CPF - Carlos Fernandes
-------------------------------------------------------------------------------------------
...
select      
  ...
  case when dbo.fn_empresa() in (317,318) then '' else
  ' '+ cast(ns.cd_cliente as varchar(20)) + ' ' +  
  case when isnull(ns.nm_fantasia_nota_saida,'') <> '' then   
    ' - '+ cast(ltrim(rtrim(ns.nm_fantasia_nota_saida)) as varchar(20)) + ' '  
  else  
    ''  
  end  end
                                                                  as 'xNome',      
  ltrim(rtrim(cast(ltrim(rtrim(case when isnull(ns.nm_fantasia_nota_saida,'') <> '' then isnull(ns.nm_fantasia_nota_saida,'') else c.nm_fantasia_cliente end))as varchar(60))))
  +' ('+ cast(ns.cd_cliente as varchar(20)) + ') '
  ...
  case when 
    vw.cd_tipo_mercado=2 or 
    (vw.cd_tipo_pessoa = 2 and vw.sg_estado <> 'PA') or       
    (vw.sg_estado in ('AM','BA','CE','GO','MG','MS','MT','PE','RN','SE','PR','SP') and isnull(dp.ic_consumidor_final,'N')='S' and isnull(vw.IsentoInscEstadual,'N')='S' ) then 
    '9'
  else
    case when ns.cd_inscest_nota_saida is null or ltrim(rtrim(ns.cd_inscest_nota_saida)) = '' then  
      case when isnull(vw.IsentoInscEstadual,'N') = 'S' then
      ...
      case when isnull(vw.IsentoInscEstadual,'N') = 'S' and isnull(cp.ic_obra,'N') = 'N' then
        '2'
      else       
        case when isnull(cp.ic_obra,'N') = 'S' then '9' else '1' end
      end       
    end
  end                                                                as 'E16a_indIEDest',
...
--  c.cd_suframa_cliente,
 ...
 cast(
  case when ns.cd_inscest_nota_saida is null or ltrim(rtrim(isnull(ns.cd_inscest_nota_saida,''))) = '' then       
    ''      
  else       
    case when isnull(c.cd_suframa_cliente,'')<>'' then
       replicate('0', 9 - len(rtrim(ltrim(cast(replace(replace(replace(replace(isnull(c.cd_suframa_cliente,''),'.',''),'-',''),'/',''),' ','') as varchar(9)))))) +      
      rtrim(ltrim(cast(replace(replace(replace(replace(isnull(c.cd_suframa_cliente,''),'.',''),'-',''),'/',''),' ','') as varchar(9))))       
    else
      ''
    end  
...
--    cast('' as varchar(14))      
  ...
  case when vw.cd_tipo_pessoa = 2 then      
    case when ns.cd_cnpj_nota_saida is null or ltrim(rtrim(isnull(ns.cd_cnpj_nota_saida,''))) = '' then       
      ''      
    else       
      replicate('0', 11 - len(rtrim(ltrim(cast(replace(replace(replace(replace(isnull(ns.cd_cnpj_nota_saida,''),'.',''),'-',''),'/',''),' ','') as varchar(11)))))) +      
      rtrim(ltrim(cast(replace(replace(replace(replace(isnull(ns.cd_cnpj_nota_saida,''),'.',''),'-',''),'/',''),' ','') as varchar(11))))       
    end       
...
--    ltrim(rtrim(cast(dbo.fn_Formata_Mascara('00000000000', ns.cd_cnpj_nota_saida) as varchar(20))))      
  ...
  case when vw.cd_tipo_mercado=2 then
   ...
   'EXTERIOR'
    ...
    ltrim(rtrim(cast(cid.nm_cidade_ibge as varchar(60))))            
  end                                                              as 'xMunIBGE',	
...
replace(case when isnull(ns.cd_ddd_nota_saida,'')='' then  
  ...
  case when len(ltrim(rtrim(cast(ns.cd_ddd_nota_saida as varchar(4))))) > 2 then  
    ...
    ltrim(rtrim(cast(isnull(ns.cd_ddd_nota_saida,'') as varchar(4))))
  end  
end  
+ '' + ltrim(rtrim(replace(ns.cd_telefone_nota_saida,'-',''))),' ','00000000')  as varchar(14)) as 'fone',    
...
--     ns.sg_estado_entrega                       as 'UF_G09',
    ...
    ltrim(rtrim(case when isnull(ns.cd_cnpj_entrega_nota,'')<>'' then
      cast(replace(replace(replace(replace(ns.cd_cnpj_entrega_nota,'.',''),'-',''),'/',''),' ','') as varchar(14))
    else
    ...
    ( select top 1 cide.cd_cidade_ibge
      from
        Cidade cide with (nolock) 
        inner join Estado ee on ee.cd_estado = cide.cd_estado
      where
        isnull(cide.cd_cidade_ibge,'') <> ''                              and
       ...
       ee.sg_estado    = case when isnull(ns.sg_estado_entrega,'')<>''
                         then
...
--    ns.nm_cidade_nota_saida,
   ...
   cast(c.nm_email_cliente as varchar(60))                     as 'E19_email',
   ...
   --cast('00000' as varchar(20))                as 'E03a_idEstrangeiro',

   --cast('' as varchar(20))                       as 'E03a_idEstrangeiro',
 
   case when vw.cd_tipo_mercado = 2 and opf.cd_presenca = 2 or ns.sg_estado_nota_saida='EX' then 	   
     cast('11111' as varchar(20))
   else
     ''
   end                                           as 'E03a_idEstrangeiro',


   cast('' as varchar(15))                       as 'E18a_IM',
   isnull(opf.ic_imp_operacao_fiscal,'N')        as 'ic_imp_operacao_fiscal',
   isnull(opf.ic_exportacao_op_fiscal,'N')       as 'ic_exportacao_op_fiscal',
   isnull(opf.ic_importacao_op_fiscal,'N')       as 'ic_importacao_op_fiscal',
   c.cd_tipo_mercado                             as cd_tipo_mercado_cliente,
   opf.cd_presenca
...
--select * from cidade where nm_cidade = 'SAO PAULO'

--select * from tipo_pessoa
--select * from nota_saida ns
--select * from cliente

--select cd_tipo_operacao_fiscal,* from nota_saida
--select * from vw_destinatario where cd_destinatario = 10

------------------------------------------------------------------------------------
-- Testando a View
------------------------------------------------------------------------------------
--select top 1000 * from vw_nfe_destintario_nota_fiscal where cd_nota_saida = 20372
-----------------------------------------------------------------------------------
--select top 1000 xFant, xNome, * from vw_nfe_destintario_nota_fiscal order by dt_nota_saida desc
go
