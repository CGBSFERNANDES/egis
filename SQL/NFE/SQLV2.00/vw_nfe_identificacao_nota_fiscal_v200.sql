...
-- 02.05.2023 - Tipo Operacao Fiscal diretamente do cadastro de Grupo Operacao Fiscal - Pedro Jardim
-----------------------------------------------------------------------------------------------------------------    
as    
...
--  dbo.fn_strzero(ns.cd_identificacao_nota_saida,08)                                             as 'cNF',    
  ...
  case when dbo.fn_empresa() = 277 
    then ltrim(rtrim(cast(opf.cd_mascara_operacao as varchar(60))))
    else ltrim(rtrim(cast(ns.cd_mascara_operacao as varchar(60))))
  end                                                                                            as 'natOp',
  
  case when dbo.fn_empresa() = 277 
    then ltrim(rtrim(cast(opf.nm_operacao_fiscal  as varchar(60)))) 
    else ltrim(rtrim(cast(ns.nm_operacao_fiscal  as varchar(60))))
  end                                                                                            as 'DesNatOP',    
...
-- Indicador_Forma_Pagamento    
  ...
  case when (isnull(pf.ic_atualiza_data_saida_nf,'N') = 'S') and (ns.dt_saida_nota_saida is null) then
    ltrim(rtrim(replace(convert(char,getdate(),102),'.','-')))
  else
...
--FormatDateTime('YYYY-MM-DD"T"HH:MM:SS',Now)    
  ...
  ltrim( rtrim( dbo.fn_strzero(datepart(yyyy,ns.dt_nota_saida),4) +'-'+    
                ...
                case when isnull(ns.hr_nota_saida,'')='' or ns.dt_nota_saida<convert(datetime,left(convert(varchar,getdate(),121),10)+' 00:00:00',121)      
       then    
                  dbo.fn_hora_HHMMSS_NFE ( getdate() )    
               else      
                  ns.hr_nota_saida    
               end)) + case when isnull(v.ic_horario_verao,'N')='N' then '-03:00' else '-02:00' end   as 'dEmiNota',    
  ...
  ltrim( rtrim( dbo.fn_strzero(datepart(yyyy,ns.dt_saida_nota_saida),4) +'-'+    
                ...
                case when isnull(ns.hr_saida_nota_saida,'')=''  then    
                  ...
                  then    
                       case when isnull(ns.hr_nota_saida,'')='' or ns.dt_nota_saida<convert(datetime,left(convert(varchar,getdate(),121),10)+' 00:00:00',121)  then    
                        dbo.fn_hora_HHMMSS_NFE ( getdate() )    
                      else      
                        ns.hr_nota_saida    
                      end    
               ...
               end))+case when isnull(v.ic_horario_verao,'N')='N' then '-03:00' else '-02:00' end    
  else    
  ltrim( rtrim( dbo.fn_strzero(datepart(yyyy,convert(datetime,left(convert(varchar,getdate(),121),10)+' 00:00:00',121)),4) +'-'+    
                dbo.fn_strzero(datepart(mm,convert(datetime,left(convert(varchar,getdate(),121),10)+' 00:00:00',121)),2)   +'-'+    
                dbo.fn_strzero(datepart(dd,convert(datetime,left(convert(varchar,getdate(),121),10)+' 00:00:00',121)),2)   +'T'+    
...
--  ns.dt_nota_saida,    
  ...
  case when isnull(ns.hr_saida_nota_saida,'')='' or ns.dt_nota_saida<convert(datetime,left(convert(varchar,getdate(),121),10)+' 00:00:00',121)  then    
   dbo.fn_hora_HHMMSS_NFE ( getdate() )     
  else    
    ns.hr_saida_nota_saida    
...
--FormatDateTime('YYYY-MM-DD"T"HH:MM:SS',Now)    
  ...
  case when isnull(gof.cd_tipo_operacao_fiscal,2) = 2 then '1' else '0' end                     as 'tpNF',    
 ...
 --Nota Fiscal Referenciada    
  ...
  then    
    ...
    case when isnull(chvent.cd_chave_acesso,'') <> '' then
      isnull(chvent.cd_chave_acesso,'')
    else
      ''
    end
  end                                                                                           as 'refNFe',    
...
--  ns.cd_nota_saida                                                                           as 'nNf',      
  ...
  isnull(ns.vl_total,0)                  as vPag,
  case when isnull(nec.sg_presenca,'9') not in ('1','2','3','4','9') then
    ''
  else
    '0'
  end                                                as indIntermed
...
from    
  ...
  left outer join vw_nfe_chave_nota_entrada_devolucao chvent with (nolock) on chvent.cd_nota_saida = ns.cd_nota_saida    
...
-- Testando a View
------------------------------------------------------------------------------------
--select B25b_indPres,  * from vw_nfe_identificacao_nota_fiscal where cd_nota_saida = 548
...
--select cd_nota_saida, cd_identificacao_nota_saida, * from Nota_Saida