IF EXISTS (SELECT name 
	   FROM   sysobjects 
	   WHERE  name = N'vw_nfe_produto_servico_nota_fiscal_api' 
	   AND 	  type = 'V')
    DROP VIEW vw_nfe_produto_servico_nota_fiscal_api

GO
------------------------------------------------------------------------------------ 
--sp_helptext vw_nfe_produto_servico_nota_fiscal_api
------------------------------------------------------------------------------------
...
-- 10.08.2024 - Acerto da View NCM para geração de NFE por API - Carlos Fernandes
-- 05.08.2025 - Ajustes -- ccf/fagner - PIs/cofins = base
------------------------------------------------------------------------------------------------------------------------------- 
CREATE VIEW vw_nfe_produto_servico_nota_fiscal_api            
as            
...
select            
 ...
 case when isnull(pc.ic_desc_produto_cliente,'N')='S' then 
   case when isnull(pc.nm_produto_cliente,'') <> '' then 
     ...
     ltrim(rtrim(isnull(nsi.nm_produto_item_nota,p.nm_produto))) +
	  case when isnull(snf.ic_pcd_serie_nota_fiscal,'N') = 'S' and isnull(nsi.cd_pd_compra_item_nota,'')<>'' then    
        ' '+rtrim(ltrim(isnull(nsi.cd_pd_compra_item_nota,''))) +
	    ' Seq.: '+rtrim(ltrim(isnull(nsi.cd_it_ped_compra_cliente,''))) 	   
      else    
 ...
 --sem produto do cliente-- 
 else            
        cast( ltrim(rtrim( isnull(nsi.nm_produto_item_nota,p.nm_produto)))            
        +            
        case when isnull(pc.ic_nf_produto_cliente,'N')='S' and isnull(pc.nm_fantasia_prod_cliente,'')<>''            
        then            
 ...
 case when fam.nm_familia_produto <> 'LINHA DOMÉSTICA' then --Fagner        
  ...
  case when isnull(nsi.cd_servico,0)<>0 then            
    ...
    case when isnull(opf.ic_anulacao_frete,'N')='N' then            
      rtrim(ltrim(replace(isnull(cf.cd_mascara_classificacao,''),'.',''))) 
	  --+               
      --replicate('0',2 - len(rtrim(ltrim(replace(isnull(cf.cd_mascara_classificacao,''),'.','')))))            
    else            
      ''            
    end            
  end as varchar(8))                                                                                      as 'NCM',   

  isnull(cf.cd_cest_classificacao,'') as CEST,

  
  isnull(nsi.cd_classificacao_fiscal,0)                                                                   as cd_classificacao_fiscal,
  rtrim(ltrim(cast(isnull(cf.cd_extipi,'') as varchar(10))))                                                         as 'EXTIPI',            
  rtrim(ltrim(cast(isnull(cf.cd_genero_ncm_produto,'') as varchar(10))))                                             as 'genero',            
  ...
  then nsi.qt_item_nota_saida else null end,6,3)),103),'0.000')                                           as 'qCom',  
    ...
    then nsi.vl_unitario_item_nota else null end,25,6)),103),'0.000000')                                  as 'vUnCom',       
 --cast (round (      
 ...
 --    end , 6,4)  as varchar )                                                                                
    case when isnull(nsi.qt_item_nota_saida * nsi.vl_unitario_item_nota,0) > 0 then             
     ...
     case when isnull(efat.pc_aliquota_ibpt,0) > 0 then             
	   ((isnull(nsi.qt_item_nota_saida,0) * isnull(nsi.vl_unitario_item_nota,0)) * (efat.pc_aliquota_ibpt/100))
     else
	  case when isnull(cfi.qt_aliq_nacional,0) > 0 then
        ((isnull(nsi.qt_item_nota_saida,0) * isnull(nsi.vl_unitario_item_nota,0)) * (cfi.qt_aliq_nacional/100))
      else
	      0.00
      end
     end     as 'vTotTrib',
...
--nota_saida_item                            
  ...
  cast(             
         case when isnull(nsi.pc_desconto_item,0) > 0 then            
          isnull(CONVERT(varchar,             
            ...
            case when isnull(vl_item_desconto_nota,0)>0 then
              isnull(CONVERT(varchar,             
                convert(numeric(14,2),            
                round(  (  vl_item_desconto_nota   ),2)),103),'0.00')
            else   
               '0.00' end
            end   
  as varchar)                                    as 'vDesc',            
  ...
  dbo.fn_mascara_valor_duas_casas(isnull(cfe.pc_red_icms_clas_fiscal,0))                              as 'pRedBCICMS',                                
 ...
 rtrim(ltrim(cast(ti.cd_digito_tributacao_icms as varchar(2))))                                                        
  ...
  case when ti.cd_digito_tributacao_icms = '20' and isnull(nsi.vl_base_icms_item,0)>0 then                       
    dbo.fn_mascara_valor_duas_casas(case when isnull(nsi.pc_reducao_icms,0)>0 then (100 - isnull(nsi.pc_reducao_icms,0))else '0.00' end)                              
  else                      
    '0.00'            
  end                                                                                               as 'pREDBC20',                                
  case when ti.cd_digito_tributacao_icms = '20' then                       
...
--      cast('40' as varchar(2))                      
  ...
  cast(isnull(nsi.cd_it_ped_compra_cliente,'') as varchar (15)) as 'nItemPed',
...
--     cast(isnull(dbo.fn_strzero(nsi.cd_it_ped_compra_cliente, cp.qt_zero_ped_item_nfe),0) as varchar(15)) as 'nItemPed',
  ...
  cast(isnull(nsi.cd_pd_compra_item_nota,'') as varchar(15))                                                        as 'xPedCompra',            
...
--       cast('' as varchar(250))        
   ...
   case when isnull(vf.ic_codigo_rev_infoprod,'N')='S' then        
...
from            
  ...
  left outer join Classificacao_Fiscal_IBPT cfi     with (nolock) on cfi.cd_classificacao_fiscal   = cf.cd_classificacao_fiscal
...
-- Testando a View
------------------------------------------------------------------------------------------------------------------------------------------------------------------------
--select * from vw_nfe_produto_servico_nota_fiscal_api  where cd_nota_saida = 22410
------------------------------------------------------------------------------------------------------------------------------------------------------------------------