IF EXISTS (SELECT name 
	   FROM   sysobjects 
	   WHERE  name = N'pr_egis_relatorio_contabiliza_documentos_receber' 
	   AND 	  type = 'P')
    DROP PROCEDURE pr_egis_relatorio_contabiliza_documentos_receber
...
-- use egissql_360
------------------------------------------------------------------------------
create procedure pr_egis_relatorio_contabiliza_documentos_receber
@cd_relatorio int   = 0,
@cd_parametro int   = 0,
@json nvarchar(max) = '' 
...
------------------------------------------------------------------------------------------------------------
Create table #Contabilizacao_doc_receber(
	...
	HistoricoContabil         varchar(150),
	CodPortador               int, 
	Portador                  varchar(150)
)
insert into
	#Contabilizacao_doc_receber

exec pr_contabiliza_documento_receber @dt_inicial,@dt_final,0,'S'



--select * from ##Contabilizacao_doc_receber return
------------------------------------------------------------------------------------------------------------
select 
    identity(int,1,1)       as cd_controle,
	Portador				as Portador,
	CodPortador             as CodPortador,
	sum(Valor)              as vl_documento_receber_geral,
	sum(Debito)				as Debito_geral,
	sum(Credito)            as Credito_geral

	into 
	#Contabilizacao_doc_receber_geral
from 
	#Contabilizacao_doc_receber
...
;WITH TiposPagamento AS (
    SELECT DISTINCT
        Portador
    FROM #Contabilizacao_doc_receber
),
LinhasHTML AS (
    SELECT
        Portador,
...
(
    SELECT
        ...
        <div class="section-title" style="margin-top:30px;">
            <h3 style="margin:0;">Tipo de Pagamento: '+TP.Portador+'</h3>
        </div>
        ...
        <table style="width:100%; border-collapse:collapse;" border="1">
            <tr style="background:#f0f0f0;">
                ...
                <th>Valor</th>
                <th>Histórico</th>
				<th>Histórico Contabil</th>
				<th>Portador</th>
            </tr>
        '
        +

        -- Linhas do tipo de pagamento
        STUFF((
            SELECT L.LinhaHTML
            FROM LinhasHTML L
            WHERE L.Portador = TP.Portador
            FOR XML PATH(''), TYPE
            ...
            SELECT
                ...
                <tr style="font-weight:bold; background:#e8e8e8;">
                    <td colspan="3" style="text-align:right;">TOTAL</td>
                     ...
                     <td style="text-align:right;">'+ isnull(dbo.fn_formata_valor(G.vl_documento_receber_geral),0) +'</td>
                    <td colspan="3"></td>
                </tr>
                '
            FROM #Contabilizacao_doc_receber_geral G
            WHERE G.Portador = TP.Portador
        )
...
------------------------------------------------------------------------------
--exec pr_egis_relatorio_contabiliza_documentos_receber 231,''
------------------------------------------------------------------------------
