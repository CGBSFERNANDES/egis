IF EXISTS (SELECT name 
	   FROM   sysobjects 
	   WHERE  name = N'pr_egis_relatorio_adiantamento_fornecedor' 
	   AND 	  type = 'P')
    DROP PROCEDURE pr_egis_relatorio_adiantamento_fornecedor

GO

-------------------------------------------------------------------------------
--sp_helptext pr_egis_relatorio_adiantamento_fornecedor
-------------------------------------------------------------------------------
--pr_egis_relatorio_adiantamento_fornecedor
-------------------------------------------------------------------------------
...
--
------------------------------------------------------------------------------
create procedure pr_egis_relatorio_adiantamento_fornecedor
@cd_relatorio int   = 0,
@cd_parametro int   = 0,
@json nvarchar(max) = '' 
...
--    left outer join Tipo_Cobranca tc    with (nolock)                     on tc.cd_tipo_cobranca = d.cd_tipo_cobranca
 ...
 where
      ...
      --(isnull(d.dt_vencimento_documento,'') = @dt_hoje)  
	  --and
       d.cd_pedido_compra =  @cd_documento 
	   and
...
------------------------------------------------------------------------------------------------------------
DECLARE @cd_identificacao       nvarchar;
...
from #rel_pagamento_fornecedor
------------------------------------------------------------------------------------------------------------
 if isnull(@cd_parametro,0) = 1  
 begin  
    select * from #rel_pagamento_fornecedor  
  return  
 end  
--------------------------------------------------------------------------------------------------------------
set @html_geral = ' <h1 style="text-align: center;">'+isnull(@titulo,'Adiantamento de Forncedor')+'</h1> 
...
WHILE exists (select top 1 cd_controle from #rel_pagamento_fornecedor)
  begin
  select top 1 
	 @id         = cd_controle
	 from #rel_pagamento_fornecedor
   set  @html_geral = @html_geral+
	 '<tr style="text-align:center;">
                <td>'+cast(isnull(@cd_identificacao,0) as nvarchar(20))+'</td>
  ...
  DELETE FROM #rel_pagamento_fornecedor WHERE cd_controle = @id  
END  
-----------------------------------------------------------------------------------------------------------------
set @html_rodape = '
     </table>
...
-----------------------------------------------------------------------------
--exec pr_egis_relatorio_adiantamento_fornecedor 253,0,''
------------------------------------------------------------------------------
