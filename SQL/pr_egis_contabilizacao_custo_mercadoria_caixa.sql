IF OBJECT_ID('dbo.pr_egis_contabilizacao_custo_mercadoria_caixa', 'P') IS NOT NULL
    DROP PROCEDURE dbo.pr_egis_contabilizacao_custo_mercadoria_caixa;
GO

/*-------------------------------------------------------------------------------------------------
  pr_egis_contabilizacao_custo_mercadoria_caixa
---------------------------------------------------------------------------------------------------
...
---------------------------------------------------------------------------------------------------
  ...
  Objetivo         : Relatório HTML - Contabilização do CMV - Custo Mercadoria Vendida (cd_relatorio = 441)
...
BEGIN
    ...
    DECLARE
        @cd_relatorio             INT           = 441,
        @cd_empresa               INT           = 0,
        @cd_usuario               INT           = 0,
        @dt_inicial               DATETIME      = NULL,
        @dt_final                 DATETIME      = NULL,
        @titulo                   VARCHAR(200)  = 'Contabilização do CMV - Custo Mercadoria Vendida',
    ...
    BEGIN TRY
        ...
        /*-----------------------------------------------------------------------------------------
          4) Conta padrão de estoque para CMV
        -----------------------------------------------------------------------------------------*/
        DECLARE
            @cd_lancamento_padrao_estoque INT = 40,
            @cd_conta_debito_estoque      INT = 0,
            @cd_conta_credito_estoque     INT = 0;

        SELECT
            @cd_conta_debito_estoque = cd_conta_debito,
            @cd_conta_credito_estoque = cd_conta_credito
        FROM Lancamento_Padrao WITH (NOLOCK)
        WHERE cd_lancamento_padrao = @cd_lancamento_padrao_estoque;

        /*-----------------------------------------------------------------------------------------
          5) Base total do CMV
        -----------------------------------------------------------------------------------------*/
        SELECT
            n.dt_produto_fechamento,
            ...
            SUM(ISNULL(n.vl_custo_prod_fechamento, 0)) AS vl_debito,
            SUM(ISNULL(n.vl_custo_prod_fechamento, 0)) AS vl_credito,
            MAX('Total')             AS nm_tipo_contabilizacao
        INTO #Total
        FROM produto_fechamento AS n WITH (NOLOCK)
        INNER JOIN produto AS p WITH (NOLOCK)
            ON p.cd_produto = n.cd_produto
        INNER JOIN produto_compra AS pco WITH (NOLOCK)
            ON pco.cd_produto = n.cd_produto
        ...
        LEFT JOIN plano_conta AS pc WITH (NOLOCK)
            ON pc.cd_conta = ap.cd_conta_estoque
        LEFT JOIN plano_conta AS pd WITH (NOLOCK)
            ON pd.cd_conta = @cd_conta_debito_estoque
        WHERE n.dt_produto_fechamento BETWEEN @dt_inicial AND @dt_final
        ...
        SELECT
            @html_detalhe = (
                SELECT
                    '<tr>' +
                    '<td>' + CONVERT(VARCHAR(10), r.dt_produto_fechamento, 103) + '</td>' +
                    ...
                    '</tr>'
                FROM #Resultado AS r
                ORDER BY r.nm_tipo_contabilizacao, r.dt_produto_fechamento
                FOR XML PATH(''), TYPE
            ).value('.', 'nvarchar(max)');
