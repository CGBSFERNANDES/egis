IF EXISTS (SELECT name 
	   FROM   sysobjects 
	   WHERE  name = N'pr_egis_relatorio_pedido_aberto_vendedor' 
	   AND 	  type = 'P')
    DROP PROCEDURE pr_egis_relatorio_pedido_aberto_vendedor
...
--
------------------------------------------------------------------------------
create procedure pr_egis_relatorio_pedido_aberto_vendedor
@cd_relatorio int   = 0,
@cd_parametro int   = 0, 
@json nvarchar(max) = '' 
...
select
  ...
  sum(case when i.dt_cancelamento_item is null then        
          i.vl_unitario_item_pedido * i.qt_saldo_pedido_venda         
        else        
         0.00        
        end)                                      as vl_total_pedido,
...
from
  ...
  left outer join Pedido_Venda_Romaneio r       with(nolock) on r.cd_pedido_venda = p.cd_pedido_venda
...
where
  ...
  and
  i.cd_pedido_venda not in (select m.cd_pedido_venda from Movimento_Caixa_Item m 
													   where m.cd_pedido_venda = i.cd_pedido_venda and
	                                                         m.cd_item_pedido_venda = i.cd_item_pedido_venda)
   and
 
	-----------------------------------------
	year(p.dt_pedido_venda)>2024
	-----------------------------------------
	and
	isnull(r.cd_carga,0)=0
...
--------------------------------------------------------------------------------------------------------------
set @html_geral = '<div>
		<p class="section-title" style="text-align: center; padding: 8px;">Pedidos em Aberto por Vendedor</p>
	</div>
	<div>
    <table>  
		<tr class="tamanho">
...
------------------------------------------------------------------------------
--exec pr_egis_relatorio_pedido_aberto_vendedor 328,''
------------------------------------------------------------------------------
