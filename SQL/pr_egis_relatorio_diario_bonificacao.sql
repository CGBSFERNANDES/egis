IF EXISTS (SELECT name 
	   FROM   sysobjects 
	   WHERE  name = N'pr_egis_relatorio_diario_bonificacao' 
	   AND 	  type = 'P')
    DROP PROCEDURE pr_egis_relatorio_diario_bonificacao

GO

-------------------------------------------------------------------------------
--sp_helptext pr_egis_relatorio_diario_bonificacao
-------------------------------------------------------------------------------
--pr_egis_relatorio_diario_bonificacao
-------------------------------------------------------------------------------
...
--
------------------------------------------------------------------------------
create procedure pr_egis_relatorio_diario_bonificacao
@cd_relatorio int   = 0,
@cd_parametro int   = 0, 
@json nvarchar(max) = '' 
...
as
print 'iniciou a pr_egis_relatorio_diario_bonificacao'
...
-------------------------------------------------------------------------------------------------
  ...
  select @cd_documento           = valor from #json where campo = 'cd_vendedor'
...
where
  ...
  and
  p.cd_vendedor = case when isnull(@cd_documento,0)  = 0 then p.cd_vendedor else isnull(@cd_documento,0)  end
   and
  ...
  --and
  --ISNULL(tp.ic_gera_bi,'S') = 'N'
  and

  i.dt_cancelamento_item is null
  and
  ( ISNULL(tp.ic_bonificacao_tipo_pedido,'N') = 'S' or 
  
  ISNULL(tp.ic_indenizacao_tipo_pedido,'N') = 'S'
...
group by
  ...
  from
    Vendedor v
	    left outer join Meta_Vendedor_Periodo mvp on mvp.cd_vendedor              = v.cd_vendedor and 
											   ...
											   mvp.dt_final_validade_meta   = @dt_base_final  
  where
    v.cd_vendedor = case when isnull(@cd_documento,0)  = 0 then v.cd_vendedor else isnull(@cd_documento,0)  end
  ...
  from
   ...
   cast(cast(@qt_total as int ) as varchar(9)) 
	                                                                    as 'subtitle2Header',  
...
set @html_geral = '   
     <div class="section-title">  
        ...
        <p style="display: inline; text-align: center; padding: 15%;">Diário de Bonificação</p>  
    </div>  
	<div>
    <table>
      <tr class="tamanho">
...
WHILE EXISTS (SELECT TOP 1 cd_controle FROM #RelBonificacao)
BEGIN
    SELECT TOP 1
        @id                          = cd_controle,
        ...
        '<tr class="tamanho">
            ...
            <td>' + CAST(ISNULL(quantidade, '') AS NVARCHAR(10)) + '</td>
            <td>' + ISNULL(subcaption1, '') + '</td>
            <td>' + CAST(ISNULL(badgeCaption, '') AS NVARCHAR(10)) + '</td>
            <td>' + ISNULL(nm_tipo_pedido, '') + '</td>
            <td>' + cast(ISNULL(resultado, '') as nvarchar(20)) +'</td>
            <td>' + cast(ISNULL(percentual, '') as nvarchar(20)) + '</td>
	...
	FROM #RelBonificacao
    DELETE FROM #RelBonificacao WHERE cd_controle = @id
END
--------------------------------------------------------------------------------------------------------------------
...
select 'Diário de Bonificação' AS pdfName,isnull(@html,'') as RelatorioHTML
-------------------------------------------------------------------------------------------------------------------------------------------------------
...
------------------------------------------------------------------------------
--exec pr_egis_relatorio_diario_bonificacao 284,0,''
--exec pr_egis_relatorio_padrao '[{
--    "cd_menu": "0",
--    "cd_form": 168,
--    "cd_documento_form": 56,
--    "cd_parametro_form": "2",
--    "cd_usuario": "4439",
--    "cd_cliente_form": "0",
--    "cd_contato_form": "4439",
--    "dt_usuario": "2025-04-23",
--    "lookup_formEspecial": {},
--    "cd_parametro_relatorio": "56",
--    "cd_relatorio": "284",
--    "dt_inicial": "2025-03-01",
--    "dt_final": "2025-03-31",
--    "cd_cliente": 977,
--    "cd_tipo_pedido": 1,
--    "cd_vendedor": null ,
