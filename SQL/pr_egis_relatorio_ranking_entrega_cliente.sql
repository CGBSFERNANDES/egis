IF EXISTS (SELECT name 
	   FROM   sysobjects 
	   WHERE  name = N'pr_egis_relatorio_ranking_entrega_cliente' 
	   AND 	  type = 'P')
    DROP PROCEDURE pr_egis_relatorio_ranking_entrega_cliente

GO

-------------------------------------------------------------------------------
--sp_helptext pr_egis_relatorio_ranking_entrega_cliente
-------------------------------------------------------------------------------
--pr_egis_relatorio_ranking_entrega_clienter
-------------------------------------------------------------------------------
...
--
------------------------------------------------------------------------------
create procedure pr_egis_relatorio_ranking_entrega_cliente
@cd_relatorio int   = 0,
@cd_parametro int   = 0, 
@json nvarchar(max) = '' 
...
-------------------------------------------------------------------------------------------------
  ...
  select @dt_final               = valor from #json where campo = 'dt_final'
   ...
   if @cd_vendedor = 0
   begin
     select @cd_cliente           = valor from #json where campo = 'cd_documento_form'
...
WHILE EXISTS (SELECT TOP 1 cd_controle FROM #RankingCliente)
BEGIN
    SELECT TOP 1
        @id                          = cd_controle,
	...
	FROM #RankingCliente
    DELETE FROM #RankingCliente WHERE cd_controle = @id
END
--------------------------------------------------------------------------------------------------------------------
...
------------------------------------------------------------------------------
--exec pr_egis_relatorio_ranking_entrega_cliente 344,0,''
------------------------------------------------------------------------------
