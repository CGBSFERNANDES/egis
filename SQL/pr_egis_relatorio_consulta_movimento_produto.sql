IF EXISTS (SELECT name 
	   FROM   sysobjects 
	   WHERE  name = N'pr_egis_relatorio_consulta_movimento_produto' 
	   AND 	  type = 'P')
    DROP PROCEDURE pr_egis_relatorio_consulta_movimento_produto

GO

-------------------------------------------------------------------------------
--sp_helptext pr_egis_relatorio_consulta_movimento_produto
-------------------------------------------------------------------------------
--pr_egis_relatorio_consulta_movimento_produto
-------------------------------------------------------------------------------
...
--
------------------------------------------------------------------------------
create procedure pr_egis_relatorio_consulta_movimento_produto
@cd_relatorio int   = 0,
@cd_parametro int   = 0, 
@json nvarchar(max) = '' 
...
--set @dt_inicial = '11/07/2025'
--set @dt_final = '11/07/2025'
--use egissql_374
--------------------------------------------------------------------------------------------------------------------------
select 
	identity(int,1,1)           as cd_controle,
	mci.cd_movimento_caixa      as cd_movimento_caixa,
	dbo.fn_data_string(mc.dt_movimento_caixa)       as dt_movimento_caixa,
...
where 
	...
	and 
	mci.cd_produto = case when isnull(@cd_produto,0) = 0 then mci.cd_produto else isnull(@cd_produto,0) end 

order by 
	p.nm_produto,
	mci.cd_movimento_caixa

declare 
	@cd_produto_geral int,
	@qt_item_movimento_caixa int,
	@vl_total_item float = 0
	
select 
	@cd_produto_geral        = count(distinct(cd_produto)),
	@qt_item_movimento_caixa = sum(qt_item_movimento_caixa),
	@vl_total_item           = sum(vl_total_item)
...
select @html_geral = ' <div class="section-title">  
        ...
        <p style="display: inline; text-align: center; padding: 20%;">Consulta Produto por Movimento</p>  
    </div>
	<table>
...
(
    SELECT
        ...
        '
    FROM #MovimentoProduto
    ORDER BY nm_produto, cd_movimento_caixa
    FOR XML PATH(''), TYPE
).value('.', 'NVARCHAR(MAX)')
+
''
...
------------------------------------------------------------------------------
--exec pr_egis_relatorio_consulta_movimento_produto 374,0,''
------------------------------------------------------------------------------
