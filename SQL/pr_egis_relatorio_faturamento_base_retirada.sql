IF EXISTS (SELECT name 
	   FROM   sysobjects 
	   WHERE  name = N'pr_egis_relatorio_faturamento_base_retirada' 
	   AND 	  type = 'P')
    DROP PROCEDURE pr_egis_relatorio_faturamento_base_retirada

GO

-------------------------------------------------------------------------------
--sp_helptext pr_egis_relatorio_faturamento_base_retirada
-------------------------------------------------------------------------------
--pr_egis_relatorio_faturamento_base_retirada
-------------------------------------------------------------------------------
...
--
------------------------------------------------------------------------------
create procedure pr_egis_relatorio_faturamento_base_retirada
@cd_relatorio int   = 0,
@cd_parametro int   = 0, 
@json nvarchar(max) = '' 
...
where
 ...
 and 
 d.cd_base = case when ISNULL(@cd_base,0) = 0 then d.cd_base else ISNULL(@cd_base,0) end
 group by 
 br.cd_base,
 br.nm_base
...
------------------------------------------------------------------------------------------------------------		 
 ...
 begin  
    select * from #faturamentoBaseRetirada  
  return  
 end  
...
WHILE EXISTS (SELECT TOP 1 cd_controle FROM #faturamentoBaseRetirada)
BEGIN
    SELECT TOP 1
        @id                          = cd_controle,
        ...
        '<tr class="tamanho">
		    <td>' + ISNULL(nm_base,'')+' ('+cast(isnull(cd_base,0) as varchar(20))+')</td>		
	...
	FROM #faturamentoBaseRetirada
    DELETE FROM #faturamentoBaseRetirada WHERE cd_controle = @id
END
--------------------------------------------------------------------------------------------------------------------
...
------------------------------------------------------------------------------
--exec pr_egis_relatorio_faturamento_base_retirada 345,0,''
------------------------------------------------------------------------------
