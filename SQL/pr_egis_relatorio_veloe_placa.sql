IF EXISTS (SELECT name 
	   FROM   sysobjects 
	   WHERE  name = N'pr_egis_relatorio_veloe_placa' 
	   AND 	  type = 'P')
    DROP PROCEDURE pr_egis_relatorio_veloe_placa

GO
  
-------------------------------------------------------------------------------  
--sp_helptext pr_egis_relatorio_veloe_placa  
-------------------------------------------------------------------------------  
--pr_egis_relatorio_veloe_placa  
-------------------------------------------------------------------------------  
...
--  
------------------------------------------------------------------------------  
create procedure pr_egis_relatorio_veloe_placa
@cd_relatorio int   = 0,  
@cd_parametro int   = 0,  
@json nvarchar(max) = ''   
...
if isnull(@cd_parametro,0) = 2    
 ...
 ------------------------------------------------------------------------------------------------------------------------- 
  ...
  select 
	...
	MAX(year(transactionDate))             as cd_ano
...
WHILE EXISTS (SELECT TOP 1 cd_controle FROM #VeloeVeiculo)  
BEGIN  
    SELECT TOP 1  
        @id                          = cd_controle,  
        @html_geral = @html_geral +  
			'<tr class="tamanho">  
			   <td>'+isnull(nm_placa,0)+'</td>  
			   <td>'+isnull(dbo.fn_data_string(dt_movimento),'')+'</td>  
               <td>'+isnull(vehicleModel,0)+'</td> 
	...
	FROM
	  #VeloeVeiculo  
    DELETE FROM #VeloeVeiculo WHERE cd_controle = @id  
END  
--------------------------------------------------------------------------------------------------------------------  
...
--exec pr_egis_relatorio_veloe_placa 305,0,''