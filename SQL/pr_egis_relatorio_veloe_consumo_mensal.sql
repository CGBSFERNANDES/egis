IF EXISTS (SELECT name 
	   FROM   sysobjects 
	   WHERE  name = N'pr_egis_relatorio_veloe_consumo_mensal' 
	   AND 	  type = 'P')
    DROP PROCEDURE pr_egis_relatorio_veloe_consumo_mensal

GO
  
-------------------------------------------------------------------------------  
--sp_helptext pr_egis_relatorio_veloe_consumo_anual  
-------------------------------------------------------------------------------  
--pr_egis_relatorio_veloe_consumo_anual  
-------------------------------------------------------------------------------  
...
--  
------------------------------------------------------------------------------  
create procedure pr_egis_relatorio_veloe_consumo_mensal
@cd_relatorio int   = 0,  
@cd_parametro int   = 0,  
@json nvarchar(max) = ''   
...
if isnull(@cd_parametro,0) = 2    
 ...
 ------------------------------------------------------------------------------------------------------------------------- 
  ...
  select 
    ...
    year(transactionDate)                  as cd_ano,
	MONTH(transactionDate)                 as cd_mes,
  ...
  #MensalVeloeRel
  from 
    Veloe_Historico_Abastecimento v
	left outer join Mes m            on m.cd_mes = MONTH(v.transactionDate)

  where
    year(transactionDate) = year(@dt_inicial)
...
WHILE EXISTS (SELECT TOP 1 cd_controle FROM #MensalVeloeRel)  
BEGIN  
    SELECT TOP 1  
        @id                          = cd_controle,  
        @html_geral = @html_geral +  
			'<tr class="tamanho">  
               <td>'+cast(isnull(cd_ano,0)as nvarchar(20))+'</td>  
			   <td>'+isnull(nm_mes,0)+'</td>  
	...
	FROM
	  #MensalVeloeRel  
    DELETE FROM #MensalVeloeRel WHERE cd_controle = @id  
END  
--------------------------------------------------------------------------------------------------------------------  
...
--exec pr_egis_relatorio_veloe_consumo_mensal 305,0,''