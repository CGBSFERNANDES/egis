-------------------------------------------------------------------------------          
--sp_helptext pr_egismob_relatorio_pedido_venda          
-------------------------------------------------------------------------------          
...
-- 03.10.2025 - carlos Fernandes
---------------------------------------------------------------------------------------------
create or alter procedure pr_egismob_relatorio_pedido_venda   
@cd_relatorio int = 0,
@cd_usuario   int = 0,
@json         nvarchar(max) = ''       
...
begin
  ...
  -- select @cd_usuario                = valor from #json  with(nolock) where campo = 'cd_usuario'     
   ...
   --empresa_faturamento--
	...
	--select @cd_pedido_venda, @cd_relatorio, @cd_usuario --select * from parametro_relatorio
	...
	select
	  ...
	  dbo.fn_formata_valor(isnull(c.vl_unitario_item_consulta,0))		 as vl_unitario_item_consulta,
	...
	select 
	  ...
	  isnull(case when pvi.dt_cancelamento_item is null 
			 ...
			 end ,0)													as vl_unitario_item_consulta2,
	  dbo.fn_formata_valor(isnull(case when pvi.dt_cancelamento_item is null 
									   then pvi.vl_unitario_item_pedido * pvi.qt_item_pedido_venda         
									   else 0.00        
								  end,0))								as vl_unitario_item_consulta,
	  'R$' + dbo.fn_formata_valor(isnull(case when pvi.dt_cancelamento_item is null 
											  then pvi.vl_unitario_item_pedido * pvi.qt_item_pedido_venda         
											  else 0.00        
	...
	begin
      ...
      if @qt_bem>0
	  begin
	    set @dados_bem = 'Comodato: '+cast(@qt_bem as varchar(9)) + ' - ' +@nm_bem
		--select @dados_bem
	  end
...
<head>
    ...
    <style>
		...
		p {
		  ...
		  .totalQuantidade {  
             padding-left: '+ case when isnull(@ic_imposto_tipo_pedido,'N') = 'S'    and @cd_empresa in (342,350) 
                                   then '230px;'   
		                           else '250px;'   
                              end +  
        '}  
...
<body>
	  ...
	  <div class="proposal-info">
		...
		<div style="display: flex; justify-content: space-between; align-items:center; margin-bottom: 0px; padding-bottom: 0px">
			<div style="width:50%">
				...
				<p><strong>Pedido de Compra: </strong>'+isnull(@cd_pdcompra_pedido_venda,'')+'</p>
			</div>
			<div style="width:50%;">
				<p><strong>Status: </strong>' + ltrim(rtrim(isnull(@nm_status, ''))) + '</p>' +
				case 
					when @ic_empresa_faturamento = 'S' then 
    ...
    <p><strong>Telefone: </strong>'+isnull(@cd_telefone,'')+' <strong> - Contato: </strong>' + isnull(@nm_contato_cliente,'') + '</p>
    ...
    <table>
        <tr>
			...
			<th style="font-size:12px;">Preço (R$)</th>'+
			 case when isnull(@ic_imposto_tipo_pedido,'N') = 'N' and @cd_empresa in (342,350)
        then '' 
        else ' <th style="font-size:12px;">ICMS-ST (R$)</th>  
    ...
    SET @html = @html + '
        <tr>
			...
			<td style="font-size:12px;text-align:right">'+isnull(@vl_unitario_item_consulta,'')+'</td>'+
			 case when isnull(@ic_imposto_tipo_pedido,'N') = 'N' and @cd_empresa in (342,350)  
        then ''
        else '<td style="font-size:12px;text-align:right">'+isnull(@ICMS,'')+'</td>  
...
set @vl_final = 'R$ ' + dbo.fn_formata_valor(isnull(@vl_total_pedido_venda,0))
...
--exec pr_modulo_processo_egismob_post '[{"cd_consulta": 7, "cd_menu": 5428, "cd_parametro": 14, "cd_pedido_venda": 7, "cd_usuario": 4073}]'
--select * from parametro_relatorio

--exec pr_egismob_relatorio_pedido_venda 166,4375,''
...
--    "cd_modulo": "362",
--    "cd_menu": "0",
--    "cd_relatorio_form": 385,
--    "cd_processo": "",
--    "cd_form": 91,
--    "cd_documento_form": 149,
--    "cd_parametro_form": "2",
--    "cd_usuario": "4254",
--    "cd_cliente_form": "0",
--    "cd_contato_form": "4254",
--    "dt_usuario": "2025-09-19",
--    "lookup_formEspecial": {},
--    "cd_parametro_relatorio": "166",
--    "cd_relatorio": "385",
