IF EXISTS (SELECT name 
	   FROM   sysobjects 
	   WHERE  name = N'pr_relatorio_produto_preco_zero_tabela_preco' 
	   AND 	  type = 'P')
    DROP PROCEDURE pr_relatorio_produto_preco_zero_tabela_preco

GO

-------------------------------------------------------------------------------
--sp_helptext pr_relatorio_produto_preco_zero_tabela_preco
-------------------------------------------------------------------------------
--pr_relatorio_produto_preco_zero_tabela_preco
-------------------------------------------------------------------------------
...
--
------------------------------------------------------------------------------
create procedure pr_relatorio_produto_preco_zero_tabela_preco
@cd_relatorio int   = 0,
@json nvarchar(max) = '' 
...
select
  ...
  cast(@dt_hoje - tpp.dt_usuario as int ) as dias
  into
  #preco_zero
...
order by
  ...
  --select * from #preco_zero
-------------------------------------------------------------------------------------------------------------
declare @cd_tabela_preco       int = 0
declare @nm_tabela_preco       nvarchar(100)
declare @sg_tabela_preco       nvarchar(30)
declare @dt_usuario            datetime
...
--------------------------------------------------------------------------------------------------------------
 select 
	@qt_total =	COUNT(cd_tabela_preco)
  from 
  #preco_zero
--------------------------------------------------------------------------------------------------------------
set @html_geral = '<div style="text-align: center" class="section-title">
			           <strong>Produtos com Preço Zero na Tabela </strong>
				   </div>
				   <table>
					   <tr>
					    	<td class="tamanho" style="font-size: 110%;"><strong>Item</strong></td> 
							<td class="tamanho" style="font-size: 110%;"><strong>Tabela</strong></td>
...
declare @id int = 0
while exists ( select top 1 cd_controle from #preco_zero where cd_controle <= 500)
begin
	select top 1
		@id                         = cd_controle,
		@sg_tabela_preco            = sg_tabela_preco,
  ...
  set @html_geral = @html_geral +'<tr>
									...
									<td class="tamanho">'+isnull(@nm_tabela_preco,0)+' ('+CAST(isnull(@cd_tabela_preco,'') as nvarchar(10))+')</td>
									<td class="tamanho">'+CONVERT(VARCHAR,@dt_usuario, 103)+'</td>
	 ...
	 delete from #preco_zero where cd_controle = @id
 end
--------------------------------------------------------------------------------------------------------------------
...
set @html_rodape =
	...
	<div class="section-title">
		<p><strong>Total de Produtos:</strong> '+cast(isnull(@qt_total,'') as nvarchar(10))+'</p>
    </div>
		'+
	case when isnull(@id,0) >= 400 then 
	'<div>
...
------------------------------------------------------------------------------
--exec pr_relatorio_produto_preco_zero_tabela_preco 221,'' 
------------------------------------------------------------------------------
