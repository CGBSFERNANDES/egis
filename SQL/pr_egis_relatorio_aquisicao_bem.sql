IF OBJECT_ID('dbo.pr_egis_relatorio_aquisicao_bem', 'P') IS NOT NULL
    DROP PROCEDURE dbo.pr_egis_relatorio_aquisicao_bem;
GO

/*-------------------------------------------------------------------------------------------------
  pr_egis_relatorio_aquisicao_bem
---------------------------------------------------------------------------------------------------
...
---------------------------------------------------------------------------------------------------
  ...
  Objetivo         : Relatório HTML - Aquisição de Bens do Ativo (cd_relatorio = 442)
...
BEGIN
    ...
    BEGIN TRY
        ...
        SELECT
            b.dt_aquisicao_bem,
            CAST(b.dt_aquisicao_bem AS DATE) AS dt_aquisicao_bem_data,
            b.cd_bem,
            b.nm_bem,
            b.cd_patrimonio_bem,
            gb.nm_grupo_bem,
        ...
        LEFT JOIN Status_Bem AS sb
            ON sb.cd_status_bem = b.cd_status_bem
        LEFT JOIN Centro_Custo AS cc
            ON cc.cd_centro_custo = b.cd_centro_custo
        LEFT JOIN Serie_Nota_Fiscal AS snf
            ON snf.cd_serie_nota_fiscal = b.cd_serie_nota_fiscal
        LEFT JOIN fornecedor AS f
            ON f.cd_fornecedor = b.cd_fornecedor
        LEFT JOIN Departamento AS d
            ON d.cd_departamento = b.cd_departamento
        LEFT JOIN Planta AS p
            ON p.cd_planta = b.cd_planta
        LEFT JOIN Seguradora AS s
            ON s.cd_seguradora = b.cd_seguradora
        LEFT JOIN Operacao_Fiscal AS opf
            ON opf.cd_operacao_fiscal = b.cd_operacao_fiscal
        LEFT JOIN Valor_Bem AS vb
            ON vb.cd_bem = b.cd_bem
        WHERE
        ...
        SELECT
            ...
            @vl_total_geral = ISNULL(SUM(vl_original_bem), 0)
        FROM #dados;
...
N'<style>
    ...
    .total { background: #e9f5e9; font-weight: 700; }
        ...
        SET @html_body = (
            SELECT
                N'<tr class="group"><td colspan="22">Data Aquisição: ' +
                CONVERT(VARCHAR(10), d.dt_aquisicao_bem_data, 103) + N'</td></tr>' +
                (
                    SELECT
                        N'<tr>' +
                            N'<td>' + CONVERT(VARCHAR(10), d2.dt_aquisicao_bem, 103) + N'</td>' +
                            ...
                            N'<td>' + ISNULL(d2.sg_serie_nota_fiscal, '') + N'</td>' +
                            N'<td>' + ISNULL(CAST(d2.cd_item_nota_entrada AS NVARCHAR(20)), '') + N'</td>' +
                            N'<td>' + ISNULL(CAST(d2.qt_item_nota_entrada AS NVARCHAR(20)), '') + N'</td>' +
                            N'<td>' + ISNULL(CAST(d2.cd_fornecedor AS NVARCHAR(20)), '') + N'</td>' +
                            N'<td>' + ISNULL(d2.nm_fantasia_fornecedor, '') + N'</td>' +
                            N'<td>' + ISNULL(d2.nm_departamento, '') + N'</td>' +
                            N'<td>' + ISNULL(CAST(d2.qt_bem AS NVARCHAR(20)), '') + N'</td>' +
                            N'<td>' + ISNULL(d2.nm_planta, '') + N'</td>' +
                            N'<td>' + ISNULL(d2.nm_seguradora, '') + N'</td>' +
                            N'<td>' + ISNULL(d2.cd_mascara_operacao, '') + N'</td>' +
                            N'<td>' + ISNULL(CONVERT(VARCHAR(10), d2.dt_inicio_uso_bem, 103), '') + N'</td>' +
                            N'<td>' + ISNULL(CONVERT(VARCHAR(10), d2.dt_garantia_bem, 103), '') + N'</td>' +
                            N'<td>' + ISNULL(CONVERT(NVARCHAR(30), CAST(ISNULL(d2.vl_original_bem, 0) AS MONEY), 1), '0,00') + N'</td>' +
                        N'</tr>'
                    FROM #dados AS d2
                    WHERE d2.dt_aquisicao_bem_data = d.dt_aquisicao_bem_data
                    ORDER BY d2.dt_aquisicao_bem, d2.cd_patrimonio_bem
                    FOR XML PATH(''), TYPE
                ).value('.', 'nvarchar(max)') +
                N'<tr class="subtotal"><td colspan="21">Subtotal (' +
                CONVERT(VARCHAR(10), d.dt_aquisicao_bem_data, 103) + N')</td>' +
                N'<td>' + CONVERT(NVARCHAR(30), CAST(ISNULL(s.vl_subtotal, 0) AS MONEY), 1) + N'</td></tr>'
            FROM (SELECT DISTINCT dt_aquisicao_bem_data FROM #dados) AS d
            CROSS APPLY (
                SELECT SUM(vl_original_bem) AS vl_subtotal
                FROM #dados AS d3
                WHERE d3.dt_aquisicao_bem_data = d.dt_aquisicao_bem_data
            ) AS s
            ORDER BY d.dt_aquisicao_bem_data
            FOR XML PATH(''), TYPE
        ).value('.', 'nvarchar(max)');

        IF @qt_registros = 0
            SET @html_body = N'<tr><td colspan="22">Nenhum registro encontrado para o período informado.</td></tr>';
                ...
                N'<table class="table">' +
                    N'<thead>' +
                        N'<tr>' +
                            ...
                            N'<th>Qt. Item</th>' +
                            N'<th>Fornecedor</th>' +
                            N'<th>Fantasia Fornecedor</th>' +
                            N'<th>Departamento</th>' +
                            N'<th>Qt. Bem</th>' +
                            N'<th>Planta</th>' +
                            N'<th>Seguradora</th>' +
                            N'<th>Operação</th>' +
                            N'<th>Dt. Início Uso</th>' +
                            N'<th>Dt. Garantia</th>' +
                    ...
                    N'<tbody>' +
                        @html_body +
                        N'<tr class="total"><td colspan="21">Total Geral</td><td>' +
