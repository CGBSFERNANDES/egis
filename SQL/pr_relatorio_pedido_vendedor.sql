IF EXISTS (SELECT name 
	   FROM   sysobjects 
	   WHERE  name = N'pr_relatorio_pedido_vendedor' 
	   AND 	  type = 'P')
    DROP PROCEDURE pr_relatorio_pedido_vendedor

GO

  
-------------------------------------------------------------------------------  
--sp_helptext pr_relatorio_pedido_vendedor  
-- use egissql_342
-------------------------------------------------------------------------------  
--pr_relatorio_pedido_vendedor  
-------------------------------------------------------------------------------  
...
--  
------------------------------------------------------------------------------  
create procedure pr_relatorio_pedido_vendedor
@cd_relatorio int   = 0,  
@cd_documento int   = 0,  
@cd_parametro int   = 0,  
@json nvarchar(max) = ''   
...
--  dbo.fn_hora_string(p.hr_inicial_pedido) as hr_inicial_pedido,  
  ...
  max(ap.nm_aplicacao_produto)                                                as nm_aplicacao_produto,
...
from  
  ...
  left outer join Aplicacao_Produto ap                with (nolock)              on ap.cd_aplicacao_produto = p.cd_aplicacao_produto
...
group by  
 ...
 if @cd_empresa in (342,350)
 begin
   delete from #Diario
   where cd_tipo_pedido in (20,25,30,31)
 end
...
--------------------------------------------------------------------------------------------------------------  
DECLARE @id int = 0   
declare @nm_condicao_pagamento nvarchar(60)  
...
------------------------------------------------------------------------------
--exec pr_relatorio_pedido_vendedor 232,11,''
------------------------------------------------------------------------------
