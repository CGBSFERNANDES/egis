IF EXISTS (SELECT name 
	   FROM   sysobjects 
	   WHERE  name = N'pr_egis_relatorio_carteira_pedidos' 
	   AND 	  type = 'P')
    DROP PROCEDURE pr_egis_relatorio_carteira_pedidos

GO

-------------------------------------------------------------------------------
--sp_helptext pr_egis_relatorio_carteira_pedidos
-------------------------------------------------------------------------------
--pr_egis_relatorio_carteira_pedidos
-------------------------------------------------------------------------------
...
--
------------------------------------------------------------------------------
create procedure pr_egis_relatorio_carteira_pedidos
@cd_relatorio int   = 0,
@json nvarchar(max) = '' 
...
where
  p.cd_pedido_venda in ( select i.cd_pedido_venda 
...
order by
  dt_pedido_venda desc, vl_total_pedido_ipi desc
...
select
  ...
  *,
  pc_faturamento = cast(round(vl_total_pedido_ipi/@vl_total,2)*100 as decimal(25,2))
into
  #CarteiraFinal
from 
  #Carteira
order by
  vl_total_pedido_ipi desc

--select * from #CarteiraFinal
...
while exists ( select top 1 cd_controle from #CarteiraFinal )
begin
  ...
  select 
    ...
    @html_detalhe = @html_detalhe + '
            <tr> 					           
			<td style="font-size:12px; text-align:center;width: 20px">'+isnull(dbo.fn_data_string(dt_pedido_venda),'')+'</td>
			<td style="font-size:12px; text-align:center;width: 20px">'+CAST(isnull(cd_pedido_venda,'') as varchar(20))+'</td>
			<td style="font-size:12px; text-align:center;width: 20px">'+isnull(dbo.fn_formata_valor(vl_total_pedido_ipi),'')+'</td>			
			<td style="font-size:12px; text-align:center;width: 20px">'+isnull(dbo.fn_formata_valor(pc_faturamento),'')+'</td>	
			<td style="font-size:12px; text-align:center;width: 20px">'+cast(isnull(cd_cliente,'') as varchar(20))+'</td>			
			<td style="font-size:12px; text-align:center;width: 20px">'+isnull(nm_fantasia_cliente,'')+'</td>			
			...
			<td style="font-size:12px; text-align:center;width: 20px">'+isnull(nm_itinerario,'')+'</td>
...
set @html_totais = '<div class="section-title"><strong>TOTAL</strong>					
					  <p style="font-size:18px; text-align:left;">Pedidos: '+cast(isnull(@qt_total,0) as varchar(10))+'</p>
			          <p style="font-size:18px; text-align:left;">Valor: '+'R$ '+isnull(dbo.fn_formata_valor(@vl_total),0)+'</p>
...
------------------------------------------------------------------------------
--exec pr_egis_relatorio_carteira_pedidos 38,''
------------------------------------------------------------------------------
