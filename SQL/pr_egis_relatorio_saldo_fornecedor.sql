IF EXISTS (SELECT name 
	   FROM   sysobjects 
	   WHERE  name = N'pr_egis_relatorio_saldo_fornecedor' 
	   AND 	  type = 'P')
    DROP PROCEDURE pr_egis_relatorio_saldo_fornecedor

GO

-------------------------------------------------------------------------------
--sp_helptext pr_egis_relatorio_saldo_fornecedor
-------------------------------------------------------------------------------
--pr_egis_relatorio_saldo_fornecedor
-------------------------------------------------------------------------------
...
--
------------------------------------------------------------------------------
create procedure pr_egis_relatorio_saldo_fornecedor
@cd_relatorio int   = 0,
@cd_usuario   int   = 0,
@cd_parametro int   = 0,
@json nvarchar(max) = '' 
...
select
  d.cd_fornecedor,
  MAX(isnull(f.nm_fantasia_fornecedor, ed.nm_empresa_diversa))              as nm_fantasia,
  MAX(isnull(f.nm_razao_social, ed.nm_empresa_diversa))                     as nm_razao,
  Max(ed.nm_empresa_diversa)                                                as nm_empresa_diversa,
...
from
  Documento_Pagar d
  left outer join Fornecedor f       on f.cd_fornecedor       = d.cd_fornecedor
  left outer join Empresa_Diversa ed on ed.cd_empresa_diversa = d.cd_empresa_diversa
...
select  
  @qt_total_fornecedor  = count(cd_fornecedor),
...
begin
  ...
  begin
    ...
    select top 1 
      ...
      @html_detalhe = @html_detalhe + '
			...
			<td style="font-size:12px; text-align:center;width: 20px">'+CAST(nm_fantasia as varchar(30)) +'</td>
  ...
  --Dados da Tabela---
   ...
   set @tabela = '<div class="container mt-5">
    <h2 class="mb-4">'+'Saldo de Fornecedores'+'</h2>
...
<div class="secao4" id="sobre">
  <div class="secao4-div">
      ...
      <div class="secao4-div-card">   
          ...
          <p> '+cast(@qt_total_fornecedor as varchar(20))+ '<br></p>
      </div>
...
------------------------------------------------------------------------------
--Gráfico
exec pr_egis_relatorio_saldo_fornecedor 185,4253,3,''
------------------------------------------------------------------------------
