IF EXISTS (SELECT name 
	   FROM   sysobjects 
	   WHERE  name = N'pr_egis_relatorio_veloe_fornecedor' 
	   AND 	  type = 'P')
    DROP PROCEDURE pr_egis_relatorio_veloe_fornecedor

GO
  
-------------------------------------------------------------------------------  
--sp_helptext pr_egis_relatorio_veloe_fornecedor  
-------------------------------------------------------------------------------  
--pr_egis_relatorio_veloe_fornecedor  
-------------------------------------------------------------------------------  
...
--  
------------------------------------------------------------------------------  
create procedure pr_egis_relatorio_veloe_fornecedor
@cd_relatorio int   = 0,  
@cd_parametro int   = 0,  
@json nvarchar(max) = ''   
...
if isnull(@cd_parametro,0) = 2    
 ...
 ------------------------------------------------------------------------------------------------------------------------- 
  ...
  select 
    ...
    supplyLocation                         as nm_fornecedor,
	max(merchantState)                     as nm_cidade,
...
WHILE EXISTS (SELECT TOP 1 cd_controle FROM #VeloeFornecedor)  
BEGIN  
    SELECT TOP 1  
        @id                          = cd_controle,  
        @html_geral = @html_geral +  
			'<tr class="tamanho">  
			   <td>'+isnull(nm_fornecedor,'')+'</td>  
	...
	FROM
	  #VeloeFornecedor  
    DELETE FROM #VeloeFornecedor WHERE cd_controle = @id  
END  
--------------------------------------------------------------------------------------------------------------------  
...
--exec pr_egis_relatorio_veloe_fornecedor 305,0,''