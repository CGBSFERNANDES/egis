IF EXISTS (SELECT name 
	   FROM   sysobjects 
	   WHERE  name = N'pr_egis_relatorio_entrega_ph_cloro' 
	   AND 	  type = 'P')
    DROP PROCEDURE pr_egis_relatorio_entrega_ph_cloro

GO

-------------------------------------------------------------------------------
--sp_helptext pr_egis_relatorio_entrega_ph_cloro
-------------------------------------------------------------------------------
--pr_egis_relatorio_entrega_ph_cloro
-------------------------------------------------------------------------------
...
--
------------------------------------------------------------------------------
create procedure pr_egis_relatorio_entrega_ph_cloro
@cd_relatorio int   = 0,
@cd_parametro int   = 0, 
@json nvarchar(max) = '' 
...
-------------------------------------------------------------------------------------------------
  ...
  select @cd_base                = valor from #json where campo = 'cd_base'
...
select 
   identity(int,1,1)               as cd_controle,
	r.cd_romaneio                   as cd_romaneio,
	ri.vl_total_romaneio            as vl_romaneio,
	...
	br.nm_base						as nm_base,
	d.qt_cloro                      as qt_cloro,
	d.qt_ph_agua                    as qt_ph_agua,
	qt_km = isnull(d.qt_odometro_final,0) - isnull(d.qt_odometro_inicial,0)
...
declare
	...
	@qt_ph_agua    float = 0
select 	
	@qt_entrega    = count(cd_romaneio),
	@vl_romaneio   = sum(vl_romaneio),
	@qt_produto    = count(distinct(nm_produto)),
	@qt_servico    = count(distinct(nm_servico)),
	@qt_cliente    = count(distinct(cd_cliente)),
	@qt_veiculo    = count(distinct(nm_veiculo)),
	@qt_base       = count(distinct(nm_base)), 
	@qt_entregador = count(distinct(nm_entregador)),
	@qt_km         = sum(qt_km),
	@qt_cloro  	   = sum(qt_cloro),
	@qt_ph_agua    = sum(qt_ph_agua) 
from #EntregaPhCloro
--------------------------------------------------------------------------------------------------------------
set @html_geral = '
    <div class="section-title" style="margin-top: 20px;">    
        <p style="display: inline;">Data: '+isnull(dbo.fn_data_string(@dt_hoje),'')+'</p>     
        <p style="display: inline; text-align: center; padding: 35%;"> '+isnull(+@titulo,'')+' </p>    
    </div>
	<div>
    <table>  
		<tr class="tamanho">
...
WHILE EXISTS (SELECT TOP 1 cd_controle FROM #EntregaPhCloro)
BEGIN
    SELECT TOP 1
        @id                          = cd_controle,
        ...
        '<tr class="tamanho">
			...
			<td>' + ISNULL(nm_base,'')+'</td>
			<td>' + cast(ISNULL(dbo.fn_formata_valor(qt_cloro),0) as varchar(20))+'</td>
			<td>' + cast(ISNULL(dbo.fn_formata_valor(qt_ph_agua),0) as varchar(20))+'</td>
            <td>' + cast(isnull(dbo.fn_formata_valor(qt_km),0) as varchar(20))+'</td>
         </tr>'
	FROM #EntregaPhCloro
    DELETE FROM #EntregaPhCloro WHERE cd_controle = @id
END
--------------------------------------------------------------------------------------------------------------------
set @html_rodape =
    '<tr class="tamanhoTotal">
		    <td>Total</td>
			<td>'+cast(isnull(dbo.fn_formata_valor(@vl_romaneio),0) as varchar(20))+'</td>
			<td>'+cast(isnull(@qt_cliente,0) as varchar(20))+'</td>
			<td></td>
			<td>'+cast(isnull(@qt_produto,0) as varchar(20))+'</td>
			<td>'+cast(isnull(@qt_servico,0) as varchar(20))+'</td>
			<td>'+cast(isnull(@qt_veiculo,0) as varchar(20))+'</td>
			<td>'+cast(isnull(@qt_entregador,0) as varchar(20))+'</td>
			<td>'+cast(isnull(@qt_base,0) as varchar(20))+'</td>
			<td>' + cast(ISNULL(dbo.fn_formata_valor(@qt_cloro),0) as varchar(20))+'</td>
			<td>' + cast(ISNULL(dbo.fn_formata_valor(@qt_ph_agua),0) as varchar(20))+'</td>
			<td>'+cast(isnull(dbo.fn_formata_valor(@qt_km),0) as varchar(20))+'</td>
         </tr>
...
------------------------------------------------------------------------------
--exec pr_egis_relatorio_entrega_ph_cloro 345,0,''
------------------------------------------------------------------------------
