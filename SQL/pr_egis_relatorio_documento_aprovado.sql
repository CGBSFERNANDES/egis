IF EXISTS (SELECT name 
	   FROM   sysobjects 
	   WHERE  name = N'pr_egis_relatorio_documento_aprovado' 
	   AND 	  type = 'P')
    DROP PROCEDURE pr_egis_relatorio_documento_aprovado

GO

-------------------------------------------------------------------------------
--sp_helptext pr_egis_relatorio_documento_aprovado
-------------------------------------------------------------------------------
--pr_egis_relatorio_documento_aprovado
-------------------------------------------------------------------------------
...
--
------------------------------------------------------------------------------
create procedure pr_egis_relatorio_documento_aprovado
@cd_relatorio int   = 0,
@cd_parametro int   = 0, 
@json nvarchar(max) = '' 
...
from
  ...
  and 
  a.dt_aprovacao_movimento is not null
  and
...
-------------------------------------------------------------------------------------------------------------------
 ...
 select 
	@qt_documento = count(cd_documento)
 from 
    #DocumentoAprovado
--------------------------------------------------------------------------------------------------------------
set @html_geral = '
 <div class="section-title">  
        ...
        <p style="display: inline; text-align: center; padding: 20%;">Documentos Aprovados</p>  
    </div> 
	<table>
...
while exists ( select top 1 cd_controle from #DocumentoAprovado)
begin
	select top 1
	    ...
	    @html_geral  = @html_geral + '
		                          <tr class="tamanho">
									<td>'+cast(isnull(cd_movimento,0)as varchar)+'</td>
									...
									<td>'+isnull(nm_tipo_aprovacao,'')+'</td>
									<td>'+isnull(nm_usuario,'')+'</td>
									<td>'+isnull(dbo.fn_data_string(dt_aprovacao_movimento),'')+' '+case when isnull(hr_aprovacao_movimento,'') = '' then '' else isnull(hr_aprovacao_movimento,'') end+'</td>
									<td>'+isnull(nm_departamento,'')+'</td>
								  </tr>'

     from #DocumentoAprovado
	 delete from #DocumentoAprovado where cd_controle = @id
 end
...
set @html_rodape =
    '</table>
    <div class="section-title">  
	  <p>Total Aprovados: <td>'+cast(isnull(@qt_documento,0)as varchar)+'</td></p>
	</div>
...
------------------------------------------------------------------------------
--exec pr_egis_relatorio_documento_aprovado 374,0,''
------------------------------------------------------------------------------
