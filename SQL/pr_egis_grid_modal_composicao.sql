...
IF EXISTS (SELECT name 
	   FROM   sysobjects 
	   WHERE  name = N'pr_egis_grid_modal_composicao' 
	   AND 	  type = 'P')
    DROP PROCEDURE pr_egis_grid_modal_composicao

GO

-------------------------------------------------------------------------------
--sp_helptext pr_egis_grid_modal_composicao
-------------------------------------------------------------------------------
--pr_egis_grid_modal_composicao
-------------------------------------------------------------------------------
...
--
------------------------------------------------------------------------------
create procedure pr_egis_grid_modal_composicao
@json nvarchar(max) = ''
...
----------------------------------------------------------------------------------------------------------      
 ...
 SELECT   
    case when isnull(mc.qt_ordem_atributo,0)>0 then
      mc.qt_ordem_atributo
    else
     isnull(a.nu_ordem,mc.qt_ordem_atributo)
    end                                                     as nu_ordem,
    ...
    isnull(m.ic_arquivo_texto,'N')                              as ic_arquivo_texto,
        ...
        case when isnull(mc.ic_contador_grid,'N') = 'S' then
            mc.ic_contador_grid
        else
     	    isnull(a.ic_contador_grid,'N')
        end                                 as ic_contador_grid,
        case when isnull(mc.ic_total_grid,'N') = 'S' then
           mc.ic_total_grid
        else
     	isnull(a.ic_total_grid,'N')  
        end                                 as ic_total_grid,
       ...
       case when isnull(mc.nm_datatype,'') <> '' then
          mc.nm_datatype
       else
       ...
       case when isnull(mc.ic_retorno,'N')='S' then
         mc.ic_retorno
       else
	   	   isnull(a.ic_retorno_atributo,'N') 
       end                                              as ic_retorno_atributo,
       ...
       isnull(mc.ic_calculado,'N')                                                as ic_calculado,
       isnull(mc.nm_formula_calculo,'')                                           as nm_formula_calculo,
       isnull(mc.cd_regra,0)                                                      as cd_regra_validacao,
      ...
      --select * from egisadmin.dbo.menu_tabsheet
       ...
       FROM
        ...
        inner join      egisadmin.dbo.modal_grid_composicao mc   on mc.cd_modal = m.cd_modal
...
------------------------------------------------------------------------------
--exec pr_egis_grid_modal_composicao 
------------------------------------------------------------------------------
--exec pr_egis_grid_modal_composicao '[{"cd_parametro":1,"cd_modal":"10","cd_usuario":"113","dt_inicial":"2025-05-01 00:00:00","dt_final":"2025-05-31 00:00:00"}]'
------------------------------------------------------------------------------
