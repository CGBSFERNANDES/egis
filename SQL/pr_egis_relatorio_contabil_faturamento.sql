IF OBJECT_ID('dbo.pr_egis_relatorio_contabil_faturamento', 'P') IS NOT NULL
    DROP PROCEDURE dbo.pr_egis_relatorio_contabil_faturamento;
GO

/*-------------------------------------------------------------------------------------------------
  pr_egis_relatorio_contabil_faturamento
---------------------------------------------------------------------------------------------------
  GBS Global Business Solution Ltda                                                     2026-01-05
---------------------------------------------------------------------------------------------------
  Stored Procedure : Microsoft SQL Server 2016
  Autor(es)        : Codex (assistente)
  Banco de Dados   : Egissql - Banco do Cliente
  Objetivo         : Relatório HTML - Contabilização do Faturamento (cd_relatorio = 413)

  Requisitos:
    - Apenas 1 parâmetro de entrada (@json)
    - SET NOCOUNT ON e TRY...CATCH com THROW
    - Sem cursor e focado em performance
    - Utiliza a procedure pr_contabiliza_nota_saida (analítica ou sintética)
...
BEGIN
    ...
    BEGIN TRY
        ...
        DECLARE
            @titulo              VARCHAR(200)  = 'Contabilização do Faturamento',
        ...
        /*-----------------------------------------------------------------------------------------
          5) Carrega a base a partir da pr_contabiliza_nota_saida
        -----------------------------------------------------------------------------------------*/
        IF @ic_parametro = 1
        BEGIN
            CREATE TABLE #FaturamentoAnalitico (
            ...
            INSERT INTO #FaturamentoAnalitico
            EXEC pr_contabiliza_nota_saida
                @ic_parametro = 1,
            ...
            SET @html_rows = (
                SELECT
                    ...
                    '</tr>'
                FROM #FaturamentoAnalitico AS f
        ...
        -----------------------------------------------------------------------------------------*/
        CREATE TABLE #FaturamentoSintetico (
            Nome_Lancamento_Padrao    VARCHAR(100),
            Mascara_Lancamento_Padrao VARCHAR(50),
            Reduzido_Lancamento_Padrao VARCHAR(20),
        ...
        INSERT INTO #FaturamentoSintetico
        EXEC pr_contabiliza_nota_saida
            @ic_parametro = 2,
            @dt_inicial   = @dt_inicial,
            @dt_final     = @dt_final_limit;
        ...
        SET @html_rows_s = (
            SELECT
                ...
                '</tr>'
            FROM #FaturamentoSintetico AS s
        ...
        SET @html_header_s =
            ...
            '      <h3>Contabilização do Faturamento - Sintético</h3>' +
            '      <p><strong>' + @nm_fantasia_empresa + '</strong></p>' +
            '      <p>' + @nm_endereco_empresa + ', ' + @cd_numero_endereco + ' - ' + @cd_cep_empresa + ' - ' + @nm_cidade + ' - ' + @sg_estado + ' - ' + @nm_pais + '</p>' +
            '      <p><strong>Fone: </strong>' + @cd_telefone_empresa + ' | <strong>Email: </strong>' + @nm_email_internet + '</p>' +
            '      <p><strong>Período: </strong>' + CONVERT(CHAR(10), @dt_inicial, 103) + ' a ' + CONVERT(CHAR(10), @dt_final, 103) + '</p>' +
