IF EXISTS (SELECT name 
	   FROM   sysobjects 
	   WHERE  name = N'pr_relatorio_pedido_cliente' 
	   AND 	  type = 'P')
    DROP PROCEDURE pr_relatorio_pedido_cliente

GO
  
-------------------------------------------------------------------------------  
--sp_helptext pr_relatorio_pedido_cliente  
-------------------------------------------------------------------------------  
--pr_relatorio_pedido_cliente  
-------------------------------------------------------------------------------  
...
--  
------------------------------------------------------------------------------  
create procedure pr_relatorio_pedido_cliente  
@cd_relatorio int   = 0,   
@cd_parametro int   = 0,  
@json nvarchar(max) = ''   
...
declare @cd_grupo_cliente       int = 0
...
-------------------------------------------------------------------------------------------------  
 ...
 -- select @cd_vendedor            = valor from #json where campo = 'cd_vendedor'   
  ...
  select @cd_grupo_cliente       = valor from #json where campo = 'cd_grupo_cliente' 
...
--  @cd_vendedor      = isnull(cd_vendedor,0),  
  ...
  --@nm_razao_social  = '%' + isnull(nm_razao_social,'') + '%',
  @cd_grupo_cliente	= isnull(cd_grupo_cliente,0),
  @cd_tipo_pedido	= isnull(cd_tipo_pedido,0),
  @ic_faturado	    = isnull(ic_faturado,0),
...
where  
  ...
  and  
  p.cd_cliente = case when isnull(@cd_cliente,0) = 0 then p.cd_cliente else isnull(@cd_cliente,0) end  
 -- c.cd_cliente_grupo = case when @cd_cliente_grupo = 0 then c.cd_cliente_grupo else @cd_cliente_grupo end  
  and  
  p.cd_vendedor = case when isnull(@cd_vendedor,0) = 0 then p.cd_vendedor else isnull(@cd_vendedor,0) end  
  and  
  cg.cd_cliente_grupo = case when isnull(@cd_grupo_cliente,0) = 0 then cg.cd_cliente_grupo else isnull(@cd_grupo_cliente,0) end
  and
  ...
  and
  n.ic_faturado = case when isnull(@ic_faturado,0) = 0 then n.ic_faturado else isnull(@ic_faturado,0) end
  and
  ...
  --and  
  --c.nm_razao_social_cliente like @nm_razao_social  
...
--declare @ic_faturado                 nvarchar(10)
...
--------------------------------------------------------------------------------------------------------------  
set @html_geral = '<div>  
  <p class="section-title" style="text-align: center; padding: 8px;">Pedidos Por Clientes de '+cast(isnull(dbo.fn_data_string(@dt_inicial),'')as nvarchar(20))+' á '+cast(isnull(dbo.fn_data_string(@dt_final),'')as nvarchar(20))+'</p>  
 </div>  
 <div>  
    <table>  
      <tr>  
...
--exec pr_relatorio_pedido_cliente 252,0,''
--exec pr_egis_relatorio_padrao
--'[{
--    "cd_menu": "0",
--    "cd_form": 170,
--    "cd_documento_form": 30,
--    "cd_parametro_form": "2",
--    "cd_usuario": "4439",
--    "cd_cliente_form": "0",
...
--    "lookup_formEspecial": {},
--    "cd_parametro_relatorio": "30",
--    "cd_relatorio": "252",
--    "dt_inicial": "2025-01-01",
--    "dt_final": "2025-04-30",
--    "nm_razao_social": null,
--    "cd_cliente": null,
--    "cd_grupo_cliente": null,
--    "cd_tipo_pedido": null,
--    "ic_faturado": null,
