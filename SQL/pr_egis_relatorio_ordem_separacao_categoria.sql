...
-------------------------------------------------------------------------------
--sp_helptext pr_egis_relatorio_ordem_separacao_categoria
-------------------------------------------------------------------------------
...
--
------------------------------------------------------------------------------
create procedure pr_egis_relatorio_ordem_separacao_categoria
@json nvarchar(max) = '' 
...
-------------------------------------------------------------------------------------------------
set @cd_relatorio = isnull(@cd_relatorio,424)
...
from
	...
	left outer join categoria_produto cp          with (nolock) on cp.cd_categoria_produto     = case 
																									when isnull(pvc.cd_produto,0) <> 0 and isnull(pvi.cd_produto,0) = 0 then prod.cd_categoria_produto 
																									else p.cd_categoria_produto 
																								  end
...
declare @nm_categoria_produto varchar(200) = ''

while exists ( select cd_controle from #CategoriaSeparacao )  
begin  
  ...
  set @html_detalhe = @html_detalhe + '  
           <tr>
            <td colspan="10" style="text-align:left;font-weight:bold;padding-top:10px;">Categoria: '+isnull(nullif(@nm_categoria_produto,''),'SEM CATEGORIA')+'</td>
           </tr>'
...
-------------------------------------------------------------------------------------------------------------------------------------------------------
	...
	begin catch
		declare @ds_erro nvarchar(4000) = error_message()
		declare @cd_erro int = error_number()
		declare @ds_procedure nvarchar(200) = isnull(error_procedure(),'')
		declare @cd_linha int = error_line()

		raiserror('Erro na pr_egis_relatorio_ordem_separacao_categoria (%d:%s:%d): %s',16,1,@cd_erro,@ds_procedure,@cd_linha,@ds_erro)
	end catch
end
