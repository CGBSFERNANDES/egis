IF EXISTS (SELECT name 
	   FROM   sysobjects 
	   WHERE  name = N'pr_egis_relatorio_faturamento_mensal_cliente' 
	   AND 	  type = 'P')
    DROP PROCEDURE pr_egis_relatorio_faturamento_mensal_cliente

GO

-------------------------------------------------------------------------------
--sp_helptext pr_egis_relatorio_faturamento_mensal_cliente
-------------------------------------------------------------------------------
--pr_egis_relatorio_faturamento_mensal_cliente
-------------------------------------------------------------------------------
...
--
------------------------------------------------------------------------------
create procedure pr_egis_relatorio_faturamento_mensal_cliente
@cd_relatorio int   = 0,
@cd_parametro int   = 0, 
@json nvarchar(max) = '' 
...
where
 ...
 and 
 r.cd_cliente = case when ISNULL(@cd_cliente,0) = 0 then r.cd_cliente else ISNULL(@cd_cliente,0) end
 group by 
...
------------------------------------------------------------------------------------------------------------		 
 ...
 begin  
    select * from #faturamentoCliente  
  return  
 end  
...
select 
	...
	@qt_entregador = COUNT(qt_entregador)
...
WHILE EXISTS (SELECT TOP 1 cd_controle FROM #faturamentoCliente)
BEGIN
    SELECT TOP 1
        @id                          = cd_controle,
	...
	FROM #faturamentoCliente
    DELETE FROM #faturamentoCliente WHERE cd_controle = @id
END
--------------------------------------------------------------------------------------------------------------------
...
------------------------------------------------------------------------------
--exec pr_egis_relatorio_faturamento_mensal_cliente 345,0,''
------------------------------------------------------------------------------
