IF OBJECT_ID('dbo.pr_egis_relatorio_contabil_entrada', 'P') IS NOT NULL
    DROP PROCEDURE dbo.pr_egis_relatorio_contabil_entrada;
GO

/*-------------------------------------------------------------------------------------------------
  pr_egis_relatorio_contabil_entrada
---------------------------------------------------------------------------------------------------
  GBS Global Business Solution Ltda                                                     2026-01-06
---------------------------------------------------------------------------------------------------
...
-------------------------------------------------------------------------------------------------*/
CREATE PROCEDURE dbo.pr_egis_relatorio_contabil_entrada
    ...
    BEGIN TRY
        ...
        BEGIN
            ...
            SET @html_rows = (
                SELECT
                    ...
                    '<td style="text-align:right">' + FORMAT(ISNULL(f.ValorIRRF, 0), 'N2') + '</td>' +
                    '<td style="text-align:right">' + FORMAT(ISNULL(f.ValorINSS, 0), 'N2') + '</td>' +
                    '<td style="text-align:right">' + FORMAT(ISNULL(f.ValorISS, 0), 'N2') + '</td>' +
                    '<td style="text-align:right">' + FORMAT(ISNULL(f.vl_pis_nota_entrada, 0), 'N2') + '</td>' +
                    '<td style="text-align:right">' + FORMAT(ISNULL(f.vl_cofins_nota_entrada, 0), 'N2') + '</td>' +
                    '<td style="text-align:right">' + FORMAT(ISNULL(f.vl_csll_nota_entrada, 0), 'N2') + '</td>' +
                    '<td style="text-align:center">' + ISNULL(f.Servico, '') + '</td>' +
                    '</tr>'
            ...
            SET @html_header =
                ...
                '    <tr>' +
                '      <td style=\"width:80px\"><img src=\"' + @logo + '\" alt=\"logo\" style=\"max-height:60px\"></td>' +
                '      <td>' +
                '        <h1>' + ISNULL(@titulo_exibir, '') + '</h1>' +
                '        <div>' + ISNULL(@nm_fantasia_empresa, '') + '</div>' +
                '        <div>' + ISNULL(@nm_endereco_empresa, '') + ', ' + ISNULL(@cd_numero_endereco, '') + ' - ' +
                                     ISNULL(@cd_cep_empresa, '') + ' - ' + ISNULL(@nm_cidade, '') + '/' + ISNULL(@sg_estado, '') + '</div>' +
                '        <div>Fone: ' + ISNULL(@cd_telefone_empresa, '') + ' - Email: ' + ISNULL(@nm_email_internet, '') + '</div>' +
                '      </td>' +
                '      <td style=\"text-align:right; white-space:nowrap\">Emitido em ' + @data_hora_atual + '</td>' +
                '    </tr>' +
                '  </table>' +
        ...
        SET @html_rows_s = (
            SELECT
                ...
                '<td>' + ISNULL(f.Nome_Lancamento_Padrao, '') + '</td>' +
                '<td>' + ISNULL(f.Reduzido_Lancamento_Padrao, '') + '</td>' +
                '<td>' + ISNULL(CAST(f.cd_debito_nota_fiscal AS VARCHAR(20)), '') + '</td>' +
                '<td>' + ISNULL(CAST(f.cd_credito_nota_fiscal AS VARCHAR(20)), '') + '</td>' +
                '<td>' + ISNULL(CAST(f.cd_debito_icms AS VARCHAR(20)), '') + '</td>' +
                '<td>' + ISNULL(CAST(f.cd_credito_icms AS VARCHAR(20)), '') + '</td>' +
                '<td>' + ISNULL(CAST(f.cd_debito_ipi AS VARCHAR(20)), '') + '</td>' +
                '<td>' + ISNULL(CAST(f.cd_credito_ipi AS VARCHAR(20)), '') + '</td>' +
                '<td style=\"text-align:right\">' + FORMAT(ISNULL(f.ValorTotal, 0), 'N2') + '</td>' +
                '<td style=\"text-align:right\">' + FORMAT(ISNULL(f.ValorICMS, 0), 'N2') + '</td>' +
                '<td style=\"text-align:right\">' + FORMAT(ISNULL(f.ValorIPI, 0), 'N2') + '</td>' +
                '<td style=\"text-align:right\">' + FORMAT(ISNULL(f.ValorIRRF, 0), 'N2') + '</td>' +
                ...
                '<td style=\"text-align:right\">' + FORMAT(ISNULL(f.ValorCSLL, 0), 'N2') + '</td>' +
                '<td style=\"text-align:center\">' + ISNULL(f.Servico, '') + '</td>' +
                '</tr>'
