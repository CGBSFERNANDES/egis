IF EXISTS (SELECT name 
	   FROM   sysobjects 
	   WHERE  name = N'pr_relatorio_produto_tabela_preco_bloqueado_cliente' 
	   AND 	  type = 'P')
    DROP PROCEDURE pr_relatorio_produto_tabela_preco_bloqueado_cliente

GO

-------------------------------------------------------------------------------
--sp_helptext pr_relatorio_produto_tabela_preco_bloqueado_cliente
-------------------------------------------------------------------------------
--pr_relatorio_produto_tabela_preco_bloqueado_cliente
-------------------------------------------------------------------------------
...
--
------------------------------------------------------------------------------
create procedure pr_relatorio_produto_tabela_preco_bloqueado_cliente
@cd_relatorio int   = 0,
@json nvarchar(max) = '' 
...
select
  ...
  tpp.dt_usuario                              as dt_bloqueio_produto,
...
from
 ...
 where
   ISNULL(tpp.ic_bloqueio_produto_tabela,'N') = 'S'
...
declare @dt_bloqueio_produto   datetime
...
declare @id int = 0
while exists ( select top 1 cd_controle from #bloqueados_cliente)
begin
	select top 1
		@id                         = cd_controle,
		...
		@dt_bloqueio_produto        = dt_bloqueio_produto,
  ...
  set @html_geral = @html_geral +'<tr>
									...
									<td class="tamanho">'+CONVERT(VARCHAR,@dt_bloqueio_produto, 103)+'</td>
	 ...
	 delete from #bloqueados_cliente where cd_controle = @id
 end
--------------------------------------------------------------------------------------------------------------------
...
set @html_rodape =
    ...
    <p>'+@ds_relatorio+'</p>
        ...
        <p>Total de Produtos: '+cast(isnull(@qt_total,0) as nvarchar(10))+'</p>
    </div>
...
------------------------------------------------------------------------------
exec pr_relatorio_produto_tabela_preco_bloqueado_cliente 220,'' 
------------------------------------------------------------------------------
