IF EXISTS (SELECT name 
	   FROM   sysobjects 
	   WHERE  name = N'pr_egis_relatorio_romaneio_embarque_nota' 
	   AND 	  type = 'P')
    DROP PROCEDURE pr_egis_relatorio_romaneio_embarque_nota

GO
  
-------------------------------------------------------------------------------  
--sp_helptext pr_egis_relatorio_romaneio_embarque_nota  
-------------------------------------------------------------------------------  
--pr_egis_relatorio_romaneio_embarque_nota
-------------------------------------------------------------------------------  
...
--  
------------------------------------------------------------------------------  
create procedure pr_egis_relatorio_romaneio_embarque_nota
@cd_relatorio int   = 0,  
@cd_parametro int   = 0,  
@json nvarchar(max) = ''   
...
set @html_geral = ' 
		 ...
		 <p style="text-align: center;font-size: 15px;" > <b>Romaneio de Embarque: '+CAST(ISNULL(@cd_carga_rel,0) AS nvarchar(20))+'</b></p>
...
SET @contadorCidade = @contadorCidade + 1

SET @html_cab_det = @html_cab_det + '
  <div style="display: flex; justify-content: space-between; align-items: flex-start; width: 100%;margin-top:15px;">
    <h4 style="width: 3%;">' + CAST(@sg_estado_agrupamento AS nvarchar(20)) + ' </h4>	
    <h4 style="width: 49%; text-align: left;">' + ISNULL(@nm_cidade_agrupamento,'') + '</h4>
    <h4 style="width: 32%; text-align: left;">Entregas: ' + CAST(ISNULL(@qt_total_agrupamento,'') AS varchar(6)) + '</h4>
  </div>
'
        	  ...
        	  <table style="width: 100%;">
        		<tr class="tamanho" style="display: flex; flex-direction: column;color: #000000;border: 1px solid #000000aa;">
        		   <div style="display: flex; justify-content: space-between; align-items: flex-start; width: 100%;">
        		    <p style="width: 13%;">Código:'+CAST(ISNULL(cd_cliente,0) AS nvarchar(20))+' </p>	
        			<p style="width: 41%;text-align: left;">'+ISNULL(nm_razao_social_cliente,'')+'</p>
        			<p style="width: 30%;text-align: left;"><b>'+ISNULL(nm_fantasia_cliente,'')+'</b></p>
        			<p style="width: 16%;">'+CAST(ISNULL(cd_ddd,'') AS nvarchar(20))+' '+CAST(ISNULL(cd_telefone,'') AS nvarchar(20))+'</td> 
        		   </div>
        			<div style="display: flex; justify-content: space-between; align-items: flex-start; width: 100%;">
        		       <p style="width: 11%;">'+CAST(ISNULL(cd_identificacao_nota_saida,0) AS nvarchar(20))+' </p>
        			   <p style="width: 33%;">'+CAST(ISNULL(nm_serie_nota_fiscal,'') AS nvarchar(20))+' </p>
                       <p style="width: 23%;">'+ISNULL(nm_condicao_pagamento,'')+' </p>
        			   <p style="width: 33%;">'+CAST(ISNULL(nm_forma_pagamento,'') AS nvarchar(50))+' </p>
        			</div>  
        			...
        			case when ISNULL(sg_estado,'')<>'' and @cd_empresa <> 342 then ' - '+ISNULL(sg_estado,'')+'' else '' end+' '+
        			case when ISNULL(nm_cidade,'') <> '' and @cd_empresa <> 342 then '/ ' +ISNULL(nm_cidade,'')+'' else '' end 
        			+'</b></p>		   
        	</tr>
        		</table>
        		</div>'
        ...
        <tr>  
            <td style="display: flex; flex-direction: column; gap: 20px;">  
                <div style="display: flex; justify-content: space-between; align-items: flex-start; width: 100%;">  
                    ...
                    <div style="text-align: right;">  
                        <p><strong>Peso Líquido:</strong> '+cast(isnull(dbo.fn_formata_valor(@vl_total_geral_liq),0)as nvarchar(20))+' </p>  
                    </div>
					<div style="text-align: right;">  
...
set @html         = 
 ...
 --   @html_titulo ,
	--@html_geral  , 
	--@html_cab_det as html_cab_det,
 --   @html_detalhe,
	--@html_rod_det,
 --   @html_rodape  
-------------------------------------------------------------------------------------------------------------------------------------------------------  
select 'Romaneio_Embarque_'+CAST(isnull(@cd_carga,'')as varchar)+'' AS pdfName,isnull(@html,'') as RelatorioHTML
--select isnull(@html,'') as RelatorioHTML  
-------------------------------------------------------------------------------------------------------------------------------------------------------  
 
  
go
--exec pr_egis_relatorio_romaneio_embarque_nota 325,0,''