IF EXISTS (SELECT name 
	   FROM   sysobjects 
	   WHERE  name = N'pr_egis_relatorio_mapa_orcamento_compras' 
	   AND 	  type = 'P')
    DROP PROCEDURE pr_egis_relatorio_mapa_orcamento_compras

GO
  
-------------------------------------------------------------------------------  
--sp_helptext pr_egis_relatorio_mapa_orcamento_compras  
-------------------------------------------------------------------------------  
--pr_egis_relatorio_mapa_orcamento_compras  
-------------------------------------------------------------------------------  
...
--  
------------------------------------------------------------------------------  
create procedure pr_egis_relatorio_mapa_orcamento_compras  
@cd_relatorio int   = 0,  
@cd_parametro int   = 0,  
@json nvarchar(max) = ''   
...
select
 ...
 (select top 1 u.nm_usuario 
  from Plano_Financeiro_Responsavel pfr
  left outer join EGISADMIN.dbo.Usuario u  on u.cd_usuario = pfr.cd_usuario_responsavel 
  where pfr.cd_plano_financeiro = pf.cd_plano_financeiro)  as nm_responsavel_conta,
  ...
  isnull(o.vl_previsto_plano_compra,0)-(isnull(o.vl_realizado_plano_compra,0))                    as Saldo,
...
order by
 ...
 begin    
    select * from #RelComprasOrc    
  return    
 end    
 ------------------------------------------------------------------------------------------------------------
 declare @vl_total_plano int = 0 
 select 
	@vl_total_plano = count(Codigo)
 from #RelComprasOrc
--------------------------------------------------------------------------------------------------------------  
set @html_detalhe = '' 
set @html_cab_det = '  
...
WHILE EXISTS (SELECT TOP 1 cd_controle FROM #RelComprasOrc)  
BEGIN  
    SELECT TOP 1  
        @id                          = cd_controle,  
      ...
      @html_detalhe = @html_detalhe + 
        '<tr>  
		   ...
		   <td class="tamanho">'+cast(ISNULL(dbo.fn_formata_valor(Limite),0)as nvarchar(20))+'</td>  
		   <td class="tamanho">'+cast(ISNULL(dbo.fn_formata_valor(Previsto),0)as nvarchar(20))+'</td> 
		   <td class="tamanho">'+cast(ISNULL(dbo.fn_formata_valor(Realizado),0)as nvarchar(20))+'</td> 
		   <td class="tamanho">'+cast(ISNULL(dbo.fn_formata_valor(Saldo),0)as nvarchar(20))+'</td> 
		   <td class="tamanho">'+cast(ISNULL(dbo.fn_formata_valor(PercRealizado),0)as nvarchar(20))+'</td> 
        </tr>'  
  from #RelComprasOrc  
    DELETE FROM #RelComprasOrc WHERE cd_controle = @id  
END  
--------------------------------------------------------------------------------------------------------------------  
...
set @html_rodape ='  
 ...
 <div class="section-title">  
     <p>Total de Planos de Compras: '+cast(isnull(@vl_total_plano,0)as nvarchar(10))+'</p>  
    </div>  
...
--exec pr_egis_relatorio_mapa_orcamento_compras 267,0,''
