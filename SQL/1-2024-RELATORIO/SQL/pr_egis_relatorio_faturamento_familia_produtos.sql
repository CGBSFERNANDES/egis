IF EXISTS (SELECT name 
	   FROM   sysobjects 
	   WHERE  name = N'pr_egis_relatorio_faturamento_familia_produtos' 
	   AND 	  type = 'P')
    DROP PROCEDURE pr_egis_relatorio_faturamento_familia_produtos

GO

-------------------------------------------------------------------------------
--sp_helptext pr_egis_relatorio_faturamento_familia_produtos
-------------------------------------------------------------------------------
--pr_egis_relatorio_faturamento_familia_produtos
-------------------------------------------------------------------------------
...
--
------------------------------------------------------------------------------
create procedure pr_egis_relatorio_faturamento_familia_produtos
@cd_relatorio int   = 0,
@cd_usuario   int   = 0,
@json nvarchar(max) = '' 
...
from
  ...
  inner join Vendedor v                     with(nolock) on v.cd_vendedor                = d.cd_vendedor
...
where
  d.cd_vendedor = case when isnull(@cd_vendedor,0) = 0 then d.cd_vendedor else isnull(@cd_vendedor,0) end
  and
...
from
  ...
  inner join Vendedor v                     with(nolock) on v.cd_vendedor                = n.cd_vendedor
  left outer join operacao_fiscal opf       with(nolock) on opf.cd_operacao_fiscal       = i.cd_operacao_fiscal
  left outer join grupo_operacao_fiscal gof with(nolock) on gof.cd_grupo_operacao_fiscal = opf.cd_grupo_operacao_fiscal
  inner join Unidade_Medida umc                          on umc.cd_unidade_medida        = i.cd_unidade_medida
  inner join Unidade_Medida um                           on um.cd_unidade_medida         = case when isnull(fp.cd_unidade_medida,0)>0 
	                                                                                       then 
...
order by
 ...
 --select m.* from  Vendedor_Meta_Familia_Produto m
 --where
 --@dt_inicial between m.dt_inicio_meta_vendedor and m.dt_inicio_meta_vendedor
	--										          and
	--										          @dt_final   between m.dt_final_meta_vendedor and m.dt_final_meta_vendedor
 --and
 --m.cd_vendedor = 5
--select * from Vendedor_Meta_Familia_Produto where cd_vendedor = 4
...
from  
  ...
  left outer join Meta_Vendedor_Periodo mvp on mvp.cd_vendedor = v.cd_vendedor and
                                               @dt_inicial between mvp.dt_inicial_validade_meta and mvp.dt_final_validade_meta
											   and
...
begin
 ...
 --     0                             as qt_nota_devolucao
	...
	set @vl_premiacao     = @vl_premiacao 
							...
							+
							--nao considerei a devolucao este mês
							--30.09.2024 
...
set @html_totais = '<div class="section-title">'+@titulo_total+'
                    ...
                    <tr>										
			        <p style="border: none;color: white;font-size:18px; text-align:left;"><b>Quantidade Total: '+cast(isnull(@qt_total_vendedor,0) as varchar(10))+'</p>
			        <p style="border: none;color: white;font-size:18px; text-align:left;"><b>Valor Total: '+'R$ '+dbo.fn_formata_valor(isnull(@vl_total_vendedor,0))+'</p>	
					</tr>
					</table>					
					</div>'
...
set @html_totais = '<div class="section-title"><strong>'+@titulo_total+'</strong>
					...
					<table style="border: none;">
                    <tr>										
			        <p style="border: none;color: white;font-size:18px; text-align:left;"><b>Notas: '+cast(isnull(@qt_total,0) as varchar(15))+'</p>
			        <p style="border: none;color: white;font-size:18px; text-align:left;"><b>Valor: '+'R$ '+dbo.fn_formata_valor(isnull(@vl_total,0))+'</p>	
					</tr>
					</table>
					</div>
					</div>'
...
------------------------------------------------------------------------------
--exec pr_egis_relatorio_faturamento_familia_produtos 61,4254,''
------------------------------------------------------------------------------
