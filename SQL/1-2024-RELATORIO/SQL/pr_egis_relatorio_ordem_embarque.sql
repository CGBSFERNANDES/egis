IF EXISTS (SELECT name 
	   FROM   sysobjects 
	   WHERE  name = N'pr_egis_relatorio_ordem_embarque' 
	   AND 	  type = 'P')
    DROP PROCEDURE pr_egis_relatorio_ordem_embarque
...
--  
------------------------------------------------------------------------------  
create procedure pr_egis_relatorio_ordem_embarque 
@cd_relatorio int   = 0,  
@cd_parametro int   = 0,  
@json nvarchar(max) = ''   
...
where    
  ...
  --Comentado para poder fazer a reimpressão quando ja faturado
  --and      
...
where  
  cd_carga = @cd_carga  
  and
  r.cd_pedido_venda in (select pvi.cd_pedido_venda from Pedido_Venda_Item pvi
                          where pvi.cd_pedido_venda = r.cd_pedido_venda and pvi.dt_cancelamento_item is null)
...
set @html_geral = '   
  <div style="display: flex; justify-content: space-between;">  
   ...
   <p style="text-align: center;font-size: 16px;" > <b>Ordem de Embarque '+CAST(ISNULL(@cd_carga_rel,0) AS nvarchar(20))+'</b></p>  
...
set @html_cab_det = @html_cab_det + '<h3 class="titulo"><b> Tipo de Pedido: '+ case when isnull(@id_ped_cab,0) >  0 then ''+ISNULL(@nm_tipo_agrupamento,'')+'' else '' end +'</b></h3>  
             <div>  
              <div class="linha">  
...
begin
  ...
  -- Gera rodapé do grupo de produto (total por produto)
  if @vl_total_item > 0
  begin
    set @html_cab_det = @html_cab_det + '  
    </tr>  
...
--exec pr_egis_relatorio_ordem_embarque 284,0,''