IF EXISTS (SELECT name 
	   FROM   sysobjects 
	   WHERE  name = N'pr_egis_relatorio_veloe_consumo_anual' 
	   AND 	  type = 'P')
    DROP PROCEDURE pr_egis_relatorio_veloe_consumo_anual
...
--  
------------------------------------------------------------------------------  
create procedure pr_egis_relatorio_veloe_consumo_anual  
@cd_relatorio int   = 0,  
@cd_parametro int   = 0,  
@json nvarchar(max) = ''   
...
WHILE EXISTS (SELECT TOP 1 cd_controle FROM #AnualVeloeRel)  
BEGIN  
    SELECT TOP 1  
        @id                          = cd_controle,  
	...
	FROM
	  #AnualVeloeRel  
    DELETE FROM #AnualVeloeRel WHERE cd_controle = @id  
END  
--------------------------------------------------------------------------------------------------------------------  
...
--exec pr_egis_relatorio_veloe_consumo_anual 305,0,''