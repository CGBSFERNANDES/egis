IF EXISTS (SELECT name 
	   FROM   sysobjects 
	   WHERE  name = N'pr_egis_relatorio_ordem_servico_garantia' 
	   AND 	  type = 'P')
    DROP PROCEDURE pr_egis_relatorio_ordem_servico_garantia

GO

-------------------------------------------------------------------------------
--sp_helptext pr_egis_relatorio_ordem_servico_garantia
-------------------------------------------------------------------------------
...
--
------------------------------------------------------------------------------
create procedure pr_egis_relatorio_ordem_servico_garantia
@cd_relatorio int   = 0,
@json nvarchar(max) = '' 
...
select 
  ...
  dbo.fn_formata_valor(oso.vl_final_ordem_servico)                 as vl_final_ordem_servico,
  --Usuário
...
from
  ...
  left outer join forma_pagamento fp           on fp.cd_forma_pagamento    = case when isnull(oso.cd_forma_pagamento,0) = 0 then os.cd_forma_pagamento
	...
	where
	 ...
	 --and
	 DATEADD(DAY, 180, oso.dt_pagamento) >= @dt_hoje
	 --and
...
--select * from #ordemServicoGarantia return
--------------------------------------------------------------------------------------------------------------  
 if isnull(@cd_parametro,0) = 1  
 begin  
    select * from #ordemServicoGarantia  
  return  
 end  
--------------------------------------------------------------------------------------------------------------
...
--------------------------------------------------------------------------------------------------------------
declare @id int = 0
while exists ( select top 1 cd_controle from #ordemServicoGarantia)
begin
	select top 1
		@id                         = cd_controle,
		...
		@html_geral = @html_geral +'<tr>
						...
						<td class="tamanho">'+isnull(garantia,'')+'</td>
						<td class="tamanho">'+isnull(dt_entrega,'')+'</td>
						<td class="tamanho">'+cast(isnull(vl_final_ordem_servico,0) as nvarchar(10))+'</td>
						<td class="tamanho">'+isnull(nm_forma_pagamento,'')+'</td>
						<td class="tamanho">'+isnull(nm_tecnico,'')+'</td>
					  </tr>'
     from #ordemServicoGarantia
	 delete from #ordemServicoGarantia where cd_controle = @id
 end
--------------------------------------------------------------------------------------------------------------------
...
set @html_rodape =
   ' <tr style="font-weight: bold;font-size: 20px;">
	 ...
	 <td class="tamanho"></td>
	 <td class="tamanho">'+cast(isnull(@qt_ordem_os,0) as nvarchar(20))+'</td>
	 <td class="tamanho"></td>
	 <td class="tamanho"></td>
	 <td class="tamanho"></td>
	 <td class="tamanho"></td>
...
------------------------------------------------------------------------------
--exec pr_egis_relatorio_ordem_servico_garantia 333,'' 
------------------------------------------------------------------------------
