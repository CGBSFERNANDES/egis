IF EXISTS (SELECT name 
	   FROM   sysobjects 
	   WHERE  name = N'pr_egis_relatorio_analise_trocas' 
	   AND 	  type = 'P')
    DROP PROCEDURE pr_egis_relatorio_analise_trocas

GO

-------------------------------------------------------------------------------
--sp_helptext pr_egis_relatorio_analise_trocas
-------------------------------------------------------------------------------
--pr_egis_relatorio_analise_trocas
-------------------------------------------------------------------------------
...
--
------------------------------------------------------------------------------
create procedure pr_egis_relatorio_analise_trocas
@cd_relatorio int   = 0,
@cd_usuario   int   = 0,
@cd_parametro int   = 0,
@json nvarchar(max) = '' 
...
select
  count(pv.cd_pedido_venda)                as qt_pedido_analise,
  sum(isnull(pv.vl_total_pedido_ipi,0))    as vl_total_analise
...
where
  ...
  and
  (isnull(tp.ic_avaria_tipo_pedido,'N')      = 'S'  OR
  --isnull(tp.ic_amostra_tipo_pedido,'N')      = 'S'  OR
    --OR
...
order by a.dt_agenda desc
  ...
  --select @cd_relatorio, @dt_inicial

--select * from #Entrada
--select * from parametro_relatorio

select
  @vl_total_analise = SUM( isnull(vl_total_analise,0))
from
  #Analise
...
set @pc_meta = case when @vl_total_analise>0 then round(@vl_total / @vl_total_analise * 100,2) else 0.00 end
...
begin
  ...
  --Dados da Tabela---
   ...
   set @tabela = '<div class="container mt-5">
    <h2 class="mb-4">'+ 'Trocas, Indenizações e Amostras'+'</h2>
...
<div class="secao4" id="sobre">
  <div class="secao4-div">
      ...
      <div class="secao4-div-card">   
          ...
          <p> '+'R$ '+dbo.fn_formata_valor(@vl_total_analise)+'</p>
      </div>
      ...
      <div class="secao4-div-card">          
          ...
          <p>'+cast( cast(round(@qt_cliente/@qt_base_cliente*100,2) as int ) as varchar(20))+'</p>
      </div>
...
set @html_titulo = '<div class="section-title"><strong>Trocas e Indenizações</strong></div>
...
set @html_totais = '<div class="section-title"><strong>'+@titulo_total+'</strong>
			        ...
			        <p style="border: none;color: white;font-size:18px; text-align:left;"><b>Pedidos: '+cast(@qt_total as varchar(10))+'</p>
...
--delete from select * from parametro_relatorio 
--select * from parametro_relatorio
------------------------------------------------------------------------------
...
------------------------------------------------------------------------------
--Gráfico
--exec pr_egis_relatorio_analise_trocas 196,4253,0,''
------------------------------------------------------------------------------
