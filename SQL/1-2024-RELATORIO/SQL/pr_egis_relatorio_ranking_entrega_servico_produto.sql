IF EXISTS (SELECT name 
	   FROM   sysobjects 
	   WHERE  name = N'pr_egis_relatorio_ranking_entrega_servico_produto' 
	   AND 	  type = 'P')
    DROP PROCEDURE pr_egis_relatorio_ranking_entrega_servico_produto

GO

-------------------------------------------------------------------------------
--sp_helptext pr_egis_relatorio_ranking_entrega_servico_produto
-------------------------------------------------------------------------------
--pr_egis_relatorio_ranking_entrega_servico_produto
-------------------------------------------------------------------------------
...
--
------------------------------------------------------------------------------
create procedure pr_egis_relatorio_ranking_entrega_servico_produto
@cd_relatorio int   = 0,
@cd_parametro int   = 0, 
@json nvarchar(max) = '' 
...
where
 ...
 --and 
 --p.cd_produto = case when ISNULL(@cd_produto,0) = 0 then p.cd_produto else ISNULL(@cd_produto,0) end
 --and
 --s.cd_servico = case when ISNULL(@cd_servico,0) = 0 then s.cd_servico else ISNULL(@cd_servico,0) end
 group by 
 p.nm_produto,
 s.nm_servico
...
------------------------------------------------------------------------------------------------------------		 
 ...
 begin  
    select * from #rankingProdutoServico  
  return  
 end  
...
WHILE EXISTS (SELECT TOP 1 cd_controle FROM #rankingProdutoServico)
BEGIN
    SELECT TOP 1
        @id                          = cd_controle,
	...
	FROM #rankingProdutoServico
    DELETE FROM #rankingProdutoServico WHERE cd_controle = @id
END
--------------------------------------------------------------------------------------------------------------------
...
------------------------------------------------------------------------------
--exec pr_egis_relatorio_ranking_entrega_servico_produto 345,0,''
------------------------------------------------------------------------------
