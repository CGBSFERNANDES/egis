IF EXISTS (SELECT name 
	   FROM   sysobjects 
	   WHERE  name = N'pr_egis_relatorio_diario_ordem_separacao' 
	   AND 	  type = 'P')
    DROP PROCEDURE pr_egis_relatorio_diario_ordem_separacao

GO

-------------------------------------------------------------------------------
--sp_helptext pr_egis_relatorio_diario_ordem_separacao
-------------------------------------------------------------------------------
--pr_egis_relatorio_diario_ordem_separacao
-------------------------------------------------------------------------------
...
--
------------------------------------------------------------------------------
create procedure pr_egis_relatorio_diario_ordem_separacao
@cd_relatorio int   = 0,
@cd_parametro int   = 0,
@json nvarchar(max) = '' 
...
--  dbo.fn_hora_string(p.hr_inicial_pedido) as hr_inicial_pedido,
  ...
  max(isnull(p.vl_total_pedido_ipi,0))       as vl_total_pedido,
...
select
  --hp = dbo.fn_hora_hhmmss(s.hr_inicial_pedido),
...
from 
  #AuxSeparacao s
  left outer join egisadmin.dbo.usuario u on u.cd_usuario = s.cd_usuario_ordsep
...
---------------------------------------------------------------------------------------------------------------------------
declare @hr_inicial_pedido_tb nvarchar(20)
declare @cd_pedido_venda_tb nvarchar(20)
declare @dt_pedido_venda_tb nvarchar(20)
declare @nm_tipo_pedido_tb nvarchar(20)
declare @cd_cliente_tb nvarchar(20)
declare @nm_fantasia_cliente_tb nvarchar(20)
declare @nm_fantasia_vendedor_tb nvarchar(20)
declare @ic_ordsep_pedido_venda nvarchar(20)
declare @nm_fantasia_empresa_tb nvarchar(20)
declare @nm_fantasia_usuario_tb nvarchar(20)
declare @vl_total_pedido nvarchar(20)
declare @nm_cidade_tb nvarchar(20)
...
WHILE EXISTS (SELECT TOP 1 cd_controle FROM #Separacao )
begin

  select top 1
    @id                      = cd_controle,
	@hr_inicial_pedido_tb    = hr_inicial_pedido,
	@cd_pedido_venda_tb      = cd_pedido_venda,
	@dt_pedido_venda_tb      = dt_pedido_venda,
	@nm_tipo_pedido_tb       = nm_tipo_pedido,
	@cd_cliente_tb           = cd_cliente,
	@nm_fantasia_cliente_tb  = nm_fantasia_cliente,
	@ic_ordsep_pedido_venda  = ic_ordsep_pedido_venda,
	@nm_fantasia_empresa_tb  = nm_fantasia_empresa,
	@nm_fantasia_usuario_tb  = nm_fantasia_usuario,
	@vl_total_pedido         = vl_total_pedido,
	@nm_cidade               = nm_cidade,
	@nm_fantasia_vendedor_tb = nm_fantasia_vendedor,
  ...
  set @html_cab_det = @html_cab_det + '
            <tr> 					
            <td style="font-size:12px; text-align:center;width: 20px">'+isnull(@hr_inicial_pedido_tb,'')+'</td>
			<td style="font-size:12px; text-align:center;width: 20px">'+isnull(cast(@cd_pedido_venda_tb as varchar(9)),'')+'</td>
			<td style="font-size:12px; text-align:center;width: 20px">'+isnull(@dt_pedido_venda_tb,'')+'</td>
			<td style="font-size:12px; text-align:center;width: 20px">'+isnull(cast(@nm_tipo_pedido_tb as varchar(20)),'')+'</td>	
			<td style="font-size:12px; text-align:center;width: 20px">'+isnull(cast(@cd_cliente_tb as varchar(20)),'')+'</td>	
			<td style="font-size:12px; text-align:center;width: 20px">'+isnull(cast(@nm_fantasia_cliente_tb as varchar(30)),'')+'</td>	
			<td style="font-size:12px; text-align:center;width: 20px">'+isnull(cast(@nm_fantasia_vendedor_tb as varchar(30)),'')+'</td>	
			<td style="font-size:12px; text-align:center;width: 20px">'+isnull(cast(@ic_ordsep_pedido_venda as varchar(10)),'')+'</td>	
			<td style="font-size:12px; text-align:center;width: 20px">'+isnull(cast(@nm_fantasia_empresa_tb as varchar(30)),'')+'</td>	
			<td style="font-size:12px; text-align:center;width: 20px">'+isnull(cast(@nm_fantasia_usuario_tb as varchar(30)),'')+'</td>	
			<td style="font-size:12px; text-align:center;width: 20px">'+isnull(dbo.fn_formata_valor(@vl_total_pedido),'')+'</td>	
			<td style="font-size:12px; text-align:center;width: 20px">'+isnull(@nm_cidade,'')+'</td>
			<td style="font-size:12px; text-align:center;width: 20px">'+isnull(@sg_estado_tb,'')+'</td>
            </tr>'
...
set @html_titulo = '<div class="section-title"><strong>'+isnull(@titulo,'')+' de '+cast(isnull(dbo.fn_data_string(@dt_inicial),'')as nvarchar(20))+' á '+cast(isnull(dbo.fn_data_string(@dt_final),'')as nvarchar(20))+'</strong></div>
...
set @html_rodape = '<div class="section-title"><strong>'+@titulo_total+'</strong></div>
                    <div> 
                    <tr>					
...
set @html_rodape =
	...
	<p>Total Pedidos:<strong> '+cast(isnull(@qt_total,0) as nvarchar(15))+'</strong></p>
	<p>Valor Total:<strong> '+cast(isnull(dbo.fn_formata_valor(@vl_total),0) as nvarchar(15))+'</strong></p>
    </div>
...
------------------------------------------------------------------------------
--exec pr_egis_relatorio_diario_ordem_separacao 20,''
------------------------------------------------------------------------------
