IF EXISTS (SELECT name 
	   FROM   sysobjects 
	   WHERE  name = N'pr_egis_relatorio_mapa_entrega_pedido' 
	   AND 	  type = 'P')
    DROP PROCEDURE pr_egis_relatorio_mapa_entrega_pedido

GO
  
-------------------------------------------------------------------------------  
--sp_helptext pr_egis_relatorio_mapa_entrega_pedido  
-------------------------------------------------------------------------------  
--pr_egis_relatorio_mapa_entrega_pedido  
-------------------------------------------------------------------------------  
...
--  
------------------------------------------------------------------------------  
create procedure pr_egis_relatorio_mapa_entrega_pedido  
@cd_relatorio int   = 0,  
@cd_parametro int   = 0,  
@json nvarchar(max) = ''   
...
select 
	@vl_total = SUM(vl_total_item_pedido_comp)
from #RelMapaCompra
--------------------------------------------------------------------------------------------------------------  
set @html_detalhe = '' 
set @html_cab_det = '  
    ...
    SELECT TOP 1  
      ...
      @html_detalhe = @html_detalhe + 
        ...
        '<tr>  
		   ...
		   <td class="tamanho">'+cast(ISNULL(dbo.fn_formata_valor(vl_total_item_pedido_comp),'') as nvarchar(25))+'</td> 
		   <td class="tamanho">'+ISNULL(nm_plano_compra,'')+'</td> 
		   <td class="tamanho">'+ISNULL(nm_centro_custo,'')+'</td> 
		   <td class="tamanho">'+ISNULL(nm_fantasia_comprador,'')+'</td> 
		   <td class="tamanho">'+ISNULL(nm_departamento,'')+'</td> 
		   <td class="tamanho">'+ISNULL(nm_condicao_pagamento,'')+'</td> 
		   <td class="tamanho">'+ISNULL(nm_fantasia_transportadora,'')+'</td>
		   <td class="tamanho">'+ISNULL(nm_tipo_pagamento_frete,'')+'</td>
...
set @html_rodape ='  
 ...
 <div class="section-title">  
     <p>Valor Total: '+cast(ISNULL(dbo.fn_formata_valor(@vl_total),'') as nvarchar(20))+'</p>  
    </div>  
...
--exec pr_egis_relatorio_mapa_entrega_pedido 267,0,''
