IF EXISTS (SELECT name 
	   FROM   sysobjects 
	   WHERE  name = N'pr_egis_relatorio_ordem_embarque_vendedor_periodo' 
	   AND 	  type = 'P')
    DROP PROCEDURE pr_egis_relatorio_ordem_embarque_vendedor_periodo

GO
  
-------------------------------------------------------------------------------  
--sp_helptext pr_egis_relatorio_ordem_embarque_vendedor_periodo  
-------------------------------------------------------------------------------  
--pr_egis_relatorio_ordem_embarque_vendedor_periodo
-------------------------------------------------------------------------------  
...
--  
------------------------------------------------------------------------------  
create procedure pr_egis_relatorio_ordem_embarque_vendedor_periodo 
@cd_relatorio int   = 0,  
@cd_parametro int   = 0,  
@json nvarchar(max) = ''   
...
where    
  r.dt_carga between @dt_inicial and @dt_final  
  and
  ...
  --Comentado para poder fazer a reimpress„o quando ja faturado
  --and      
...
select
  @qt_total_pedido = count(r.cd_pedido_venda)
from
  Pedido_Venda_Romaneio r
  inner join pedido_venda pv		            on pv.cd_pedido_venda      = r.cd_pedido_venda      
...
select
  ...
  cd_categoria_produto As cd_categoria_produto,
  qt_ordem_carga       As qt_ordem_carga,
  nm_tipo_agrupamento  As nm_tipo_agrupamento,
  qt_ordem_carga_produto as qt_ordem_carga_produto,
...
--  select * from #CategoriaRelDetalhe order by cd_controle return
...
set @html_geral = '<div class="section-title">  
        <h3 style="display: inline;">Periodo: '+isnull(dbo.fn_data_string(@dt_inicial),'')+' · '+isnull(dbo.fn_data_string(@dt_final),'')+'</h3>   
        <h3 style="display: inline; text-align: center; padding: 20%;">Ordem de Embarque</h3>  
    </div>  '  
------------------------------------------------------------------------------------------------------------------------------------------------      
...
set @html_cab_det = @html_cab_det + '
			...
			<p style="margin: 0; font-size:18px; text-align: center; flex: 1;"><b> Tipo de Pedido: '+ case when isnull(@id_ped_cab,0) >  0 then ''+ISNULL(@nm_tipo_agrupamento,'')+'' else '' end +'</b></p>  			
			</div>
             <div>  
              <div class="linha">  
              <tr class="tamanho" style="display: flex; flex-direction: column;color: #000000;">  
              <div style="display: flex; justify-content: space-between; align-items: flex-start; width: 100%;">  
              <p class="tamanho" style="width:10%"><b>C”DIGO</b></p>  
              <p class="tamanho" style="width:65%"><b>DESCRI«√O</b></p>  
              <p class="tamanho" style="width:10%"><b>QTD.</b></p>  
              <p class="tamanho" style="width:10%"><b>TOTAL</b></p>  
             </tr>  
            </div>  
           </div>'               
...
while exists( select Top 1 cd_controle from #TipoPedidoRel where @nm_tipo_agrupamento = nm_tipo_agrupamento order by nm_tipo_agrupamento, qt_ordem_carga)  
  begin  
...
begin
  ...
  while exists (select top 1 cd_controle from #CategoriaRelDetalhe where isnull(cd_categoria_produto,0) = isnull(@cd_categoria_produto,0) 
																		 and ISNULL(qt_ordem_carga,0) = @qt_ordem_carga
																		 and nm_tipo_agrupamento = @nm_tipo_agrupamento																		 
																		 order by nm_tipo_agrupamento
      ...
      @html_cab_det   = @html_cab_det  + '  
        <tr class="tamanho" style="display: flex; flex-direction: column;color: #000000;">  
          <div class="nao-quebrar" style="display: flex; justify-content: space-between; align-items: flex-start; width: 100%;">                    
            <p class="tamanho" style="width: 15%;">'+CAST(isnull(cd_produto,'')as nvarchar(20))+'</p>  
            <p class="tamanho" style="text-align: left;width: 65%;">'+isnull(nm_produto,'')+'</p>   
            <p class="tamanho" style="width: 10%;">'+CAST(isnull(qt_produto,'')as nvarchar(20))+' '+isnull(sg_unidade_medida,'')+'</p>  
            <p class="tamanho" style="width: 10%;">'+CAST(isnull(dbo.fn_formata_valor(vl_total),'')as nvarchar(20))+'</p>   
          </div>  
        '
  ...
  -- Gera rodapÈ do grupo de produto (total por produto)
  if @vl_total_item > 0
  begin
    set @html_cab_det = @html_cab_det + '  
    </tr>  
...
--exec pr_egis_relatorio_ordem_embarque_vendedor_periodo 284,0,''