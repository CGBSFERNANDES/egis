IF EXISTS (SELECT name 
	   FROM   sysobjects 
	   WHERE  name = N'pr_egismob_relatorio_pedido_vendedor' 
	   AND 	  type = 'P')
    DROP PROCEDURE pr_egismob_relatorio_pedido_vendedor

GO

  
-------------------------------------------------------------------------------  
--sp_helptext pr_egismob_relatorio_pedido_vendedor  
-------------------------------------------------------------------------------  
--pr_egismob_relatorio_pedido_vendedor  
-------------------------------------------------------------------------------  
...
--Objetivo         : Relatorio Padrao Egis HTML - EgisMob
--Data             : 01.04.2025  
------------------------------------------------------------------------------  
create procedure pr_egismob_relatorio_pedido_vendedor
@cd_relatorio int   = 0,  
@cd_parametro int   = 0,  
@json nvarchar(max) = ''   
...
-------------------------------------------------------------------------------------------------  
  ...
  set @cd_vendedor = dbo.fn_usuario_vendedor(@cd_vendedor)
-------------------------------------------------------------------------------------------------  
...
--  dbo.fn_hora_string(p.hr_inicial_pedido) as hr_inicial_pedido,  
  ...
  MAX(ISNULL(CAST(p.vl_total_pedido_ipi AS DECIMAL(18,2)), 0))                AS vl_total,
...
declare @cd_cliente_tb               int = 0   declare @nm_fantasia_cliente         nvarchar(60)  
...
--------------------------------------------------------------------------------------------------------------  
set @html_geral = '<div>  
  <p class="section-title" style="text-align: center; padding: 8px;">Pedidos Por Vendedor de '+cast(isnull(dbo.fn_data_string(@dt_inicial),'')as nvarchar(20))+' á '+cast(isnull(dbo.fn_data_string(@dt_final),'')as nvarchar(20))+'</p>  
 </div>  
 <div>  
    <table>  
      <tr>  
...
set @html_rodape =  
 ...
 <div class="company-info">  
  <p><strong>'+@footerTitle+'</strong></p>  
   ...
   <div class="section-title">
	<p>Total Pedidos: '+cast(isnull(@vl_total_pedido,0) as nvarchar(15))+'</p>
	<p>Valor Total: '+cast(isnull(dbo.fn_formata_valor(@qt_total),0) as nvarchar(15))+'</p>
 </div>
 ...
 <script>  
     ...
     --<button id="salva">Salvar</button>  
    --<script>    document.querySelector("#salva").addEventListener("click", () => {  
    --  const conteudoHtml = document.querySelector("#conteudo");  
    --  const options = {  
    --    margin: 10,  
    --    filename: "'+isnull(@titulo,'')+'.pdf",  
...
------------------------------------------------------------------------------
--exec pr_egismob_relatorio_pedido_vendedor 232,11,''
------------------------------------------------------------------------------
