IF EXISTS (SELECT name 
	   FROM   sysobjects 
	   WHERE  name = N'pr_egis_relatorio_entregas_efetuadas_faturamento' 
	   AND 	  type = 'P')
    DROP PROCEDURE pr_egis_relatorio_entregas_efetuadas_faturamento

GO

-------------------------------------------------------------------------------
--sp_helptext pr_egis_relatorio_entregas_efetuadas_faturamento
-------------------------------------------------------------------------------
--pr_egis_relatorio_entregas_efetuadas_faturamento
-------------------------------------------------------------------------------
...
--
------------------------------------------------------------------------------
create procedure pr_egis_relatorio_entregas_efetuadas_faturamento
@cd_relatorio int   = 0,
@json nvarchar(max) = '' 
...
--vw.cd_romaneio = 55025
 (isnull(vw.cd_pedido_venda,0) <= 0) 
  ...
  AND 
  vw.hr_saida is not null
  and 
  vw.hr_chegada is not null
  and 
...
vw.dt_entrega
  ...
  --return
-----------------------------------------------------------------------------------------------------------------------------------------
  delete from #EntregaFaturamento where isnull(cd_pedido_venda,0) > 0    
  delete from #EntregaFaturamento where isnull(Entregue,'N') = 'N'    
  delete from #EntregaFaturamento where isnull(hr_saida,'') = ''    
  delete from #EntregaFaturamento where isnull(hr_chegada,'') = ''    
  select     
    *,    
...
------------------------------------------------------------------------------
--exec pr_egis_relatorio_entregas_efetuadas_faturamento 370,'[{
--    "cd_menu": "0",
--    "cd_form": 188,
--    "cd_documento_form": 3,
--    "cd_parametro_form": "2",
