-------------------------------------------------------------------------------          
--sp_helptext pr_egis_relatorio_negociacao_proposta          
-------------------------------------------------------------------------------          
...
---------------------------------------------------------------------------------------------
create or alter procedure pr_egis_relatorio_negociacao_proposta 
@cd_relatorio int = 0, 
@json nvarchar(max) = ''       
...
----Caso das Cisneros
--if @cd_empresa in (329,340,354)
--begin
  
--  EXEC pr_egismob_relatorio_pedido_cisneros @json
...
from                 
  ...
  ----------------------------------------------------------------------------------------------------------           
   ...
   select @cd_consulta               = valor from #json  with(nolock) where campo = 'cd_documento_form'      
   ...
   --empresa_faturamento--
	...
	declare @titulo                     varchar(200),
			...
			@pdfName                    varchar(120) = '',
			@ic_desconto_total_proposta char(1) = '',
			@vl_desconto_consulta       float = 0,
			@vl_desconto_pedido_venda   float = 0,
		    @dt_negociacao_consulta     datetime,
	        @nm_negociacao_consulta     varchar(60),
	        @ds_negociacao_consulta     varchar(8000),
	...
	select 
		...
		@ic_desconto_total_proposta = isnull(ic_desconto_total_proposta,'N')
	from config_egismob
	...
	--Dados da Proposta/Pedido-----------------------------------------------------------
		...
		select 
			...
			@vl_desconto_consulta   = max(isnull(c.vl_desconto_consulta,0)),
	...
	select
	  ...
	  isnull(cc.vl_desconto_consulta,0)                                 as vl_desconto,
	  isnull(pc.nm_produto_cliente,'')                                  as nm_produto_cliente
	...
	from
	  ...
	  left outer join produto_cliente pc with(nolock) on pc.cd_cliente  = c.cd_cliente and pc.cd_produto = p.cd_produto
  ...
  case when isnull(c.nm_produto_cliente,'') <> '' 
			then isnull(c.nm_produto_cliente,'') 
	       when isnull(p.cd_produto,0) > 0 
			then isnull(p.nm_produto,'')
		   when isnull(c.cd_servico,0) > 0 
	  ...
	  isnull(c.vl_desconto,0)										    as vl_desconto,
	  isnull(cn.dt_negociacao_consulta,'')								as dt_negociacao_consulta,
	  isnull(cn.nm_negociacao_consulta,'')								as nm_negociacao_consulta,
	  isnull(cn.ds_negociacao_consulta,'')								as ds_negociacao_consulta,
	  isnull(ui.nm_usuario,'')											as nm_usuario
	...
	from
	  ...
	  left outer join consulta_negociacao cn    with(nolock) on cn.cd_consulta = c.cd_consulta
	  left outer join egisadmin.dbo.usuario ui  with(nolock) on ui.cd_usuario  = cn.cd_usuario
...
select 
	  @dt_negociacao_consulta	=  cn.dt_negociacao_consulta,
	  @nm_negociacao_consulta	=  cn.nm_negociacao_consulta,
	  @ds_negociacao_consulta	=  cn.ds_negociacao_consulta,
	  @nm_usuario				=  ui.nm_usuario

from consulta_negociacao cn
left outer join egisadmin.dbo.usuario ui  with(nolock) on ui.cd_usuario  = cn.cd_usuario
...
select 
	...
	@vl_desconto_total      = (@vl_total_pedido_venda - vl_desconto)
...
SET @html = @html + '
	...
	<div style="display: flex; justify-content: space-between; align-items:start; margin-bottom: 0px; padding-bottom: 0px">
		...
		<div class="company-info" style="width:30%; justify-content:right; align-items:right">'+
			case when isnull(@ic_imposto_tipo_pedido,'N') = 'S' and @cd_empresa <> 342	
			 ...
			 case when isnull(@ic_desconto_total_proposta,'N') = 'S'
			     then '<p><strong>Desconto: </strong>'+cast(isnull(dbo.fn_formata_valor(@vl_desconto),0) as varchar)+'</p>'
				 else ''
			end+

			case when isnull(@ic_desconto_total_proposta ,'N') = 'S'
			     then '<p><strong>'+isnull(@footerTitle,'')+'</strong> '+cast(isnull(dbo.fn_formata_valor(@vl_desconto_total),0)as varchar(20))+ '</p>'
				 else '<p><strong>'+isnull(@footerTitle,'')+'</strong> '+isnull(@vl_final,'')+ '</p>'				  
				 end +
	...
	'	<div style="width: 100%;height: 2px;background: black;"></div>
		<h3 style="text-align: center;">Negociação</h3>
	    <div class="info">
            <p><strong>Data</strong>'+isnull(dbo.fn_data_string(@dt_negociacao_consulta),'')+'</p>
            <p><strong>Usúario</strong>'+isnull(@nm_usuario,'')+'</p>
            <p><strong>Vendedor</strong>'+isnull(@nm_vendedor,'')+'</p>
            <p><strong>Contato</strong>'+isnull(@nm_negociacao_consulta,'')+'</p>
            <p><strong>Descritivo</strong>'+isnull(@ds_negociacao_consulta,'')+'</p>
        </div>
...
select @html as RelatorioHTML, 'Proposta_'+cast(@cd_consulta as varchar) as pdfName


go

--exec pr_egis_relatorio_negociacao_proposta  381,'[{"cd_consulta": 2438, "cd_menu": 0, "cd_parametro": 0, "cd_pedido_venda":0, "cd_usuario": 4645}]'
------------------------------------------------------------------------------------------------------------------------------------------------------------
--Executando
------------------------------------------------------------------------------------------------------------------------------------------------------------
--Relatório do diário de Pedido (Egismob)
--exec pr_modulo_processo_egismob_post '[{"cd_consulta": 8444, "cd_menu": 5428, "cd_parametro": 14, "cd_pedido_venda":0, "cd_usuario": 4645}]'
