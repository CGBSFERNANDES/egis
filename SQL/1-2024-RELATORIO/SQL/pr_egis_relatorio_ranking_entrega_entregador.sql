IF EXISTS (SELECT name 
	   FROM   sysobjects 
	   WHERE  name = N'pr_egis_relatorio_ranking_entrega_entregador' 
	   AND 	  type = 'P')
    DROP PROCEDURE pr_egis_relatorio_ranking_entrega_entregador

GO

-------------------------------------------------------------------------------
--sp_helptext pr_egis_relatorio_ranking_entrega_entregador
-------------------------------------------------------------------------------
--pr_egis_relatorio_ranking_entrega_entregador
-------------------------------------------------------------------------------
...
--
------------------------------------------------------------------------------
create procedure pr_egis_relatorio_ranking_entrega_entregador
@cd_relatorio int   = 0,
@cd_parametro int   = 0, 
@json nvarchar(max) = '' 
...
select
  ...
  count(distinct r.cd_romaneio)            as qt_romaneio,
...
WHILE EXISTS (SELECT TOP 1 cd_controle FROM #RankingEntregador)
BEGIN
    SELECT TOP 1
        @id                          = cd_controle,
	...
	FROM #RankingEntregador
    DELETE FROM #RankingEntregador WHERE cd_controle = @id
END
--------------------------------------------------------------------------------------------------------------------
...
------------------------------------------------------------------------------
--exec pr_egis_relatorio_ranking_entrega_entregador 344,0,''
------------------------------------------------------------------------------
