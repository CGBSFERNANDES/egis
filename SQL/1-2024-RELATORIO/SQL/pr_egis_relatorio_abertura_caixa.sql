IF EXISTS (SELECT name 
	   FROM   sysobjects 
	   WHERE  name = N'pr_egis_relatorio_abertura_caixa' 
	   AND 	  type = 'P')
    DROP PROCEDURE pr_egis_relatorio_abertura_caixa
...
--
------------------------------------------------------------------------------
create procedure pr_egis_relatorio_abertura_caixa
@cd_relatorio int   = 0,
@cd_parametro int   = 0, 
@json nvarchar(max) = '' 
...
--Abertura do caixa---------------------------------------
	...
	select   
       ...
       tc.cd_numero_terminal_caixa,
       ac.cd_abertura_caixa,
       ac.dt_abertura_caixa,
       ac.hr_abertura_caixa,
       ac.dt_ultimo_fechamento,
       oc.nm_operador_caixa,
       oc.cd_operador_caixa,
       ac.cd_movimento_caixa,
	   ac.cd_vendedor,
	   ac.vl_ultimo_fechamento,
	   ac.nm_obs_abertura_caixa,
	   ac.vl_abertura_caixa,
       case when ac.cd_abertura_caixa > 0 
         then 'Em Operação'
         else 'Disponível'
       end                  as 'Status'  
	   into
		#abertura_caixa_rel
     	from
        abertura_caixa ac with (nolock)   
        left outer join terminal_caixa tc with (nolock) on ac.cd_terminal_caixa    = tc.cd_terminal_caixa  
                                                       and ac.dt_ultimo_fechamento is null                                                 
        left outer join operador_caixa oc with (nolock) on oc.cd_operador_caixa    = ac.cd_operador_caixa
     	where
		convert(datetime, left(convert(varchar, ac.dt_abertura_caixa, 121), 10) + ' 00:00:00', 121) between @dt_inicial and @dt_final	
		--and
  --      tc.ic_status_terminal_caixa = 'A' 

		--select * from #abertura_caixa_rel
declare
	@vl_abertura_caixa    float = 0, 
	@vl_ultimo_fechamento float = 0

select 

@vl_abertura_caixa     = sum(vl_abertura_caixa),
@vl_ultimo_fechamento  = sum(vl_ultimo_fechamento)
...
set @html_geral = ' <div class="section-title">  
        ...
        <p style="display: inline; text-align: center; padding: 20%;">Abertura de Caixa</p>  
    </div>
	<table>
...
while exists ( select top 1 cd_controle from #abertura_caixa_rel)
begin
	select top 1
...
------------------------------------------------------------------------------
--exec pr_egis_relatorio_abertura_caixa 374,0,''
------------------------------------------------------------------------------
