IF EXISTS (SELECT name 
	   FROM   sysobjects 
	   WHERE  name = N'pr_egis_relatorio_falta_estoque_produto' 
	   AND 	  type = 'P')
    DROP PROCEDURE pr_egis_relatorio_falta_estoque_produto

GO

-------------------------------------------------------------------------------
--sp_helptext pr_egis_relatorio_falta_estoque_produto
-------------------------------------------------------------------------------
...
--
------------------------------------------------------------------------------
create procedure pr_egis_relatorio_falta_estoque_produto
@cd_relatorio int   = 0,
@json nvarchar(max) = '' 
...
select 
 Distinct
 a.*,
 g.nm_grupo_relatorio into #RelAtributo 
from
...
select 
  isnull(tp.nm_tipo_proposta,'') as nm_tipo_proposta,
...
from
  ...
  left outer join produto_saldo ps  on ps.cd_produto        = i.cd_produto
                                   and ps.cd_fase_produto   = i.cd_fase_produto
  left outer join fase_produto fp   on fp.cd_fase_produto   = i.cd_fase_produto
  ...
  inner join Consulta_Item_Perda cp on cp.cd_consulta       = i.cd_consulta
                                   and cp.cd_item_consulta  = i.cd_item_consulta
where
  c.dt_consulta between @dt_hoje-3 and @dt_hoje
--where
...
--select * from #Consulta order by dt_consulta desc
...
select 
  ...
  c.nm_produto,
  c.sg_unidade_medida,
  sum(c.qt_saida_estoque)                as qt_saida_estoque,
  MAX(c.qt_multiplo_embalagem)           as Embalagem,
  MAX(c.qt_saldo_reserva_produto)        as qt_disponivel,
  MAX(c.nm_fase_produto)                 as nm_fase_produto,
...
while exists ( select top 1 cd_controle from #Ruptura )
begin
  ...
  select 
    ...
    @html_detalhe = @html_detalhe 
	         +
            '<tr>
			<td style="font-size:12px; text-align:center;width: 20px">'+ dbo.fn_data_string(dt_consulta) +'</td>
			...
			<td style="font-size:12px; text-align:center;width: 20px">'+ cast(Embalagem as varchar(20))+'</td>			
			<td style="font-size:12px; text-align:center;width: 20px">'+ cast(qt_disponivel as varchar(20))+'</td>			
			<td style="font-size:12px; text-align:center;width: 20px">'+ nm_fase_produto+'</td>	
			<td style="font-size:12px; text-align:center;width: 20px">'+ cast(qt_saida_estoque as varchar(20))+'</td>			
			<td style="font-size:12px; text-align:center;width: 20px">'+ cast(qt_consulta as varchar(20))+'</td>			
			...
			<td style="font-size:12px; text-align:center;width: 20px">'+ cast(qt_grupo_cliente as varchar(20))+'</td>						
  ...
  order by
    dt_consulta desc, qt_saida_estoque desc
...
set @html_totais = '<div class="section-title"><strong>'+@titulo_total+'</strong></div>
                    <div> 
                    <tr>					
					<td style="font-size:12px; text-align:center;width: 80px;">'+cast(isnull(@qt_total,0) as varchar(10))+'</td>
					<td style="font-size:12px; text-align:center;width: 80px;">'+cast('----' as varchar(10))+'</td>
			        <td style="font-size:12px; text-align:center;width: 80px;">'+'R$ '+dbo.fn_formata_valor(isnull(@vl_total,0))+'</td>
					</tr>
					</div>'
...
------------------------------------------------------------------------------
--exec pr_egis_relatorio_falta_estoque_produto 40,''
------------------------------------------------------------------------------
