IF EXISTS (SELECT name 
	   FROM   sysobjects 
	   WHERE  name = N'pr_relatorio_consulta_fiscal_produto' 
	   AND 	  type = 'P')
    DROP PROCEDURE pr_relatorio_consulta_fiscal_produto
...
--
------------------------------------------------------------------------------
create procedure pr_relatorio_consulta_fiscal_produto
@cd_relatorio int   = 0,
@json nvarchar(max) = '' 
...
select
  ...
  p.cd_codigo_barra_produto                 as ean13,
...
declare @nm_fantasia_produto        nvarchar(30)
declare @sg_unidade_medida          nvarchar(10)
declare @ean13                      nvarchar(50)
declare @qt_multiplo_embalagem      int = 0
...
set @html_geral = '<div style="text-align: center" class="section-title">
			           <strong> Consulta Fiscal por Produto </strong>
				   </div>
				   <table>
					   <tr>
							...
							<td class="tamanho" style="font-size: 110%;"><strong>Ean13</strong></td>
...
declare @id int = 0
while exists ( select top 1 cd_controle from #produto)
begin
	select top 1
		@id                         = cd_controle,
  ...
  set @html_geral = @html_geral +'<tr>
						...
						<td class="tamanho">'+isnull(@nm_produto,0)+' ('+isnull(@nm_fantasia_produto,'')+')</td>
						<td class="tamanho">'+isnull(@sg_unidade_medida,'')+'</td>
						<td class="tamanho">'+isnull(@ean13,'')+'</td>
						<td class="tamanho">'+cast(isnull(@qt_multiplo_embalagem,'') as nvarchar(10))+'</td>
						...
						<td class="tamanho">'+isnull(@ic_base_reduzida,'')+'</td>
						<td class="tamanho">'+isnull(@ic_subst_tributaria,'')+'</td>
	 ...
	 delete from #produto where cd_controle = @id
 end
--------------------------------------------------------------------------------------------------------------------
...
------------------------------------------------------------------------------
--exec pr_relatorio_consulta_fiscal_produto 218,'' 
------------------------------------------------------------------------------
