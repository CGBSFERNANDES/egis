IF EXISTS (SELECT name 
	   FROM   sysobjects 
	   WHERE  name = N'pr_egis_relatorio_analise_assistencia_tecnica' 
	   AND 	  type = 'P')
    DROP PROCEDURE pr_egis_relatorio_analise_assistencia_tecnica

GO

-------------------------------------------------------------------------------
--sp_helptext pr_egis_relatorio_analise_assistencia_tecnica
-------------------------------------------------------------------------------
--pr_egis_relatorio_analise_assistencia_tecnica
-------------------------------------------------------------------------------
...
------------------------------------------------------------------------------  
create procedure pr_egis_relatorio_analise_assistencia_tecnica  
@cd_relatorio int   = 0,  
@cd_usuario   int   = 0,  
@cd_parametro int   = 0,  
@json nvarchar(max) = ''   
...
declare @cd_objetivo int = 1  
declare @cd_tema     int = 2  
declare @vl_total           decimal(25,2) = 0.00  
declare @vl_meta            decimal(25,2) = 0.00  
declare @qt_equipamento  decimal(25,2) = 0.00  
...
declare @qt_pedido_analise  int = 0  
declare @vl_total_analise   decimal(25,2) = 0  
declare @qt_dia_mes      int           = 0  
  
--Valor da Meta dos Vendedores--  
  --set @dt_inicial = '06/01/2025'
  --set @dt_final = '06/30/2025'
select  
  @vl_meta = vl_periodo  
from  
  Objetivo_Tema_Valores v  
where  
  cd_objetivo = 1  
  and  
  cd_mes = @cd_mes  
...
select  
  @qt_base_cliente = count(c.cd_cliente)  
from  
  cliente c  
where  
...
select  
  a.dt_agenda AS dt_agenda,                             --Data da Ordem de Serviço  
  --o.dt_entrega as dt_entrega,  
  os.dt_ordem_servico as dt_ordem_servico,
  count(distinct os.cd_ordem_servico)      as qt_ordem_analise,  
  count(distinct os.cd_cliente)            as qt_cliente,  
    
  count(distinct os.cd_ordem_servico)      as qt_equipamento,  
  
  sum(case when isnull(o.vl_final_ordem_servico,0)>0   
      then o.vl_final_ordem_servico  
      else  
     case when isnull(o.vl_pagamento,0)>0 then  
    o.vl_pagamento  
  else  
...
from  
  agenda a  
  left outer join ordem_servico os             on os.dt_ordem_servico = a.dt_agenda  
  left outer join Ordem_Servico_Orcamento o    on o.cd_ordem_servico  = os.cd_ordem_servico  
  
where  
  os.dt_ordem_servico between @dt_inicial and @dt_final  
  and   
  o.vl_final_ordem_servico > 0  
  
group by  
  a.dt_agenda,  
  os.dt_ordem_servico
----------------------------------------------------------------------  
--2. Dedução do Total de Vendas  
--------------------------------  
  
  
select    
  @qt_total           = sum(qt_ordem_analise),  
  @qt_equipamento     = sum(qt_ordem_analise),  
  @vl_total           = SUM(vl_total_analise),  
  @qt_cliente         = sum(qt_cliente),  
  @qt_dia_mes         = COUNT(dt_ordem_servico)  
from  
  #Analise  
  
  
declare @vl_meta_dia decimal(25,2) = 0  
  
set @pc_meta     = case when @vl_meta>0 then round(@vl_total / @vl_meta * 100,2) else 0.00 end  
set @vl_meta_dia = @vl_meta/case when @qt_dia_mes>0 then @qt_dia_mes else 30 end  
...
select  
  ...
  f.*,  
  pc_faturamento = case when @vl_total > 0 then cast(round(vl_total_analise / @vl_total * 100,2) as decimal(25,2)) else 0.00 end,  
  @vl_meta_dia as vl_meta_diaria  
...
begin  
  ...
  begin  
   ...
   @nm_valor     = replace(CAST(dbo.fn_formata_valor(isnull(vl_total_analise,0)) as varchar(100)),'.',''),  
  
      @html_detalhe = @html_detalhe + '  
            <tr>                      
   <td style="font-size:12px; text-align:center;width: 20px">'+isnull(dbo.fn_data_string(dt_agenda),'')+'</td>  
   <td style="font-size:12px; text-align:center;width: 20px">'+cast(isnull(qt_ordem_analise,'') as varchar(20))+'</td>  
   <td style="font-size:12px; text-align:center;width: 20px">'+cast(isnull(dbo.fn_formata_valor(vl_total_analise),0) as varchar(20))+'</td>  
   <td style="font-size:12px; text-align:center;width: 20px">'+cast(isnull(pc_faturamento,0) as varchar(20))+'</td>  
   <td style="font-size:12px; text-align:center;width: 20px">'+cast(isnull(qt_cliente,0) as varchar(20))+'</td>  
   <td style="font-size:12px; text-align:center;width: 20px">'+cast(isnull(qt_equipamento,0) as varchar(20))+'</td>  
            </tr>'  
    from  
   #FinalEntrada  
...
<div class="secao4" id="sobre">  
  <div class="secao4-div">  
    ...
    <div>  
          <p>'+cast(isnull(@qt_total,0) as varchar(20))+'</p>  
    </div>  
      </div>  
   ...
   <div class="secao4-div-card">     
     ...
     <div class="card_separator"></div>  
          ...
          <p> '+'R$ '+isnull(dbo.fn_formata_valor(@vl_meta),0)+ '<br> (%) '+ cast(isnull(@pc_meta,0) as varchar(20))+'</p>  
      </div>  
    ...
    <div class="secao4-div-card">     
     ...
     <div class="card_separator"></div>  
          ...
          <p> '+'R$ '+isnull(dbo.fn_formata_valor(@vl_total),0)+'</p>  
      </div>  
    ...
    <div class="card_separator"></div>  
          ...
          <p>'+cast(cast(isnull(@qt_cliente,0) as int) as varchar(20))+'</p>  
      </div>  
    ...
    <div class="card_separator"></div>  
          ...
          <p>'+ cast(cast(isnull(@qt_equipamento,0) as int) as varchar(20))+'</p>  
      </div>  
...
'<body>  
  <div class="container mt-5">  
    ...
    <div class="separador" style="font-weight: bold; font-size: 18px">'
      + isnull(@titulo,'') + ' - Data Base : ' + dbo.fn_data_string(@dt_final) + 
    '</div>'  
...
set @html_cab_det = '<div class="section-title"><strong> '+isnull(@nm_grupo_relatorio,'') + ' </strong></div>   
                     <table>  
                     <tr>  
      <th>DATA</th>  
      <th>ORDEM SERVIÇO</th>  
...
begin  
 ...
 --@nm_pedido    = @nm_pedido  + ' ' + cast(cd_pedido_venda as varchar(15)),  
      ...
      <td style="font-size:12px; text-align:center;width: 20px">'+isnull(dbo.fn_data_string(dt_agenda),'')+'</td>  
      <td style="font-size:12px; text-align:center;width: 20px">'+cast(isnull(qt_ordem_analise,'') as varchar(20))+'</td>  
      <td style="font-size:12px; text-align:center;width: 20px">'+isnull(dbo.fn_formata_valor(vl_total_analise),0)+'</td>  
      <td style="font-size:12px; text-align:center;width: 20px">'+cast(isnull(pc_faturamento,0) as varchar(20))+'</td>  
      <td style="font-size:12px; text-align:center;width: 20px">'+cast(isnull(qt_cliente,0) as varchar(20))+'</td>  
      <td style="font-size:12px; text-align:center;width: 20px">'+cast(isnull(qt_equipamento,0) as varchar(20))+'</td>  
            </tr>'  
...
set @html_totais = '<div class="section-title">'+isnull(@titulo_total,'')+'  
     ...
     <td style="border: none;color: white;font-size:18px; text-align:center;width: 25px"><b>&nbsp</td>  
           ...
           <td style="border: none;color: white;font-size:18px; text-align:left;width: 45px"><b>'+'R$ '+isnull(dbo.fn_formata_valor(@vl_total),0)+'</td>   
     </tr>  
     </table>       
     </div>'  
...
set @titulo = @titulo + ' - Período : '+isnull(dbo.fn_data_string(@dt_inicial),'') + ' á '+isnull(dbo.fn_data_string(@dt_final),'')  
...
set @html_totais = '<div class="section-title"><strong>TOTAIS</strong>  
                    <div>   
     <table style="border: none;">  
                    <tr>            
     <td style="border: none;color: white;font-size:18px; text-align:center;width: 25px"><b>&nbsp</td>  
...
------------------------------------------------------------------------------
--Gráfico
--exec pr_egis_relatorio_analise_assistencia_tecnica 183,4253,3,'[{
--    "cd_empresa": null,
--    "cd_modulo": "385",
--    "cd_menu": "0",
--    "cd_parametro": 3,
--    "cd_relatorio_form": "336",
--    "cd_processo": "",
--    "cd_form": 32,
--    "cd_documento_form": 35,
--    "cd_parametro_form": "2",
--    "cd_usuario": "4595",
--    "cd_cliente_form": "0",
--    "cd_contato_form": "4595",
--    "dt_usuario": "2025-09-15",
--    "lookup_formEspecial": {},
--    "cd_parametro_relatorio": "35",
--    "cd_relatorio": "336",
