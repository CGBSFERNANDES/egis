IF EXISTS (SELECT name 
	   FROM   sysobjects 
	   WHERE  name = N'pr_egis_relatorio_posicao_contas_receber_portador' 
	   AND 	  type = 'P')
    DROP PROCEDURE pr_egis_relatorio_posicao_contas_receber_portador

GO

-------------------------------------------------------------------------------
--sp_helptext pr_egis_relatorio_posicao_contas_receber_portador
-------------------------------------------------------------------------------
--pr_egis_relatorio_posicao_contas_receber_portador
-------------------------------------------------------------------------------
...
--
------------------------------------------------------------------------------
create procedure pr_egis_relatorio_posicao_contas_receber_portador
@cd_relatorio int   = 0,
@cd_usuario   int   = 0,
@cd_parametro int   = 0,
@json nvarchar(max) = '' 
...
declare @vl_total_vencidas    decimal(25,2) = 0.00
declare @vl_total_hoje        decimal(25,2) = 0.00
declare @vl_total_mes         decimal(25,2) = 0.00
declare @vl_total_30          decimal(25,2) = 0.00
declare @vl_total_60          decimal(25,2) = 0.00
declare @vl_total_90          decimal(25,2) = 0.00
declare @vl_total_120         decimal(25,2) = 0.00
declare @vl_total_180         decimal(25,2) = 0.00
declare @qt_cliente           int
declare @vl_atraso            decimal(25,2) = 0.00
declare @dt_inicio_vencimento datetime
declare @dt_fim_vencimento    datetime
...
declare @qt_total int = 0
  ...
  select
	...
	count(dr.cd_documento_receber)                       as 'qt_documento',
	count(distinct c.cd_cliente_grupo)                   as 'qt_grupo_cliente',
	count(distinct dr.cd_cliente)                        as 'qt_cliente',
	count(distinct dr.cd_vendedor)                       as 'qt_vendedor',

    isnull(max(p.nm_portador),'Portador não Cadastrado') as 'Portador',
    --Vencidas
    sum( case when ((dr.dt_vencimento_documento < @dt_hoje) and (isnull (vl_saldo_documento,0)>0 )) then
    dr.vl_saldo_documento else 0 end ) as 'Vencidas',

	--hoje--
	sum( case when ((dr.dt_vencimento_documento = @dt_hoje) and (isnull (vl_saldo_documento,0)>0 )) then
    dr.vl_saldo_documento else 0 end ) as 'Hoje',
    ...
    sum( case when ((dr.dt_vencimento_documento between @dt_final+91 and @dt_final+120) ) then
    dr.vl_saldo_documento else 0 end ) as '120 dias',
      
    sum( case when ((dr.dt_vencimento_documento >= @dt_final+121 ) ) then
    dr.vl_saldo_documento else 0 end ) as '180 dias',

    sum ( dr.vl_saldo_documento )      as 'TotalGeral',

	min(dr.dt_vencimento_documento)      as 'dt_menor_vencimento',
	max(dr.dt_vencimento_documento)      as 'dt_maior_vencimento',
	MAX(isnull(tb.nm_logo_web_banco,'')) as nm_logo_banco_web
  into
    #AuxPosicao
  ...
  from
	...
	left outer join EGISADMIN.dbo.tabela_bancos tb on tb.cd_banco         = p.cd_banco
  ...
  group by 
    dr.cd_portador
  order by
    16 desc --dr.cd_portador



select
  @qt_total             = sum(qt_documento),
  @vl_total             = sum(TotalGeral),
  @vl_total_vencidas    = sum(vencidas),
  @vl_total_hoje        = sum(hoje),
  @vl_total_mes         = sum(mes),
  @vl_total_30          = sum([30 dias]),
  @vl_total_60          = sum([60 dias]),
  @vl_total_90          = sum([90 dias]),
  @vl_total_120         = sum([120 dias]),
  @vl_total_180         = sum([180 dias]),
  @qt_cliente           = SUM(qt_cliente),
  @vl_atraso            = SUM(vencidas),
  @dt_inicio_vencimento = MAX(dt_menor_vencimento),
  @dt_fim_vencimento    = max(dt_maior_vencimento)
from
  #AuxPosicao

select
 *,
 pc_receber = round(TotalGeral/@vl_total * 100,2)
 into
   #Posicao
...
select
 -- '999999'           as 'cd_controle',
   999               as 'cd_portador',	
	CAST(0 as int )  as 'qt_documento',
	CAST(0 as int )  as 'qt_grupo_cliente',
	CAST(0 as int )  as 'qt_cliente',
	CAST(0 as int )  as 'qt_vendedor',
  'TOTAL GERAL'      as 'Portador',
  @vl_total_vencidas as 'Vencidas',
  @vl_total_hoje     as 'Hoje',
  @vl_total_mes      as 'Mes',
  @vl_total_30       as '[30 Dias]',
  @vl_total_60       as '[60 Dias]',
  @vl_total_90       as '[90 Dias]',
  @vl_total_120      as '[120 Dias]',
  @vl_total_180      as '[180 Dias]',
  @vl_total          as 'TotalGeral',  
  @dt_hoje           as 'dt_menor_vencimento',
  @dt_hoje           as 'dt_maior_vencimento',
  cast('' as varchar(500)) as nm_logo_banco_web,
  0.00                     as 'pc_receber'
...
begin
  ...
  select top 1
    @id           = cd_controle,
    --Gráfico----
	  @nm_label_mensal = isnull(portador,''),
	  @nm_valor_mensal = replace(CAST(dbo.fn_formata_valor(isnull(TotalGeral,0)) as varchar(100)),'.',''),
    --
    ...
    @html_detalhe = @html_detalhe + '
            <tr> 					
            <td style="font-size:12px; text-align:center;width: 20px">'
			+
			'<img src="'+isnull(nm_logo_banco_web,'')+'" class="bank-logo">'
			+
			'</td>
            <td style="font-size:12px; text-align:center;width: 20px">'+isnull(Portador,'')+'</td>
			<td style="font-size:12px; text-align:center;width: 20px">'+isnull(dbo.fn_formata_valor([Vencidas]),'')+'</td>	
			<td style="font-size:12px; text-align:center;width: 20px">'+isnull(dbo.fn_formata_valor([Hoje]),'')+'</td>	
			<td style="font-size:12px; text-align:center;width: 20px">'+isnull(dbo.fn_formata_valor([30 dias]),'')+'</td>	
			<td style="font-size:12px; text-align:center;width: 20px">'+isnull(dbo.fn_formata_valor([60 dias]),'')+'</td>	
			<td style="font-size:12px; text-align:center;width: 20px">'+isnull(dbo.fn_formata_valor([90 dias]),'')+'</td>	
			<td style="font-size:12px; text-align:center;width: 20px">'+isnull(dbo.fn_formata_valor([120 dias]),'')+'</td>	
			<td style="font-size:12px; text-align:center;width: 20px">'+isnull(dbo.fn_formata_valor([180 dias]),'')+'</td>	
			<td style="font-size:12px; text-align:center;width: 20px"> R$ '+isnull(dbo.fn_formata_valor(TotalGeral),'')+'</td>	
			<td style="font-size:12px; text-align:center;width: 20px">'+isnull(dbo.fn_formata_valor(pc_receber),'')+'</td>	
			<td style="font-size:12px; text-align:center;width: 20px">'+isnull(dbo.fn_data_string(dt_menor_vencimento),'')+'</td>
			<td style="font-size:12px; text-align:center;width: 20px">'+isnull(dbo.fn_data_string(dt_maior_vencimento),'')+'</td>			
            </tr>'
...
<div class="secao4" id="sobre">
  <div class="secao4-div">
      ...
      <div class="secao4-div-card">   
          ...
          <p> '+'R$ '+dbo.fn_formata_valor(@vl_atraso)+'</p>
      </div>
      ...
      <div class="secao4-div-card">          
          ...
          <p>'+dbo.fn_Data_string(@dt_inicio_vencimento)+'</p>
      </div>
      ...
      <div class="secao4-div-card">          
          ...
          <p>'+dbo.fn_Data_string(@dt_fim_vencimento)+'</p>
      </div>
...
------------------------------------------------------------------------------
exec pr_egis_relatorio_posicao_contas_receber_portador 188,4253,3,''
------------------------------------------------------------------------------
