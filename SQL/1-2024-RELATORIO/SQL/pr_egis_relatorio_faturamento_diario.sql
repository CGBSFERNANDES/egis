IF EXISTS (SELECT name 
	   FROM   sysobjects 
	   WHERE  name = N'pr_egis_relatorio_faturamento_diario' 
	   AND 	  type = 'P')
    DROP PROCEDURE pr_egis_relatorio_faturamento_diario

GO

-------------------------------------------------------------------------------
--sp_helptext pr_egis_relatorio_faturamento_diario
-------------------------------------------------------------------------------
--pr_egis_relatorio_faturamento_diario
-------------------------------------------------------------------------------
...
--
------------------------------------------------------------------------------
create procedure pr_egis_relatorio_faturamento_diario
@cd_relatorio int   = 0,
@json nvarchar(max) = '' 
...
select
  --n.cd_serie_nota,
  n.dt_nota_saida,
  --max(snf.sg_serie_nota_fiscal)            as nm_serie_nota_fiscal,
 -- max(isnull(ef.nm_fantasia_empresa,''))   as nm_fantasia_empresa,
  ...
  sum( 0.00 )                              as vl_devolucao,
  sum( 0.00 )                              as vl_desconto,
...
while exists ( select cd_controle from #FinalFaturamentoSerie )
begin
  ...
  select top 1
    ...
    @html_detalhe = @html_detalhe + '
			...
			<td style="font-size:12px; text-align:center;width: 20px">'+cast(vl_devolucao as varchar(20))+'</td>	
			<td style="font-size:12px; text-align:center;width: 20px">'+cast(vl_desconto as varchar(20))+'</td>	
  ...
  delete from #FinalFaturamentoSerie
  where
    cd_controle = @id
...
------------------------------------------------------------------------------
exec pr_egis_relatorio_faturamento_diario 19,''
------------------------------------------------------------------------------
