IF EXISTS (SELECT name 
	   FROM   sysobjects 
	   WHERE  name = N'pr_egis_relatorio_recebimento_periodo' 
	   AND 	  type = 'P')
    DROP PROCEDURE pr_egis_relatorio_recebimento_periodo

GO

-------------------------------------------------------------------------------
--sp_helptext pr_egis_relatorio_recebimento_periodo
-------------------------------------------------------------------------------
--pr_egis_relatorio_recebimento_periodo
-------------------------------------------------------------------------------
...
--
------------------------------------------------------------------------------
create procedure pr_egis_relatorio_recebimento_periodo
@cd_relatorio int   = 0,
@cd_usuario   int   = 0,
@cd_parametro int   = 0,
@json nvarchar(max) = '' 
...
select
 --top 1
  ...
  max( isnull(c.nm_razao_social_cliente,''))                                as nm_razao_social_cliente,
  sum( isnull(d.vl_documento_receber,0))                                    as vl_total,
  SUM( isnull(d.vl_saldo_documento,0))                                      as vl_saldo,
  ...
  SUM(case when d.dt_vencimento_documento<@dt_hoje then
        isnull(p.vl_pagamento_documento,0)
	  else
	    0.00
      end
	)                                                                       as vl_pagamento,
  ...
  max( isnull(v.nm_fantasia_vendedor,''))                                               as nm_fantasia_vendedor,
  max( isnull(cg.nm_cliente_grupo,''))                                                  as nm_cliente_grupo
...
from
  Documento_Receber d
  left outer join Documento_Receber_Pagamento p   on p.cd_documento_receber   = d.cd_documento_receber
  left outer join Vendedor v                      on v.cd_vendedor            = d.cd_vendedor
  inner join Cliente c                            on c.cd_cliente             = d.cd_cliente
...
--select * from #FinalDiarioCliente
--return
...
--delete from #FinalDiarioCliente where cd_controle>5
...
begin
  ...
  while exists ( select top 1 cd_controle from #FinalDiarioCliente )
  begin
    ...
    select top 1 
      ...
      @html_detalhe = @html_detalhe + '
			...
			<td style="font-size:12px; text-align:center;width: 20px">'+CAST(nm_cliente_grupo as varchar(30)) +'</td>
			...
			<td style="font-size:12px; text-align:center;width: 20px">'+dbo.fn_formata_valor(vl_pagamento) +'</td>
            </tr>'
    from
	  #FinalDiarioCliente
	 ...
	 delete from #FinalDiarioCliente
	 where
	   cd_controle = @cd_controle

	 if exists(select top 1 cd_controle from #FinalDiarioCliente)
	 begin
	   set @labels  = @labels + ', '
	   set @valores = @valores + ', '
	 end
...
------------------------------------------------------------------------------
--Gráfico
exec pr_egis_relatorio_recebimento_periodo 185,4253,3,''
------------------------------------------------------------------------------
