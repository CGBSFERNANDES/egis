IF EXISTS (SELECT name 
	   FROM   sysobjects 
	   WHERE  name = N'pr_egis_relatorio_faturamento_empresa' 
	   AND 	  type = 'P')
    DROP PROCEDURE pr_egis_relatorio_faturamento_empresa

GO

-------------------------------------------------------------------------------
--sp_helptext pr_egis_relatorio_faturamento_empresa
-------------------------------------------------------------------------------
--pr_egis_relatorio_faturamento_empresa
-------------------------------------------------------------------------------
...
--
------------------------------------------------------------------------------
create procedure pr_egis_relatorio_faturamento_empresa
@cd_relatorio int   = 0,
@cd_usuario   int   = 0,
@json nvarchar(max) = '' 
...
select
  snf.cd_empresa_selecao       as cd_empresa,
  count(distinct n.cd_cliente) as qt_cliente
...
select
  ...
  max( 'R$ '+dbo.fn_formata_valor(ISNULL(0.00,0)))      as nm_meta,
  MAX(isnull(0,0))                                      as vl_meta,
  COUNT(distinct ce.cd_cliente)                         as qt_cliente,
  MAX(p.qt_cliente)                                     as qt_cliente_positivado
  
  into #EmpresaFaturamento

from  
  #Final#FaturamentoEmpresa f
  inner join Empresa_Faturamento ef    on ef.cd_empresa = f.cd_empresa
  inner join Cliente_Empresa ce        on ce.cd_empresa = f.cd_empresa 
  left outer join #ClientePositivado p on p.cd_empresa  = f.cd_empresa
...
begin
  ...
  select Top 1
	...
	@nm_empresa_faturamento = nm_fantasia_empresa, --+'     -    Meta: '+nm_meta ,
	@nm_meta              = nm_meta,
  ...
  --Total do Vendedor---------------------------
    ...
    set @html_cab_det = '<div class="section-title"><strong> '+@nm_empresa_faturamento + ' </strong></div>    
                     <table>
					 <tr>
					 <td>Meta</td>
					 <td>(%) Meta</td>
  ...
  begin
	...
	--@nm_pedido    = @nm_pedido  + ' ' + cast(cd_pedido_venda as varchar(15)),
     ...
     order by
        cd_empresa, cd_item, dt_nota_saida, cd_controle
...
set @html_totais = '<div class="section-title">'+@titulo_total+'
                    ...
                    <tr>										
			        <p style="border: none;color: white;font-size:18px; text-align:left;">Notas: <b>'+cast(@qt_total_vendedor as varchar(10))+'</p>
			        <p style="border: none;color: white;font-size:18px; text-align:left;">Valor: <b>'+'R$ '+dbo.fn_formata_valor(@vl_total_vendedor)+'</p>	
					</tr>
					</table>					
					</div>'
...
set @html_totais = '<div class="section-title"><strong>'+@titulo_total+'</strong>
					...
					<table style="border: none;">
                    <tr>										
			        <p style="border: none;color: white;font-size:18px; text-align:left;"><b>Notas: '+cast(@qt_total as varchar(10))+'</p>
			        <p style="border: none;color: white;font-size:18px; text-align:left;"><b>Valor: '+'R$ '+dbo.fn_formata_valor(@vl_total)+'</p>	
					</tr>
					</table>
					</div>
					</div>'
...
------------------------------------------------------------------------------
--exec pr_egis_relatorio_faturamento_empresa 142,1,''
------------------------------------------------------------------------------
