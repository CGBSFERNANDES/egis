IF EXISTS (SELECT name 
	   FROM   sysobjects 
	   WHERE  name = N'pr_egis_relatorio_veloe_historico_movimento' 
	   AND 	  type = 'P')
    DROP PROCEDURE pr_egis_relatorio_veloe_historico_movimento

GO
  
-------------------------------------------------------------------------------  
--sp_helptext pr_egis_relatorio_veloe_historico_movimento  
-------------------------------------------------------------------------------  
--pr_egis_relatorio_veloe_historico_movimento  
-------------------------------------------------------------------------------  
...
--  
------------------------------------------------------------------------------  
create procedure pr_egis_relatorio_veloe_historico_movimento  
@cd_relatorio int   = 0,  
@cd_parametro int   = 0,  
@json nvarchar(max) = ''   
...
if isnull(@cd_parametro,0) = 2    
 ...
 ------------------------------------------------------------------------------------------------------------------------- 
	...
	select 
	  ...
	  CONVERT(nvarchar(10), transactionDate, 103)   as transactionDate,
	  vehiclePlate                                  as vehiclePlate,
	  driverName                                    as driverName,
	  vehicleModel	                                as vehicleModel,
	  fuelType		                                as fuelType,
	  amountLiters	                                as amountLiters,
	  unitValue		                                as unitValue,
	  stockedValue	                                as stockedValue,
	  supplyLocation                                as supplyLocation
	  
	into
	  #movimentoVeloeRel
	from 
	  Veloe_Historico_Abastecimento 
	where
...
WHILE EXISTS (SELECT TOP 1 cd_controle FROM #movimentoVeloeRel)  
BEGIN  
    SELECT TOP 1  
        @id                          = cd_controle,  
        @html_geral = @html_geral +  
			'<tr class="tamanho">  
               <td>'+isnull(transactionDate,'')+'</td>  
               <td>'+isnull(vehiclePlate,'')+'</td>  
			   <td>'+isnull(driverName,'')+'</td>
			   <td>'+isnull(vehicleModel,'')+'</td>
			   <td>'+isnull(fuelType,'')+' </td>   
			   <td>'+cast(isnull(dbo.fn_formata_valor(amountLiters),'') as nvarchar(20))+'</td> 
			   <td>'+cast(isnull(dbo.fn_formata_valor(unitValue),'') as nvarchar(20)) +'</td>  
			   <td>'+isnull(supplyLocation,'') +'</td>  
			
			 </tr>'  
	
	FROM
	  #movimentoVeloeRel  
    DELETE FROM #movimentoVeloeRel WHERE cd_controle = @id  
END  
--------------------------------------------------------------------------------------------------------------------  
...
--exec pr_egis_relatorio_veloe_historico_movimento 305,0,''