IF EXISTS (SELECT name 
	   FROM   sysobjects 
	   WHERE  name = N'pr_relatorio_ordem_servico_liberada_area_tecnica' 
	   AND 	  type = 'P')
    DROP PROCEDURE pr_relatorio_ordem_servico_liberada_area_tecnica

GO

-------------------------------------------------------------------------------
--sp_helptext pr_relatorio_ordem_servico_liberada_area_tecnica
-------------------------------------------------------------------------------
--pr_relatorio_ordem_servico_liberada_area_tecnica
-------------------------------------------------------------------------------
...
--
------------------------------------------------------------------------------
create procedure pr_relatorio_ordem_servico_liberada_area_tecnica
@cd_relatorio int   = 0,
@json nvarchar(max) = '' 
...
select
  ...
  dbo.fn_data_string(oso.dt_pagamento + 180)    as garantia,
  dbo.fn_data_string(oso.dt_entrega)			As dt_entrega,
  oso.vl_pagamento                       as vl_pagamento,
   dbo.fn_data_string(osl.dt_liberacao_ordem) as dt_liberacao_ordem,
  dbo.fn_data_string(oso.dt_pagamento)   as dt_pagamento,
  --Cliente--
  c.nm_fantasia_cliente                  as nm_fantasia_cliente,
  c.nm_razao_social_cliente              as nm_razao_social_cliente,
  case when isnull(nm_razao_social_cliente,'') = '' 
   then isnull(nm_fantasia_cliente,'') 
   else isnull(nm_razao_social_cliente,'') end as cliente,
...
from
  ...
  left outer join ordem_servico_laboratorio osl  on osl.cd_ordem_servico        = os.cd_ordem_servico
  ...
  left outer join forma_pagamento fp           on fp.cd_forma_pagamento    = case when isnull(oso.cd_forma_pagamento,0) = 0 then os.cd_forma_pagamento
                                                                           else
																		     oso.cd_forma_pagamento
																		   end


	where
	 osl.dt_liberacao_ordem between @dt_inicial and @dt_final
	 and
...
set @html_geral = '<div class="section-title">  
				   ...
				   <table>
					   <tr>  
					    	<td class="tamanho" style="font-size: 110%;"><strong>Data</strong></td> 
							...
							<td class="tamanho" style="font-size: 110%;"><strong>Valor</strong></td>
							<td class="tamanho" style="font-size: 110%;"><strong>Forma</strong></td>
...
begin
	select top 1
        ...
        @html_geral = @html_geral +'<tr>
						...
						<td class="tamanho">'+isnull(garantia,'180')+'</td>
...
set @html_rodape =
   ' 
    ...
    <div class="section-title">
		...
		<p>Valor Total: '+cast(isnull(dbo.fn_formata_valor(@vl_total_os),0) as nvarchar(10))+'</td>
	</div>
    <p>'+@ds_relatorio+'</p>
...
------------------------------------------------------------------------------
--exec pr_relatorio_ordem_servico_liberada_area_tecnica 246,'' 
------------------------------------------------------------------------------
