...
declare          
  ...
  set @ic_desconto= (select isnull(ic_desconto_item_pedido,'N')          
                                 ...
                                 where          
                                   cd_empresa = @cd_empresa)     
SET @cd_documento = isnull(@cd_documento,1)
------------------------------------------------------------------------------------------------  
declare @hr_imp             nvarchar(5)
...
((pc.vl_total_pedido_compra + pc.vl_frete_pedido_compra           
  ...
  - (vl_desconto_pedido_compra)                           as VL_TOT_IPI_ST,
  -- Dados do Pedido -----------------------------------------------------------          
 ...
 from           	
  ...
  left outer join cidade cf                    with(nolock) on cf.cd_cidade                  = fo.cd_cidade          
 ...
 where  
  pc.cd_pedido_compra = isnull(@cd_documento,0) 

  
  declare @nm_fantasia_fornecedor          nvarchar(max) = ''   
  ...
  declare @cd_email_comprador            nvarchar(30) = ''
  declare @nm_cidade_uf_fornecedor       nvarchar(100) = ''
  declare @nm_bairro_for                  nvarchar(50) = ''
  declare @cd_cep_for                    nvarchar(20) = ''
  declare @nm_plano_financeiro           nvarchar(100) =''
  declare @vl_total_item                 float = 0
  declare @ds_observacao_pedido          nvarchar(500) = ''
  declare @nm_forma_pagamento            nvarchar(50) = ''
...
set @html_geral = '
			<td style="width: 20%; text-align: center;">
                <p><strong>PEDIDO: '+cast(isnull(@cd_documento,'')as nvarchar(20))+'</strong></p>
	   ...
	   <p class="section-title" style=" text-align: center;">'+@titulo+'</p>
    <table>
        <tr style=" text-align: center;">
            <td><strong>Comprador:</strong> '+isnull(@nm_comprador,'')+'</td>
            <td><strong>Email: </strong>'+cast(isnull(@cd_email_comprador,'') as nvarchar(20))+'</td>
            <td><strong>Aplicação: </strong>'+isnull(@nm_aplicacao_produto,'')+'</td>
        </tr>
    </table>
            ...
            <td style="display: flex; flex-direction: column; gap: 20px;">  
                <div style="display: flex; justify-content: space-between; align-items: flex-start; width: 100%;">  
                    ...
                    <div style="text-align: left;">  
                        <p><strong>Fone:</strong> '+cast(isnull(@cd_telefone_contato_forne,'') as nvarchar(100))+' </p>  
  ...
  select top 1  
    ...
    @html_detalhe = @html_detalhe + '  
           <tr>      
			...
			<td style="text-align: left;">'+cast(ISNULL(NM_PROD, '') as varchar(max))+' </td>
			<td>'+cast(ISNULL(MARCA, '') as varchar(max))+'</td>
			...
			<td>'+cast(ISNULL(dbo.fn_formata_valor(@VL_TCOM_DESC), 0)as nvarchar(20))+'</td> </td>
...
set @html_rodape ='
        ...
        <tr>  
            <td style="display: flex; flex-direction: column; gap: 20px;">  
                <div style="display: flex; justify-content: space-between; align-items: flex-start; width: 100%;">  
                    ...
                    <div style="text-align: left;">  
                        <p><strong>Total Item:</strong> '+cast(ISNULL(dbo.fn_formata_valor(@vl_total_item), 0)as nvarchar(20))+' </p>  
                        <p><strong>Frete:</strong> '+cast(ISNULL(dbo.fn_formata_valor(@vl_frete_pedido_compra), 0)as nvarchar(20))+' </p>  
                        <p><strong>IPI:</strong> '+cast(ISNULL(dbo.fn_formata_valor(@VL_TOT_IPI), 0)as nvarchar(20))+' </p>  
                        <p><strong>ICMS-ST:</strong> '+cast(ISNULL(dbo.fn_formata_valor(@VL_TOT_ICMS), 0)as nvarchar(20))+' </p> 
                        <p><strong>Desconto:</strong> '+cast(ISNULL(dbo.fn_formata_valor(@vl_desconto_pedido_compra), 0)as nvarchar(20))+' </p> 
                        <p><strong>Total:</strong> '+cast(ISNULL(dbo.fn_formata_valor(@VL_TOTAL_GERAL), 0)as nvarchar(20))+' </p> 
                    </div>     
                </div>  
            </td>  
        </tr>  
        ...
        <tr>
            <td style="text-align: left;">
                <div>
                    ...
                    <p>'+ISNULL(@nm_condicao_pagamento_pc,'')+'</p>
                </div>
            </td>
        </tr>
    </table>
    <table>
        ...
        <td></td>
        <td>'+isnull(dbo.fn_data_string(@dt_vencimento_adiantamento),'')+'</td>
        <td>'+cast(ISNULL(dbo.fn_formata_valor(@vl_pedido_compra), 0)as nvarchar(20))+'</td>
