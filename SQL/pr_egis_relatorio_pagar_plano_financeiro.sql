IF EXISTS (SELECT name 
	   FROM   sysobjects 
	   WHERE  name = N'pr_egis_relatorio_pagar_plano_financeiro' 
	   AND 	  type = 'P')
    DROP PROCEDURE pr_egis_relatorio_pagar_plano_financeiro
	
GO

-------------------------------------------------------------------------------
--sp_helptext pr_egis_relatorio_pagar_plano_financeiro
-------------------------------------------------------------------------------
--pr_egis_relatorio_pagar_plano_financeiro
-------------------------------------------------------------------------------
...
--
------------------------------------------------------------------------------
create procedure pr_egis_relatorio_pagar_plano_financeiro
@cd_relatorio int   = 0,
@cd_usuario   int   = 0,
@cd_parametro int   = 0,
@json nvarchar(max) = '' 
...
declare @qt_total_fornecedor int = 0

 
--use egissql_279
--go
----select (row_number() OVER(ORDER BY nm_fantasia_fornecedor)) as id, * from fornecedor order by nm_fantasia_fornecedor
...
select
  isnull(d.cd_plano_financeiro,0)                                           as cd_plano_financeiro,
  MAX(isnull(pf.cd_mascara_plano_financeiro,''))                            as cd_mascara_plano_financeiro,
  MAX(isnull(pf.nm_conta_plano_financeiro,''))                              as nm_conta_plano_financeiro,
  sum( isnull(d.vl_documento_pagar,0))                                      as vl_total,
  SUM( isnull(d.vl_saldo_documento_pagar,0))                                as vl_saldo,
  SUM(case when d.dt_vencimento_documento<@dt_hoje then
...
--declare @vl_total decimal(25,2) = 0.00
declare @qt_documento      decimal(25,2) = 0.00
declare @qt_total_controle decimal(25,2) = 0.00

select  
  @qt_total_fornecedor  = count(cd_plano_financeiro),
  @vl_total             = SUM(vl_total),
  @qt_total             = sum(qt_documento),
  @vl_atraso            = sum(vl_atraso),
  @qt_total_controle    = count(vl_atraso),
...
select
  ...
  f.*,
  pc_total = cast(round(vl_total/@vl_total * 100,2) as decimal(25,2))

into
  #FinalAbertoFornecedor
from
  #AbertoFornecedor f
...
--return
 ...
 begin
    select * from  #FinalAbertoFornecedor
  return
 end
...
begin
  ...
  while exists ( select top 1 cd_controle from #FinalAbertoFornecedor )
  begin

    select top 1 
	  @cd_controle  = cd_controle,
	  @nm_label     = dbo.fn_data_string(dt_vencimento),
      ...
      @html_detalhe = @html_detalhe + '
			...
			<td style="font-size:12px; text-align:center;width: 20px">'+CAST(cd_mascara_plano_financeiro as varchar(30)) +'</td>
			<td style="font-size:12px; text-align:center;width: 20px">'+CAST(nm_conta_plano_financeiro as varchar(30)) +'</td>
			<td style="font-size:12px; text-align:center;width: 20px">'+dbo.fn_data_string(dt_menor_vencimento)+'</td>
			<td style="font-size:12px; text-align:center;width: 20px">'+dbo.fn_data_string(dt_vencimento)+'</td>
			<td style="font-size:12px; text-align:center;width: 20px">'+cast(qt_documento as varchar(20))+'</td>
			<td style="font-size:12px; text-align:center;width: 20px">'+dbo.fn_formata_valor(vl_total)   +'</td>
			<td style="font-size:12px; text-align:center;width: 20px">'+cast(pc_total as varchar(20))    +'</td>
			<td style="font-size:12px; text-align:center;width: 20px">'+dbo.fn_formata_valor(vl_atraso)   +'</td>
    ...
    from
	 ...
	 delete from #FinalAbertoFornecedor
	 where
	   cd_controle = @cd_controle

	 if exists(select top 1 cd_controle from #FinalAbertoFornecedor)
	 begin
	   set @labels  = @labels + ', '
	   set @valores = @valores + ', '
	 end
  ...
  --Dados da Tabela---
   ...
   set @tabela = '<div class="container mt-5">
    <h2 class="mb-4">'+'Demonstrativo'+'</h2>
...
<div class="secao4" id="sobre">
  <div class="secao4-div">
      ...
      <div class="secao4-div-card">   
          ...
          <p> '+'R$ '+dbo.fn_formata_valor(@vl_atraso)+ '<br></p>
      </div>
...
begin
 ...
 set @html_detalhe = @html_detalhe +
  '<td style="font-size:16px; text-align:center;width: 20px"><strong>Total</strong></td>
  <td style="font-size:12px; text-align:center;width: 20px"></td>
  <td style="font-size:12px; text-align:center;width: 20px"></td>
  <td style="font-size:12px; text-align:center;width: 20px"></td>
  <td style="font-size:12px; text-align:center;width: 20px"></td>
  <td style="font-size:16px; text-align:center;width: 20px"><strong>'+cast(isnull(@qt_total,0) as varchar(10))+'</strong></td>
  <td style="font-size:16px; text-align:center;width: 20px"><strong>'+cast(isnull(dbo.fn_formata_valor(@vl_total),'') as nvarchar(20))+'</strong></td>
  <td style="font-size:12px; text-align:center;width: 20px"></td> 
  <td style="font-size:16px; text-align:center;width: 20px"><strong>'+cast(isnull(dbo.fn_formata_valor(@vl_atraso),'') as nvarchar(20))+'</strong></td>
  '
...
set @html_totais = '
					<div class="section-title"><strong>'+@titulo_total+'</strong>
                    <div> 
...
set @html_rodape =
    ...
    <div class="section-title">
		<p>TOTAL DE DOCUMENTOS: '+CAST(ISNULL(@qt_documento,0) AS NVARCHAR(20))+'</p>
	</div>
...
------------------------------------------------------------------------------
--Gráfico
exec pr_egis_relatorio_pagar_plano_financeiro 185,4253,3,''
------------------------------------------------------------------------------
